...
res={}
me=P.groupby(P.index.to_period('M')).tail(1)   # last TRADING day (calendar month-end drops weekend months)
# 1 baseline
res['SPY buy&hold']=run(pd.DataFrame({'SPY':1.0},index=R.index),'')
# 2 60/40-ish equal weight 12
res['EqualWeight 12 (monthly)']=run(pd.DataFrame(1/12,index=me.index,columns=R.columns).reindex(R.index).ffill(),'')
# vol
vol=R.rolling(60).std()*np.sqrt(252)
# 3 TSMOM 12m vol-target 10%/asset /N
sig=np.sign(me.pct_change(12)).reindex(R.index).ffill()
W=(sig*(0.10/vol)/12).clip(-0.5,0.5); W=W.loc[me.index].reindex(R.index).ffill()
...
iv=(1/vol[['SPY','TLT','GLD']]); iv=iv.div(iv.sum(axis=1),axis=0).loc[me.index].reindex(R.index).ffill()
...
wk=P[SECT].groupby(P.index.to_period('W-FRI')).tail(1); r5=wk.pct_change(); rk=r5.rank(1); Wr=(rk<=3).astype(float)/3
...
for nm,s in [('SPY overnight only (2 trades/day)',on-2*COST),('SPY intraday only',intr-2*COST)]:
