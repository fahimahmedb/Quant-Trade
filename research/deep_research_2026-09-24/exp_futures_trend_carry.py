...
def port(F,name):
    ...
    pnl=G-(pos.diff().abs()*Cc.fillna(Cc.stack().median())).fillna(0).where(G.notna())
    n=pnl.notna().sum(axis=1); pr=pnl.sum(axis=1)/np.sqrt(n.clip(lower=1))  # equal risk, sqrt-N diversification scaling
    pr=pr[n>=5]
    f=lambda x:x.mean()/x.std()*np.sqrt(256)
    out={'SR':f(pr),'t':f(pr)*np.sqrt(len(pr)/256),'N_avg':n[n>=5].mean()}
...
pnl=(G-(pos.diff().abs()*Cc.fillna(Cc.stack().median())).fillna(0).loc['2000':].where(G.notna()))
ok=[c for c in pnl.columns if pnl[c].loc[:'2001'].notna().sum()>200]
rng=np.random.default_rng(1); f=lambda x:x.mean()/x.std()*np.sqrt(256)
