...
def run(true_sr,h1_sr,days=252*10,n=2000):
    ...
    d=np.array([o[0] for o in out]); acc=np.mean([o[1] is not None and bool(o[1]) for o in out]); und=np.mean([o[1] is None for o in out])
    return np.median(d)/21, np.percentile(d,90)/21, acc, und
print('H1 = SR target | true SR | median months to decision | p90 months | P(accept H1)')
for h1 in [1,2,4]:
    for t in [0,-0.5,h1/2,h1]:
        m,p,a,u=run(t,h1); print(f'  H1={h1}  true={t:5.1f}  median={m:6.1f}  p90={p:6.1f}  accept={a:.2f}  undecided@10y={u:.2f}')
print('Caveat: sigma assumed known; if realized vol is 1.5x assumed, false acceptance at SR=0 rises to ~17-20% -> use t-SPRT / estimated sigma.')
