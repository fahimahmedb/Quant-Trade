...
def render_status(snapshot: dict[str, Any]) -> str:
    ...
    for name in ("CONTROL", "DATA", "SEC_CAPTURE", "RESEARCH", "LEARNING", "BUILD"):
        status = components.get(name, {})
        detail = status.get("detail") or ""
        lines.append(f"{name:<13}{status.get('state', 'UNKNOWN'):<10}{detail[:51]}")
    ...
    if capture:
        ...
        lines += [
            ...
            _row("FINGERPRINT", "active=materialized "
                 + str(capture.get("fingerprint_matches_materialized"))),
            _row("STORAGE", f"append-only {storage.get('append_only')}   "
                            f"content-addressed {storage.get('content_addressed')}"),
            _row("REQUEST POLICY", f"{policy.get('max_requests_per_second', '?')}/s cap, "
                                   f"concurrency {policy.get('max_concurrency', '?')}, "
                                   f"poll cadence {policy.get('discovery_poll_seconds', '?')}s"),
            _row("LIFECYCLE", str(capture.get("lifecycle_cause") or "unattested")),
