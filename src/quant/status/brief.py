...
def build_chief_brief(snapshot: dict[str, Any]) -> str:
    ...
    if capture:
        ...
        lines += ["", "## SEC Form-4 raw capture (P0 acquisition lane)", "",
                  ...
                  f"- active/materialized fingerprint match: "
                  f"{capture.get('fingerprint_matches_materialized')}",
                  f"- storage: append-only {storage.get('append_only')}, "
                  f"content-addressed {storage.get('content_addressed')}",
                  f"- frozen request policy: {policy.get('max_requests_per_second')} req/s, "
                  f"concurrency {policy.get('max_concurrency')}, "
                  f"poll cadence {policy.get('discovery_poll_seconds')}s",
                  f"- lifecycle cause: {capture.get('lifecycle_cause')}",
