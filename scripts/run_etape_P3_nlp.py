...
def main():
    ...
    w("mais celle-ci est à ce jour un **STUB NON CÂBLÉ** -- pas un vrai appel réseau comme pour")
    w("les marchés (`pp_markets._fetch_live`, qui interroge réellement Polymarket). `pytrends`")
    w("n'est pas une dépendance du projet, l'import échoue donc immédiatement et **aucune requête")
    w("réseau n'est jamais émise** -- repli systématique et instantané sur le snapshot offline")
    w("`data/fr_nlp_snapshot.csv`. Le mapping mot-clé → candidat et le moteur de tonalité restent")
    w("à écrire avant qu'une collecte live soit seulement possible.\n")
    ...
    w("scoré, n=0), et sa collecte live (`_fetch_live`) est un stub non câblé, pas une intégration")
    w("fonctionnelle. Les poids `w1`/`w2` restent des choix a priori. Son seul test honnête")
    w("possible est un scrutin futur (2027), avec pytrends effectivement branché et des features")
    w("collectées avant le vote -- pas de remplacer un sondage, et certainement pas de faire une")
    w("prévision autonome.\n")
