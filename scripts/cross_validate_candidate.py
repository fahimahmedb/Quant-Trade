...
protocole (Russell 2000, S&P 500, DAX).
...
def main():
    ...
    for idx_name, fname in INDICES.items():
        ...
        if not path.exists():
            lines.append(f"- {idx_name}: fichier absent, skip")
            continue
        ...
        n = len(df)
        if n <= T0 + 100:
            lines.append(f"- {idx_name}: historique trop court ({n} lignes), skip")
            continue

        missing = [f for f in features if f not in X.columns]
        if missing:
            lines.append(f"- {idx_name}: features manquantes {missing}, skip")
            continue
        ...
        lines.append(f"- {idx_name}: n={n}, OOS obs={len(oos)}, Sharpe ann = {sharpe_ann:.3f}")
