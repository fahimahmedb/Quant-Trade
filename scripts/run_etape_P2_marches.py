...
def main():
    ...
    w("1. Tentative **live** (`_fetch_live`, timeout court de "
      f"{LIVE_TIMEOUT_S:.0f} s) — RÉELLEMENT câblée : une requête HTTP est")
    w("   effectivement envoyée à l'API publique Polymarket (gamma API). Mais elle renvoie")
    w("   systématiquement `None` à ce jour, faute de marché mappable : soit aucun marché FR 2027")
    w("   n'est encore listé, soit un marché générique existe mais le mapping outcome→camp de")
    w("   référence n'est pas déclaré (finalistes 2027 inconnus). Ce n'est donc PAS un stub, mais")
    w("   ce n'est pas non plus une source opérationnelle : aucun appel réel n'a encore produit de")
    w("   prix exploitable.")
    ...
    w("")
    w("3. **Live réellement câblé mais sans résultat exploitable** : `_fetch_live` interroge pour")
    w("   de vrai l'API Polymarket, mais renvoie toujours `None` (aucun marché mappable outcome→")
    w("   camp de référence à ce jour) — le fallback snapshot reste donc la voie d'exécution")
    w("   normale de ce dépôt, pas un filet de sécurité occasionnel.")
    w("")
    ...
    w("")
    w("**Conclusion** : à ce stade, cette source est un **échafaudage 2027-live**, pas une brique")
    w("opérationnelle démontrée — elle n'a **aucune skill mesurée** sur données réelles (0 pli")
    w("scoré, n=0). Le débiaisage favori-outsider et la pente de conversion en part de vote sont")
    w("des priors motivés par la littérature, non validés dans ce dépôt. Son seul test honnête")
    w("possible est un scrutin futur (2027) avec un prix effectivement horodaté avant le vote.\n")
