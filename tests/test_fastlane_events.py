...
TR_COLS = list(REQUIRED_COLUMNS["NONDERIV_TRANS.tsv"])
...
def synthetic_zip(submissions, owners, trans) -> bytes:
    buf = io.BytesIO()
...
def sub(acc, doc="4", filing="15-FEB-2010", issuer="0000000001", ticker="SYNA"):
    return {"ACCESSION_NUMBER": acc, "FILING_DATE": filing, "PERIOD_OF_REPORT": "11-FEB-2010",
            ...
            "ISSUERNAME": "SYNTHETIC ISSUER", "ISSUERTRADINGSYMBOL": ticker}
...
def trans(acc, sk, code="P", acq="A", shares="1000", price="10.5", tdate="11-FEB-2010"):
    return {"ACCESSION_NUMBER": acc, "NONDERIV_TRANS_SK": sk, "SECURITY_TITLE": "Common Stock",
...
class PopulationRuleTests(unittest.TestCase):
    def build(self, subs, owners, trs, quarter="2010q1"):
        data = synthetic_zip(subs, owners, trs)
...
class DuplicateAccessionTests(unittest.TestCase):
    ...
    def test_build_all_is_deterministic(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(first["events"]["rows"], 2)
