...
def rogers_satchell_var_pct(df: pd.DataFrame) -> pd.Series:
    """Variance quotidienne range-based de Rogers-Satchell (1991), en %^2.

    Contrairement a Parkinson et Garman-Klass, robuste a un DRIFT non nul
    intra-seance (ne suppose pas une moyenne nulle des rendements dans la
    seance). ATTENTION : comme les deux autres, ne capture pas le saut de
    la cloture veille a l'ouverture du jour (overnight gap).
    """
    hc = 100 * np.log(df["high"] / df["close"])
    ho = 100 * np.log(df["high"] / df["open"])
    lc = 100 * np.log(df["low"] / df["close"])
    lo = 100 * np.log(df["low"] / df["open"])
    v = hc * ho + lc * lo
    v.index = df["date"]
    return v.iloc[1:]  # aligne sur les rendements (1re obs sans rendement)
