"""Backtest — Corrélation cross-marché NDX-DAX, overlay défensif
(spécification pré-enregistrée dans
PREREG_cross_market_correlation_ndx_dax_overlay.md, committée avant ce
script). n_trials=1, aucune dépendance ML. Règle de succès renforcée sur
5 marchés (≥4/5).

Réutilise la fenêtre de corrélation 60j du #90, l'alignement causal
ffill+shift(1) déjà validé aux #175/#178/#186/#187/#191/#192, CUT=0,5 et
...
def build_corr_series() -> pd.Series:
    ...
    corr = ndx_c.rolling(CORR_WINDOW).corr(dax_c)
    return corr.dropna()
...
def expanding_tercile_cut_high(corr: np.ndarray) -> np.ndarray:
    ...
    (corrélation élevée = défavorable, direction réutilisée du #90)."""
    T = len(corr)
...
def main():
    ...
    lines = [
        ...
        "",
        f"`position(t) = {CUT}x` si corr(t)=Pearson(NDX,DAX) 60j est dans son tercile "
    ...
    out = ROOT / "results" / "nonml_cross_market_correlation_ndx_dax_overlay_result.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
    return verdict
