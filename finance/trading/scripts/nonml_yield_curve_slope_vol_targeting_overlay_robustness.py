"""Robustesse — Overlay vol-targeting gaté par la pente de la courbe des
taux US. Grille CAP ET grille de fenêtre de vol (PAS un retuning de
MEDIAN_WINDOW=252j, paramètre de définition), autour des valeurs
pré-enregistrées.
"""
import sys
...
def main():
    ...
    lines = [
        "# Robustesse — Overlay vol-targeting gaté par pente courbe des taux US (grilles CAP et fenêtre, PAS un retuning de MEDIAN_WINDOW=252j)",
        "",
    ...
    out = ROOT / "results" / "nonml_yield_curve_slope_vol_targeting_overlay_robustness.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
