...
de la courbe des taux US (NDX), ~3 derniers mois. Spécification
pré-enregistrée (CAP=2.0x), aucun paramètre retouché après les résultats
précédents.
"""
import sys
...
def main():
    ...
    lines = [
        ...
        "de robustesse parfait 8/8, testé sur 40 ans d'historique complet couvrant les crises "
        "2000-02/2008) doit encore passer la batterie de validation renforcée "
        "(`nonml_pass_validation_battery.py yield_curve_slope_vol_targeting_overlay`) avant toute "
        "déclaration finale."
    ]

    out = ROOT / "results" / "nonml_yield_curve_slope_vol_targeting_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
