"""Robustesse — Momentum de constance + overlay SMA200. Grille CAP (PAS
un retuning), autour du CAP pré-enregistré (2.0x).
"""
import sys
...
from nonml_momentum_consistency_sma200_overlay_backtest import (  # noqa: E402
    load_all_prices, index_trend_series, consistency_at, LOOKBACK, REBAL_EVERY, TERCILE, COST_BPS,
)
...
def main():
    ...
    pnl_base = (weights_cons[start:] * R[start:]).sum(axis=1)
    turn_base = np.abs(np.diff(weights_cons[start:], axis=0, prepend=weights_cons[start:start+1])).sum(axis=1) / 2.0
    pnl_base = pnl_base - turn_base * (COST_BPS / 1e4)
    me_base = trading_metrics(pnl_base)
    ret_base = np.cumprod(1.0 + pnl_base)[-1] - 1.0

    lines = [
        "# Robustesse — Momentum de constance + overlay SMA200 (grille CAP, PAS un retuning)",
        "",
    ...
    out = ROOT / "results" / "nonml_momentum_consistency_sma200_overlay_robustness.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
