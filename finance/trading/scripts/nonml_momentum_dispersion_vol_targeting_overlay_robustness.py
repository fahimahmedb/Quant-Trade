"""Robustesse — Overlay vol-targeting gaté par la dispersion du
momentum NDX-100. Grille CAP ET grille de fenêtre de vol (PAS un
retuning de la porte LOOKBACK=252/SKIP=21/MEDIAN_WINDOW=252), autour
des valeurs pré-enregistrées (CAP=2.0x, fenêtre=20j).
"""
import sys
...
from nonml_momentum_dispersion_vol_targeting_overlay_backtest import (  # noqa: E402
    compute_momentum_dispersion_series, MEDIAN_WINDOW, COST_BPS, TARGET_VOL_ANNUAL, ANNUALIZATION,
)
...
def main():
    ...
    lines = [
        "# Robustesse — Overlay vol-targeting gaté par la dispersion du momentum (grilles CAP et fenêtre, PAS un retuning de la porte)",
        "",
    ...
    out = ROOT / "results" / "nonml_momentum_dispersion_vol_targeting_overlay_robustness.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
