"""Backtest — Durée hebdomadaire moyenne du travail, secteur
manufacturier US (FRED AWHMAN), overlay défensif (spécification
pré-enregistrée dans PREREG_manufacturing_hours_overlay.md, committée
...
convention que #195/#203/#323/#324/#326/#328/#331.
"""
import sys
...
PUBLICATION_LAG_MONTHS = 1  # rapport BLS "Employment Situation" (meme que PAYEMS #326)


def build_manufacturing_hours_series() -> pd.Series:
    ...
    # Publication le 1er vendredi du mois suivant (rapport BLS
    # "Employment Situation", meme rapport que le PAYEMS #326) :
    # decalage conservateur d'UN MOIS calendaire complet AVANT le ffill.
...
def load_manufacturing_hours_lag(dates: pd.DatetimeIndex, hours_series: pd.Series) -> np.ndarray:
    y = hours_series.reindex(dates, method="ffill")
...
def main():
    ...
    lines = [
        ...
        "",
        f"`position(t) = {CUT}x` si AWHMAN_lag(t-1) est dans son tercile expanding le plus BAS "
        f"(durée hebdomadaire la plus faible observée à ce jour = signal avant-coureur), `1.0x` sinon. "
    ...
    for name, fname in MARKETS.items():
        ...
        hours_lag_full = load_manufacturing_hours_lag(dates, hours_series)
        hours_lag = hours_lag_full[1:]
        pos_full = expanding_tercile_cut_low(hours_lag)
        ...
        if name == "NDX (40 ans)":
            ...
            np.savez(ROOT / "results" / "nonml_manufacturing_hours_overlay_pnl.npz",
    ...
    out = ROOT / "results" / "nonml_manufacturing_hours_overlay_result.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
    return verdict
