"""Simulation — 300 EUR dans l'overlay vol-targeting cible 20% (NDX), ~3
derniers mois. Spécification pré-enregistrée (CAP=2.0x, fenêtre=20j),
...
from nonml_vol_targeting_20_overlay_backtest import vol_target_position, COST_BPS, CAP, VOL_WINDOW, TARGET_VOL_ANNUAL  # noqa: E402
...
def main():
    ...
    lines = [
        "# Simulation — 300 EUR, overlay vol-targeting cible 20% (NDX, ~3 derniers mois)",
        "",
        ...
        f"| **Overlay vol-targeting 20%** | **{equity_ov[-1]:.2f} EUR** | "
    ...
    out = ROOT / "results" / "nonml_vol_targeting_20_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
