"""Robustesse — Leaders 52-semaines + overlay SMA200. Grille CAP (PAS un
...
from nonml_sma200_leaders_overlay_backtest import (  # noqa: E402
    load_prices, index_trend_series, LOOKBACK, REBAL_EVERY, TERCILE, COST_BPS,
)
...
def main():
    ...
    lines = [
        "# Robustesse — Leaders 52-semaines + overlay SMA200 (grille CAP, PAS un retuning)",
        "",
    ...
    out = ROOT / "results" / "nonml_sma200_leaders_overlay_robustness.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
