"""Backtest — Anticipations d'inflation implicites (breakeven 10 ans), overlay défensif
(spécification pré-enregistrée dans PREREG_inflation_breakeven_overlay.md,
...
#175/#178/#186/#187/#191/#192/#193/#195/#196/#197/#198/#199, CUT=0,5 et
la technique de tercile expanding déjà établie aux #169/#177/#183/#191/
#192/#193/#195/#196/#197/#198/#199, Règle 7.
"""
import sys
...
def load_t10yie_lag(dates: pd.DatetimeIndex) -> np.ndarray:
    raw = pd.read_csv(REPO_ROOT / "data" / "t10yie_daily.csv")
    raw["observation_date"] = pd.to_datetime(raw["observation_date"])
    s = raw.set_index("observation_date")["T10YIE"].astype(float).dropna()
...
def main():
    lines = [
        ...
        "",
        f"`position(t) = {CUT}x` si T10YIE_lag(t) est dans son tercile expanding le plus haut, "
    ...
    for name, fname in MARKETS.items():
        ...
        pos_full = expanding_tercile_cut_high(t10yie_lag)
        ...
        if name == "NDX (40 ans)":
            ...
            np.savez(ROOT / "results" / "nonml_inflation_breakeven_overlay_pnl.npz",
    ...
    out = ROOT / "results" / "nonml_inflation_breakeven_overlay_result.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
    return verdict
