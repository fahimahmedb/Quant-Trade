"""Simulation — 300 EUR dans l'overlay vol-targeting gaté par le
clustering ARCH glissant (NDX), ~3 derniers mois. Spécification
pré-enregistrée (ARCH_WINDOW=252, MEDIAN_WINDOW=252, CAP=2.0x,
VOL_WINDOW=20), aucun paramètre retouché après les résultats précédents.
"""
import sys
...
from nonml_arch_clustering_vol_targeting_overlay_backtest import (  # noqa: E402
    strong_clustering_mask, combined_position, COST_BPS, CAP, VOL_WINDOW,
    TARGET_VOL_ANNUAL, ARCH_WINDOW, MEDIAN_WINDOW,
)
...
def main():
    ...
    gate_all = strong_clustering_mask(bh_full_all)
    ...
    lines = [
        ...
        f"({len(bh_full)} séances). Fenêtre ARCH/médiane {ARCH_WINDOW}j/{MEDIAN_WINDOW}j, "
        f"vol cible {TARGET_VOL_ANNUAL:.0%}, fenêtre vol {VOL_WINDOW}j, CAP={CAP}x.",
        "",
        "| | Capital final | Rendement période | MDD | Sharpe ann. |",
        "|---|---|---|---|---|",
        f"| BuyHold | {equity_bh[-1]:.2f} EUR | {100*(equity_bh[-1]/CAPITAL0-1):+.1f}% | "
        f"{mdd(equity_bh):.1f}% | {me_bh['sharpe_ann']:+.2f} |",
        f"| **Overlay vol-targeting clustering ARCH gaté** | **{equity_ov[-1]:.2f} EUR** | "
        ...
        "Composite échoue) et de la robustesse (plateau correct mais pas parfait, dip à 2/5 marchés "
        "à fenêtre=15j et 3/5 à CAP=3.0x)."
    ]

    out = ROOT / "results" / "nonml_arch_clustering_vol_targeting_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
