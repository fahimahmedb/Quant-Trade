"""Audit indépendant — Momentum du fret maritime en vrac sec (BDRY,
log-return 21j).

Recalcule BDRYmom_lag(t), le tercile expanding et la position par une
...
from nonml_dry_bulk_shipping_overlay_backtest import load_bdry_mom_lag, load_bdry_series  # noqa: E402


def manual_bdry_mom_lag(dates: pd.DatetimeIndex) -> np.ndarray:
    ...
    raw = raw.dropna(subset=["BDRY"]).drop_duplicates("observation_date").sort_values("observation_date")
    obs_dates = raw["observation_date"].values
    vals = raw["BDRY"].astype(float).values
...
def main():
    lines = ["# Audit — Momentum du fret maritime en vrac sec (BDRY, log-return 21j)", ""]
    all_ok = True
    ...
    for name, fname in MARKETS.items():
        ...
        mom_official = load_bdry_mom_lag(dates, bdry_series)[1:]
        pos_official = expanding_tercile_cut_low(mom_official)

        mom_manual = manual_bdry_mom_lag(dates)[1:]
        ...
        if n_pos_diff == 0:
            ...
            lines.append(f"- **{n_pos_diff}/{n_total} désaccords de position** — sensibilité de bord flottante "
                         f"documentée (même pattern que #193/#195/.../#354) : "
    ...
    # derniere transition de valeur reelle.
    raw = pd.read_csv(REPO_ROOT / "data" / "bdry_shipping_daily.csv")
    raw["observation_date"] = pd.to_datetime(raw["observation_date"])
    raw["BDRY"] = pd.to_numeric(raw["BDRY"], errors="coerce")
    raw = raw.dropna(subset=["BDRY"]).sort_values("observation_date")
    vals = raw["BDRY"].astype(float).values
    obs_dates = raw["observation_date"].values
    diffs = np.diff(vals)
    ...
    bdry_check = load_bdry_mom_lag(check_dates, bdry_series)

    lines.append(f"## Vérification spécifique du décalage causal d'un jour (transition {d_prev.date()}→{d_last.date()})")
    lines.append(f"- BDRY brut jour précédent ({d_prev.date()}) = {v_prev:.6f}, "
                 f"BDRY brut jour de la transition ({d_last.date()}) = {v_last:.6f} "
    ...
    dates = pd.DatetimeIndex(df["date"].values)
    mom_full = load_bdry_mom_lag(dates, bdry_series)[1:]
    ...
    out = ROOT / "results" / "nonml_dry_bulk_shipping_overlay_audit.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
    return all_ok
