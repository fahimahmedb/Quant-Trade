"""Backtest — Turn-of-Month en overlay de levier (spécification
pré-enregistrée dans PREREG_tom_overlay.md, committée avant ce script).
n_trials=1, aucune dépendance ML. Règle de succès renforcée. Réutilise le
masque ToM validé/audité au cycle #2.
"""
import sys
...
def main():
    lines = [
        "# Résultat — Turn-of-Month en overlay de levier (pré-enregistré, règle renforcée)",
        "",
        f"Position 1.0x en permanence, CAP={CAP}x pendant la fenêtre ToM (4j/3j).",
        "",
    ...
    lines.append("")
    lines.append(
        "**Rappel du risque assumé** : le levier amplifie aussi les pertes — comparer "
        "les colonnes MDD, pas seulement Sharpe/rendement, avant toute décision réelle."
    )

    out = ROOT / "results" / "nonml_tom_overlay_result.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
