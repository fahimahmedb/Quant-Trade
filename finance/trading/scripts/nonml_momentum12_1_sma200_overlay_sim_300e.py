"""Simulation — 300 EUR, Momentum 12-1 + overlay SMA200 levé, ~3
...
from nonml_momentum12_1_sma200_overlay_backtest import (  # noqa: E402
    load_prices, index_trend_series, LOOKBACK, SKIP, REBAL_EVERY, TERCILE, COST_BPS, CAP,
)
...
def main():
    ...
    pnl_base_full = (weights_mom * R).sum(axis=1)
    turn_base = np.abs(np.diff(weights_mom, axis=0, prepend=weights_mom[:1])).sum(axis=1) / 2.0
    ...
    lines = [
        "# Simulation — 300 EUR, Momentum 12-1 + overlay SMA200 levé (~3 derniers mois)",
        "",
        f"Période : {dates[0].date()} → {dates[-1].date()} ({len(pnl_base)} séances). "
        "Référence = momentum 12-1 1.0x (cycle #73), pas Buy&Hold classique.",
        "",
        "| | Capital final | Rendement période | MDD | Sharpe ann. |",
        "|---|---|---|---|---|",
        f"| Momentum 12-1 1.0x (référence) | {equity_base[-1]:.2f} EUR | "
        f"{100*(equity_base[-1]/CAPITAL0-1):+.1f}% | {mdd(equity_base):.1f}% | {me_base['sharpe_ann']:+.2f} |",
        f"| **Momentum 12-1 + overlay SMA200 x{CAP}** | **{equity_lev[-1]:.2f} EUR** | "
    ...
    out = ROOT / "results" / "nonml_momentum12_1_sma200_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
