"""Backtest — Overlay levé filtre de tendance SMA50 (spécification
pré-enregistrée dans PREREG_sma50_trend_overlay.md, committée avant ce
script). Variante moyen terme du #29 (SMA200). n_trials=1, aucune
...
def main():
    lines = [
        "# Résultat — Overlay levé filtre de tendance SMA50 (pré-enregistré, règle renforcée)",
        "",
    ...
    for name, fname in MARKETS.items():
        ...
        if fname == "nasdaq100_daily.txt":
            np.savez(ROOT / "results" / "nonml_sma50_trend_overlay_pnl.npz",
    ...
    out = ROOT / "results" / "nonml_sma50_trend_overlay_result.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
