"""Backtest — Volume anormal de l'indice comme porte défensive
(spécification pré-enregistrée dans PREREG_index_volume_overlay.md,
committée avant ce script). n_trials=1, aucune dépendance ML. Règle de
succès sur 4 marchés (≥3/4) — Composite exclu, volume=0 documenté.

Réutilise expanding_tercile_gate_high (générique, du #296, Règle 7).
"""
import sys
...
def load_volume(path: str) -> pd.Series:
    """Lecture directe du volume brut (colonne 'vol', ignorée par
    data_loader.load_ohlc). Meme format tabule que load_ohlc."""
    df = pd.read_csv(path, sep="\t", engine="python")
    df.columns = [c.strip() for c in df.columns]
    df["date"] = pd.to_datetime(df["date"], format="%d/%m/%Y %H:%M")
    df = df[["date", "vol"]].dropna().sort_values("date").reset_index(drop=True)
    return pd.Series(df["vol"].astype(float).values, index=pd.DatetimeIndex(df["date"]))
...
def main():
    lines = [
        ...
        "",
        f"`position(t) = {CUT}x` si Vol(t-1) est dans son tercile expanding le plus HAUT, "
        f"`1.0x` sinon. Design purement défensif. Coûts {COST_BPS:.0f} bps. "
        "Composite EXCLU (volume=0 documenté, voir PREREG). Critère ajusté à ≥3/4 marchés.",
        "",
    ...
    for name, fname in MARKETS.items():
        ...
        valid_t = valid[start:]
        bh_t = bh_full[start:]
        assert valid_t.all(), "le volume doit etre valide sur toute la fenetre testable"
        ...
        if name == "NDX (40 ans)":
            ...
            np.savez(ROOT / "results" / "nonml_index_volume_overlay_pnl.npz",
    ...
    out = ROOT / "results" / "nonml_index_volume_overlay_result.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
    return verdict
