"""Simulation — 300 EUR, porte breadth de momentum univers point-in-time,
...
from nonml_momentum_breadth_vol_targeting_overlay_pit_universe_backtest import (  # noqa: E402
    compute_momentum_breadth_series_pit, combined_position, BREADTH_THRESHOLD, COST_BPS,
)
...
def main():
    ...
    gate_series = pd.Series(gate_values, index=breadth.index).fillna(False).astype(bool)
    gate_aligned = gate_series.reindex(dates_idx.values, method="ffill").fillna(False).values.astype(bool)
    ...
    lines = [
        "# Simulation — 300 EUR, porte breadth de momentum univers point-in-time (NDX, ~3 derniers mois)",
        "",
        ...
        f"| **Overlay breadth de momentum (PIT)** | **{equity_ov[-1]:.2f} EUR** | "
        ...
        "les grilles CAP et fenêtre de vol, seule fenêtre 15j échoue)."
    ]

    out = ROOT / "results" / "nonml_momentum_breadth_vol_targeting_overlay_pit_universe_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
