"""Robustesse — Overlay vol-targeting gaté par le ratio de variance de
Lo-MacKinlay glissant. Grille CAP ET grille de fenêtre de vol (PAS un
retuning de Q ni de VR_WINDOW, les paramètres du signal lui-même),
autour des valeurs pré-enregistrées (CAP=2.0x, fenêtre=20j).
"""
import sys
...
from nonml_variance_ratio_vol_targeting_overlay_backtest import (  # noqa: E402
    momentum_regime_mask, COST_BPS, TARGET_VOL_ANNUAL, ANNUALIZATION, VR_WINDOW, MARKETS,
)
...
def main():
    ...
    for name, fname in MARKETS.items():
        ...
        if path.exists():
            ...
            gate = momentum_regime_mask(r)
            data[name] = (r, gate)

    lines = [
        "# Robustesse — Overlay vol-targeting gaté par le ratio de variance de Lo-MacKinlay glissant "
        "(grilles CAP et fenêtre, PAS un retuning de Q ni de VR_WINDOW)",
        "",
        "CAP pré-enregistré = 2.0x, fenêtre de vol pré-enregistrée = 20j. Q=5 et VR_WINDOW=252 "
        "(paramètres du signal de porte lui-même) restent fixes, comme pour toutes les portes "
        "précédentes de cette famille (#47/#54/#57/#68/#78/#80/#216).",
        "",
    ...
    out = ROOT / "results" / "nonml_variance_ratio_vol_targeting_overlay_robustness.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
