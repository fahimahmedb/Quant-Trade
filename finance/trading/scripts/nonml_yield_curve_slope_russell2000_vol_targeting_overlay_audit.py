"""Audit adversarial — Pente de la courbe des taux US (T10Y2Y) appliquée
au Russell 2000.
...
from nonml_yield_curve_slope_vol_targeting_overlay_backtest import load_t10y2y, MEDIAN_WINDOW  # noqa: E402
from nonml_yield_curve_slope_russell2000_vol_targeting_overlay_backtest import MARKET_FILE  # noqa: E402


def independent_gate_at(mkt_date, slope_dates: list, slope_vals: np.ndarray) -> bool:
    idx = None
    for i in range(len(slope_dates) - 1, -1, -1):
        if slope_dates[i] < mkt_date:
            idx = i
            break
...
def main():
    ...
    check_idx = list(range(2000, len(dates_idx), 2000))
    lines = ["# Audit adversarial — Pente de la courbe des taux US appliquée au Russell 2000", "",
    ...
    out = ROOT / "results" / "nonml_yield_curve_slope_russell2000_vol_targeting_overlay_audit.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
