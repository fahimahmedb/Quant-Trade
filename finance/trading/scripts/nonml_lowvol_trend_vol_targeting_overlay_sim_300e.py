"""Simulation — 300 EUR, Low-Vol Tilt + overlay combiné tendance +
...
def main():
    ...
    lines = [
        "# Simulation — 300 EUR, Low-Vol + overlay combiné tendance + vol-targeting (~3 derniers mois)",
        "",
        ...
        f"| **Low-Vol + overlay tendance+vol-targeting** | **{equity_lev[-1]:.2f} EUR** | "
        ...
        "15j-30j, MDD quasi constant ~-19% partout)."
    ]

    out = ROOT / "results" / "nonml_lowvol_trend_vol_targeting_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
