"""Robustesse — Overlay vol-targeting gaté par le clustering ARCH
glissant. Grille CAP ET grille de fenêtre de vol (PAS un retuning de
ARCH_WINDOW ni de MEDIAN_WINDOW, les paramètres du signal de porte
...
from nonml_arch_clustering_vol_targeting_overlay_backtest import (  # noqa: E402
    strong_clustering_mask, COST_BPS, TARGET_VOL_ANNUAL, ANNUALIZATION,
    ARCH_WINDOW, MEDIAN_WINDOW, MARKETS,
)
...
def main():
    ...
    for name, fname in MARKETS.items():
        ...
        if path.exists():
            ...
            gate = strong_clustering_mask(r)
            data[name] = (r, gate)

    lines = [
        "# Robustesse — Overlay vol-targeting gaté par le clustering ARCH glissant "
        "(grilles CAP et fenêtre, PAS un retuning de ARCH_WINDOW ni de MEDIAN_WINDOW)",
        "",
        "CAP pré-enregistré = 2.0x, fenêtre de vol pré-enregistrée = 20j. ARCH_WINDOW=252 et "
        "MEDIAN_WINDOW=252 (paramètres du signal de porte lui-même) restent fixes, comme pour "
        "toutes les portes précédentes de cette famille (#47/#54/#57/#68/#78/#80/#216-#220).",
        "",
    ...
    out = ROOT / "results" / "nonml_arch_clustering_vol_targeting_overlay_robustness.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
