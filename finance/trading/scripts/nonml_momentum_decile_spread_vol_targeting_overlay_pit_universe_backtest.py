"""Backtest — Overlay vol-targeting gaté par le spread décile de momentum,
...
modifié**. Seul l'univers servant à calculer le spread change — à chaque date,
seuls les titres réellement membres du NDX-100 entrent dans le classement.
...
def compute_decile_spread_series_pit():
    """Spread(t) = moyenne du decile de momentum le plus FORT moins moyenne du
    decile le plus FAIBLE, calcule sur les seuls MEMBRES du NDX-100 a la date t.

    NaN avant le 01/01/2015 (aucune appartenance connue) ou si moins de
    MIN_LISTED membres eligibles. Retourne aussi la couverture moyenne.
    """
...
def build_gate(spread):
    ...
    return (spread >= median_spread).where(signal_defined)
...
def main():
    ...
    spread, cov = compute_decile_spread_series_pit()
    gate_series = build_gate(spread)
    ...
    lines = [
        "# Résultat — spread décile de momentum, univers POINT-IN-TIME (pré-enregistré)",
        "",
        "Réutilisation stricte (Règle 7) du cycle d'origine (#100) : **aucun paramètre "
        "modifié**. Seul l'univers servant à calculer le spread change — à chaque date, "
        "seuls les titres réellement membres du NDX-100 entrent dans le classement.",
        ...
        f"Spread décile moyen : {100*np.nanmean(spread.values):.1f} pts de %",
        "",
        "| | Sharpe ann. | Rendement total net | MDD |",
        "|---|---|---|---|",
        f"| Buy&Hold (NDX) | {me_bh['sharpe_ann']:+.2f} | {100*ret_bh:+.1f}% | "
        f"{me_bh['max_drawdown_pct']:.1f}% |",
        f"| **Overlay gaté spread décile (PIT)** | **{me_ov['sharpe_ann']:+.2f}** | "
    ...
    out = ROOT / "results" / "nonml_momentum_decile_spread_vol_targeting_overlay_pit_universe_result.md"
    out.write_text("\n".join(lines), encoding="utf-8")

    np.savez(
        ROOT / "results" / "nonml_momentum_decile_spread_vol_targeting_overlay_pit_universe_pnl.npz",
