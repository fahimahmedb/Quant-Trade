"""Audit indépendant — Momentum de l'Ethereum (CBETHUSD, log-return 21j).

Recalcule ETHmom_lag(t), le tercile expanding et la position par une
...
from nonml_ethereum_momentum_overlay_backtest import load_eth_mom_lag, load_eth_series  # noqa: E402


def manual_eth_mom_lag(dates: pd.DatetimeIndex) -> np.ndarray:
    """Recalcul par boucle explicite + searchsorted (side='right'-1,
    inclusif), sans pandas.reindex/ffill."""
    raw = pd.read_csv(REPO_ROOT / "data" / "ethereum_daily.csv")
    raw["observation_date"] = pd.to_datetime(raw["observation_date"])
    raw["CBETHUSD"] = pd.to_numeric(raw["CBETHUSD"], errors="coerce")
    raw = raw.dropna(subset=["CBETHUSD"]).drop_duplicates("observation_date").sort_values("observation_date")
    obs_dates = raw["observation_date"].values
    vals = raw["CBETHUSD"].astype(float).values
...
def main():
    lines = ["# Audit — Momentum de l'Ethereum (CBETHUSD, log-return 21j)", ""]
    all_ok = True

    eth_series = load_eth_series()
    ...
    for name, fname in MARKETS.items():
        ...
        mom_official = load_eth_mom_lag(dates, eth_series)[1:]
        pos_official = expanding_tercile_cut_low(mom_official)

        mom_manual = manual_eth_mom_lag(dates)[1:]
        ...
        if n_pos_diff == 0:
            ...
            lines.append(f"- **{n_pos_diff}/{n_total} désaccords de position** — sensibilité de bord flottante "
                         f"documentée (même pattern que #193/#195/.../#344) : "
    ...
    # shift(1) externe) -- meme methode que #344.
    name, fname = "NDX (40 ans)", MARKETS["NDX (40 ans)"]
    df = load_ohlc(str(REPO_ROOT / "data" / fname))
    dates = pd.DatetimeIndex(df["date"].values)
    eth_aligned = eth_series.reindex(dates, method="ffill").values
    mom_raw = np.full(len(dates), np.nan)
    mom_raw[RET_WINDOW:] = np.log(eth_aligned[RET_WINDOW:] / eth_aligned[:-RET_WINDOW])
    ...
    mom_lag_full = load_eth_mom_lag(dates, eth_series)
    ...
    lines.append(f"- ETHmom brut séance précédente ({d_prev.date()}) = {m_prev:.6f}, "
                 f"ETHmom brut séance de la transition ({d_last.date()}) = {m_last:.6f} "
                 f"(valeurs distinctes vérifiées : {'OUI' if not np.isclose(m_prev, m_last) else 'NON — transition aveugle, coïncidence à éviter'})")
    lines.append(f"- ETHmom_lag séance {i_last} ({d_last.date()}) = {mom_lag_full[i_last]:.6f} (doit valoir {m_prev:.6f}, JAMAIS {m_last:.6f})")
    if i_last + 1 < len(dates):
        lines.append(f"- ETHmom_lag séance {i_last+1} ({dates[i_last+1].date()}) = {mom_lag_full[i_last+1]:.6f} "
    ...
    mom_full = load_eth_mom_lag(dates, eth_series)[1:]
    ...
    out = ROOT / "results" / "nonml_ethereum_momentum_overlay_audit.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
    return all_ok
