"""Backtest — Momentum de constance + overlay levé filtre de tendance
SMA200 (spécification pré-enregistrée dans
PREREG_momentum_consistency_sma200_overlay.md, committée avant ce
script). Combine les cycles #82 et #29. n_trials=1, aucune dépendance
ML. Règle de succès renforcée -- référence = momentum de constance 1.0x
(cycle #82), pas Buy&Hold.
...
#255 appliqué ici, jamais fait avant). `main(causal=True)` décale
...
def main(causal=True):
    ...
    lines = [
        "# Résultat — Momentum de constance + overlay levé filtre de tendance SMA200 (pré-enregistré, combinaison #82+#29)",
        "",
        ...
        f"| **Momentum de constance + overlay SMA200 x{CAP}** | **{me_lev['sharpe_ann']:+.2f}** | "
    ...
    out = ROOT / "results" / "nonml_momentum_consistency_sma200_overlay_result.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
