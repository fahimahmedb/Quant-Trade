"""Backtest — Overlay de confirmation multi-marché INTERNATIONALE, NDX
confirmé par le DAX (spécification pré-enregistrée dans
PREREG_intl_breadth_confirmation_overlay.md, committée avant ce script).
Variante internationale du #52 (NDX+Russell2000, domestique). n_trials=1,
...
def main():
    ...
    signal_b = near_high_series(df_confirm)  # DAX
    ...
    lines = [
        "# Résultat — Overlay de confirmation multi-marché internationale NDX+DAX (pré-enregistré, règle renforcée)",
        "",
        f"Position sur NDX = 1.0x en permanence, CAP={CAP}x quand NDX ET DAX sont "
        ...
        f"%j confirmé DAX seul (signal B, aligné ffill) : {100*b_aligned[start:-1].mean():.1f}%",
        ...
        f"| **Overlay confirmation internationale x{CAP}** | **{me_ov['sharpe_ann']:+.2f}** | "
