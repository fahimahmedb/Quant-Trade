"""Backtest — Overlay levé pré/post jour férié (spécification
pré-enregistrée dans PREREG_holiday_effect_overlay.md, committée avant
ce script). Détection data-driven identique au cycle #7. n_trials=1,
...
def holiday_masks(dates: pd.Series):
    ...
    n = len(d)
    gap_days = (d.shift(-1) - d).dt.days
    dow = d.dt.dayofweek
    normal_gap = np.where(dow == 4, 3, 1)
...
def main():
    lines = [
        "# Résultat — Overlay levé pré/post jour férié (pré-enregistré, règle renforcée)",
        "",
        f"Position 1.0x en permanence, CAP={CAP}x les jours pré ou post férié (détection data-driven).",
        "",
    ...
    out = ROOT / "results" / "nonml_holiday_effect_overlay_result.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
