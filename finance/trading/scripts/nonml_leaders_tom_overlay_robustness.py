"""Robustesse — Leaders 52-semaines + overlay ToM. Grille CAP (PAS un
...
from nonml_leaders_tom_overlay_backtest import (  # noqa: E402
    load_prices, tom_mask, LOOKBACK, REBAL_EVERY, TERCILE, COST_BPS,
)
...
def main():
    ...
    lines = [
        "# Robustesse — Leaders 52-semaines + overlay ToM (grille CAP, PAS un retuning)",
        "",
    ...
    out = ROOT / "results" / "nonml_leaders_tom_overlay_robustness.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
