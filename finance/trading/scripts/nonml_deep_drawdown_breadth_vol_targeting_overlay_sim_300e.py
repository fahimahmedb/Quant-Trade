"""Simulation — 300 EUR dans l'overlay vol-targeting gaté par la breadth
de drawdown profond (NDX), ~3 derniers mois. Spécification pré-enregistrée
...
from nonml_deep_drawdown_breadth_vol_targeting_overlay_backtest import (  # noqa: E402
    compute_deep_drawdown_breadth_series, combined_position, MEDIAN_WINDOW, COST_BPS,
)
...
def main():
    ...
    lines = [
        "# Simulation — 300 EUR, overlay vol-targeting gaté drawdown profond (NDX, ~3 derniers mois)",
        "",
        ...
        f"| **Overlay drawdown profond** | **{equity_ov[-1]:.2f} EUR** | "
        ...
        "statistique réel n'est PAS encore établi à ce stade (Règle 9) : ce PASS niveau 1 doit "
        "encore passer la batterie de validation renforcée "
        "(`nonml_pass_validation_battery.py deep_drawdown_breadth_vol_targeting_overlay`) avant "
        "toute déclaration finale."
    ]

    out = ROOT / "results" / "nonml_deep_drawdown_breadth_vol_targeting_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
