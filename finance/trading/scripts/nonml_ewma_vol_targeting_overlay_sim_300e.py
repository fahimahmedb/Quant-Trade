"""Simulation — 300 EUR dans l'overlay vol-targeting estimateur EWMA
(NDX), ~3 derniers mois. Spécification pré-enregistrée (λ=0,94, CAP=2.0x,
fenêtre d'amorçage=20j), aucun paramètre retouché après les résultats
précédents.
"""
import sys
...
from nonml_ewma_vol_targeting_overlay_backtest import ewma_vol_position, COST_BPS, CAP, VOL_WINDOW, LAM, TARGET_VOL_ANNUAL  # noqa: E402
...
def main():
    ...
    pos_full_all = ewma_vol_position(bh_full_all)
    ...
    lines = [
        ...
        f"({len(bh_full)} séances). λ={LAM}, fenêtre d'amorçage {VOL_WINDOW}j, vol cible "
        f"{TARGET_VOL_ANNUAL:.0%}, CAP={CAP}x.",
        "",
        "| | Capital final | Rendement période | MDD | Sharpe ann. |",
        "|---|---|---|---|---|",
        f"| BuyHold | {equity_bh[-1]:.2f} EUR | {100*(equity_bh[-1]/CAPITAL0-1):+.1f}% | "
        f"{mdd(equity_bh):.1f}% | {me_bh['sharpe_ann']:+.2f} |",
        f"| **Overlay vol-targeting EWMA** | **{equity_ov[-1]:.2f} EUR** | "
        ...
        "du backtest complet (PASS 4/5 marchés, seul Composite échoue) et de la robustesse "
        "(plateau solide, 4/5-5/5 sur la grille CAP, 4/5 parfaitement stable sur la grille "
        "fenêtre d'amorçage)."
    ]

    out = ROOT / "results" / "nonml_ewma_vol_targeting_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
