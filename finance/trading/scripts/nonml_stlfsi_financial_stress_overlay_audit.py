"""Audit adversarial — Indice de stress financier St. Louis Fed STLFSI4, overlay défensif.

1. Recalcul indépendant par boucle+searchsorted manuel (side="right"
   inclusif, méthode prouvée correcte au #203/#283-#291).
...
from nonml_stlfsi_financial_stress_overlay_backtest import (  # noqa: E402
    build_stlfsi_series, load_stlfsi_lag, expanding_tercile_cut_high, MARKETS,
)


def independent_lag(dates: np.ndarray, stlfsi_dates: np.ndarray, stlfsi_vals: np.ndarray) -> np.ndarray:
    ...
    for i, d in enumerate(dates):
        idx = np.searchsorted(stlfsi_dates, d, side="right") - 1
        if idx >= 0:
            out[i] = stlfsi_vals[idx]
...
def main():
    ...
    stlfsi_dates = stlfsi_series.index.values
    stlfsi_vals = stlfsi_series.values

    lines = ["# Audit adversarial — Indice de stress financier St. Louis Fed STLFSI4, overlay défensif", "",
    ...
    for name, fname in MARKETS.items():
        ...
        orig = load_stlfsi_lag(dates, stlfsi_series)
        indep = independent_lag(dates.values, stlfsi_dates, stlfsi_vals)
    ...
    raw = raw.dropna(subset=["STLFSI4"]).sort_values("observation_date")
    last_obs = raw["observation_date"].iloc[-1]
    last_avail = stlfsi_dates[-1]
    lines.append(f"Dernière observation STLFSI4 : semaine de {pd.Timestamp(last_obs).date()}. "
    ...
    stlfsi_lag_full = load_stlfsi_lag(ndx_dates, stlfsi_series)[1:]
    pos_full = expanding_tercile_cut_high(stlfsi_lag_full)
    ...
    for cut in TRUNC_POINTS:
        ...
        stlfsi_lag_trunc = load_stlfsi_lag(dates_trunc, stlfsi_series)[1:]
        pos_trunc = expanding_tercile_cut_high(stlfsi_lag_trunc)
    ...
    out = ROOT / "results" / "nonml_stlfsi_financial_stress_overlay_audit.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
