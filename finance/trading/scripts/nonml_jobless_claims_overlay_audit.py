"""Audit indépendant — Demandes initiales d'allocations chômage (ICSA).

Recalcule ClaimsYoY(t), l'alignement causal (avec décalage de
...
from nonml_jobless_claims_overlay_backtest import (  # noqa: E402
    ...
    build_claims_yoy_series, load_claims_yoy_lag, expanding_tercile_cut_high,
)
...
def main():
    lines = ["# Audit — Demandes initiales d'allocations chômage (ICSA)", ""]
    all_ok = True
    ...
    for name, fname in MARKETS.items():
        ...
        claims_lag_official = load_claims_yoy_lag(dates, claims_series)[1:]
    ...
    raw = pd.read_csv(REPO_ROOT / "data" / "icsa_weekly.csv")
    raw["observation_date"] = pd.to_datetime(raw["observation_date"])
    last_obs = raw.dropna(subset=["ICSA"]).sort_values("observation_date").iloc[-1]
    ...
    lines.append(f"- Dernière observation ICSA : semaine se terminant le {obs_date.date()} "
                 f"(valeur = {last_obs['ICSA']:.0f})")
    lines.append(f"- Disponible causalement à partir du {avail_date.date()} (décalage {PUBLICATION_LAG_DAYS}j), "
                 f"jamais avant — cohérent avec le délai réel de publication DOL (~5j, marge conservatrice de 2j)")
    ...
    dates = pd.DatetimeIndex(df["date"].values)
    claims_lag_full = load_claims_yoy_lag(dates, claims_series)[1:]
    ...
    out = ROOT / "results" / "nonml_jobless_claims_overlay_audit.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
    return all_ok
