"""Audit indépendant — Croissance de la masse monétaire M2 (glissement
annuel).

Recalcule M2Growth(t), l'alignement causal (avec décalage d'un mois) et
...
que le #195) et l'absence de fuite générale (troncature de
l'historique).
"""
import sys
...
from nonml_m2_growth_overlay_backtest import (  # noqa: E402
    COST_BPS, CUT, YOY_MONTHS, MARKETS, build_m2_growth_series,
    load_m2_growth_lag, expanding_tercile_cut_low,
)


def manual_m2_growth_lag(dates: pd.DatetimeIndex) -> np.ndarray:
    """Recalcul par boucle explicite + searchsorted, sans
    pandas.reindex/ffill/DateOffset."""
    raw = pd.read_csv(REPO_ROOT / "data" / "m2sl_monthly.csv")
    raw["observation_date"] = pd.to_datetime(raw["observation_date"])
    raw = raw.dropna(subset=["M2SL"]).drop_duplicates("observation_date").sort_values("observation_date")
    obs_dates = raw["observation_date"].values
    vals = raw["M2SL"].astype(float).values
...
def main():
    lines = ["# Audit — Croissance de la masse monétaire M2 (glissement annuel)", ""]
    all_ok = True

    m2_series = build_m2_growth_series()
    ...
    for name, fname in MARKETS.items():
        ...
        m2_lag_official = load_m2_growth_lag(dates, m2_series)[1:]
        pos_official = expanding_tercile_cut_low(m2_lag_official)

        m2_lag_manual = manual_m2_growth_lag(dates)[1:]
        diff_align = np.nanmax(np.abs(m2_lag_official - m2_lag_manual))

        start = int(np.argmax(np.isfinite(m2_lag_manual)))
        T = len(m2_lag_manual)
        pos_manual = np.full(T, np.nan)
        for t in range(start, T):
            if not np.isfinite(m2_lag_manual[t]):
                continue
            hist = m2_lag_manual[start:t + 1]
            hist = hist[np.isfinite(hist)]
            thresh = manual_percentile_low_tercile(hist)
            pos_manual[t] = CUT if m2_lag_manual[t] <= thresh else 1.0
        ...
        for i in np.where(mask)[0][pos_diff_mask]:
            local_diff = abs(m2_lag_official[i] - m2_lag_manual[i])
            if local_diff >= 1e-10:
        ...
        if n_pos_diff == 0:
            ...
            lines.append(f"- **{n_pos_diff}/{n_total} désaccords de position** — sensibilité de bord flottante "
                         f"documentée (même pattern que #193/#195/#196/#197/#198/#199/#200) : "
                         f"{'confirmé' if boundary_tie_ok else 'NON confirmé, écart réel'}.")
        lines.append(f"- **{'OK' if ok else 'ÉCART DÉTECTÉ'}**")
        lines.append("")

    # Verification specifique du decalage d'un mois (meme test que #195).
    raw = pd.read_csv(REPO_ROOT / "data" / "m2sl_monthly.csv")
    raw["observation_date"] = pd.to_datetime(raw["observation_date"])
    ...
    m2_check = load_m2_growth_lag(check_dates, m2_series)
    lines.append("## Vérification spécifique du décalage d'un mois (mai 2026)")
    lines.append(f"- M2Growth_lag(15 mai) = {m2_check[0]}, M2Growth_lag(31 mai) = {m2_check[1]} "
                 f"(doivent être la valeur d'avril ou plus ancienne, JAMAIS mai)")
    lines.append(f"- M2Growth_lag(1 juin) = {m2_check[2]}, M2Growth_lag(2 juin) = {m2_check[3]} "
                 f"(le 2 juin doit être le premier jour où mai apparaît, via shift(1))")
    ok_shift = not np.isclose(m2_check[0], m2_check[3]) and not np.isclose(m2_check[1], m2_check[3])
    all_ok &= ok_shift
    lines.append(f"- **{'OK — la valeur décalée change bien au 2 juin, cohérent avec le décalage attendu' if ok_shift else 'FUITE POTENTIELLE'}**")
    lines.append("")
    ...
    dates = pd.DatetimeIndex(df["date"].values)
    m2_lag_full = load_m2_growth_lag(dates, m2_series)[1:]
    pos_full = expanding_tercile_cut_low(m2_lag_full)
    valid_start = int(np.argmax(np.isfinite(m2_lag_full)))
    T_CUT = valid_start + 2000
    pos_truncated = expanding_tercile_cut_low(m2_lag_full[:T_CUT])
    ...
    out = ROOT / "results" / "nonml_m2_growth_overlay_audit.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
    return all_ok
