"""Audit indépendant — Indice CBOE SKEW (risque de queue).

Recalcule SKEW_lag(t), le tercile expanding et la position par une
...
from nonml_skew_index_overlay_backtest import load_skew_lag  # noqa: E402


def manual_skew_lag(dates: pd.DatetimeIndex) -> np.ndarray:
    """Recalcul par boucle explicite + searchsorted (side='right'-1,
    inclusif), sans pandas.reindex/ffill."""
    raw = pd.read_csv(REPO_ROOT / "data" / "skew_daily.csv")
    raw["DATE"] = pd.to_datetime(raw["DATE"], format="%m/%d/%Y")
    raw = raw.dropna(subset=["SKEW"]).drop_duplicates("DATE").sort_values("DATE")
    obs_dates = raw["DATE"].values
    vals = raw["SKEW"].astype(float).values
...
def main():
    lines = ["# Audit — Indice CBOE SKEW (risque de queue)", ""]
    all_ok = True
    ...
    for name, fname in MARKETS.items():
        ...
        skew_official = load_skew_lag(dates)[1:]
        pos_official = expanding_tercile_cut_high(skew_official)

        skew_manual = manual_skew_lag(dates)[1:]
        diff_align = np.nanmax(np.abs(skew_official - skew_manual))

        start = int(np.argmax(np.isfinite(skew_manual)))
        T = len(skew_manual)
        pos_manual = np.full(T, np.nan)
        for t in range(start, T):
            if not np.isfinite(skew_manual[t]):
                continue
            hist = skew_manual[start:t + 1]
            hist = hist[np.isfinite(hist)]
            thresh = manual_percentile_high_tercile(hist)
            pos_manual[t] = CUT if skew_manual[t] >= thresh else 1.0
        ...
        for i in np.where(mask)[0][pos_diff_mask]:
            local_diff = abs(skew_official[i] - skew_manual[i])
            if local_diff >= 1e-10:
        ...
        if n_pos_diff == 0:
            ...
            lines.append(f"- **{n_pos_diff}/{n_total} désaccords de position** — sensibilité de bord flottante "
                         f"documentée (même pattern que #193/#195/.../#340) : "
    ...
    # l'artefact de bord positionnel de shift(1) documente au #320/#321/#340).
    raw = pd.read_csv(REPO_ROOT / "data" / "skew_daily.csv")
    raw["DATE"] = pd.to_datetime(raw["DATE"], format="%m/%d/%Y")
    raw = raw.dropna(subset=["SKEW"]).sort_values("DATE")
    vals = raw["SKEW"].astype(float).values
    obs_dates = raw["DATE"].values
    diffs = np.diff(vals)
    ...
    skew_check = load_skew_lag(check_dates)

    lines.append(f"## Vérification spécifique du décalage causal d'un jour (transition {d_prev.date()}→{d_last.date()})")
    lines.append(f"- SKEW brut jour précédent ({d_prev.date()}) = {v_prev:.6f}, "
                 f"SKEW brut jour de la transition ({d_last.date()}) = {v_last:.6f} "
                 f"(valeurs distinctes vérifiées : {'OUI' if not np.isclose(v_prev, v_last) else 'NON — transition aveugle, coïncidence à éviter'})")
    lines.append(f"- SKEW_lag({d_last.date()}) = {skew_check[4]:.6f} (doit valoir {v_prev:.6f}, JAMAIS {v_last:.6f})")
    lines.append(f"- SKEW_lag({(d_last + pd.Timedelta(days=1)).date()}) = {skew_check[5]:.6f} "
                 f"(doit être le premier jour où {v_last:.6f} apparaît, via shift(1))")
    ok_shift = (not np.isclose(v_prev, v_last)) and np.isclose(skew_check[4], v_prev) and \
        np.isclose(skew_check[5], v_last)
    all_ok &= ok_shift
    ...
    dates = pd.DatetimeIndex(df["date"].values)
    skew_full = load_skew_lag(dates)[1:]
    pos_full = expanding_tercile_cut_high(skew_full)
    valid_start = int(np.argmax(np.isfinite(skew_full)))
    T_CUT = valid_start + 2000
    pos_truncated = expanding_tercile_cut_high(skew_full[:T_CUT])
    ...
    out = ROOT / "results" / "nonml_skew_index_overlay_audit.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
    return all_ok
