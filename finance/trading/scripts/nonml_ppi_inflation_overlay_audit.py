"""Audit indépendant — Inflation à la production US (FRED PPIACO).

Recalcule PPIGrowth_lag(t), l'alignement causal (avec décalage d'un
...
from nonml_ppi_inflation_overlay_backtest import (  # noqa: E402
    PUBLICATION_LAG_MONTHS, build_ppi_growth_series, load_ppi_growth_lag,
)


def manual_ppi_growth_lag(dates: pd.DatetimeIndex) -> np.ndarray:
    """Recalcul par boucle explicite + searchsorted, sans
    pandas.reindex/ffill/DateOffset."""
    raw = pd.read_csv(REPO_ROOT / "data" / "ppi_monthly.csv")
    raw["observation_date"] = pd.to_datetime(raw["observation_date"])
    raw = raw.dropna(subset=["PPIACO"]).drop_duplicates("observation_date").sort_values("observation_date")
    obs_dates = raw["observation_date"].values
    vals = raw["PPIACO"].astype(float).values
...
def main():
    lines = ["# Audit — Inflation à la production US (FRED PPIACO)", ""]
    all_ok = True

    ppi_series = build_ppi_growth_series()
    ...
    for name, fname in MARKETS.items():
        ...
        ppi_lag_official = load_ppi_growth_lag(dates, ppi_series)[1:]
        pos_official = expanding_tercile_cut_high(ppi_lag_official)

        ppi_lag_manual = manual_ppi_growth_lag(dates)[1:]
        diff_align = np.nanmax(np.abs(ppi_lag_official - ppi_lag_manual))

        start = int(np.argmax(np.isfinite(ppi_lag_manual)))
        T = len(ppi_lag_manual)
        pos_manual = np.full(T, np.nan)
        for t in range(start, T):
            if not np.isfinite(ppi_lag_manual[t]):
                continue
            hist = ppi_lag_manual[start:t + 1]
            hist = hist[np.isfinite(hist)]
            thresh = manual_percentile_high_tercile(hist)
            pos_manual[t] = CUT if ppi_lag_manual[t] >= thresh else 1.0
        ...
        for i in np.where(mask)[0][pos_diff_mask]:
            local_diff = abs(ppi_lag_official[i] - ppi_lag_manual[i])
            if local_diff >= 1e-10:
    ...
    raw = pd.read_csv(REPO_ROOT / "data" / "ppi_monthly.csv")
    raw["observation_date"] = pd.to_datetime(raw["observation_date"])
    raw = raw.dropna(subset=["PPIACO"]).sort_values("observation_date")
    vals = raw["PPIACO"].astype(float).values
    obs_dates = raw["observation_date"].values
    ...
    ppi_check = load_ppi_growth_lag(check_dates, ppi_series)
    ...
    lines.append(f"- PPIGrowth_lag({check_dates[1].date()}) = {ppi_check[1]:.6f}, "
                 f"PPIGrowth_lag({check_dates[2].date()}) = {ppi_check[2]:.6f} "
                 f"(doivent valoir {g_prev:.6f}, JAMAIS {g_last:.6f})")
    lines.append(f"- PPIGrowth_lag({check_dates[3].date()}) = {ppi_check[3]:.6f}, "
                 f"PPIGrowth_lag({check_dates[4].date()}) = {ppi_check[4]:.6f} "
                 f"({check_dates[3].date()} doit être le premier jour où {g_last:.6f} apparaît, via shift(1))")
    ok_shift = (not np.isclose(g_prev, g_last)) and np.isclose(ppi_check[1], g_prev) and \
        np.isclose(ppi_check[2], g_prev) and np.isclose(ppi_check[3], g_last) and np.isclose(ppi_check[4], g_last)
    all_ok &= ok_shift
    ...
    dates = pd.DatetimeIndex(df["date"].values)
    ppi_lag_full = load_ppi_growth_lag(dates, ppi_series)[1:]
    pos_full = expanding_tercile_cut_high(ppi_lag_full)
    valid_start = int(np.argmax(np.isfinite(ppi_lag_full)))
    T_CUT = valid_start + 2000
    pos_truncated = expanding_tercile_cut_high(ppi_lag_full[:T_CUT])
    ...
    out = ROOT / "results" / "nonml_ppi_inflation_overlay_audit.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
    return all_ok
