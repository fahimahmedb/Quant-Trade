"""Robustesse — Overlay vol-targeting gaté par le spread décile de
momentum. Grille CAP ET grille de fenêtre de vol (PAS un retuning de la
fraction décile 10% ni des fenêtres momentum 252j/21j, paramètres de
définition de l'hypothèse), autour des valeurs pré-enregistrées.
"""
import sys
...
from nonml_momentum_decile_spread_vol_targeting_overlay_backtest import (  # noqa: E402
    compute_decile_spread_series, MEDIAN_WINDOW, COST_BPS, TARGET_VOL_ANNUAL, ANNUALIZATION,
)
...
def main():
    ...
    lines = [
        "# Robustesse — Overlay vol-targeting gaté par spread décile de momentum (grilles CAP et fenêtre, PAS un retuning de la fraction décile/252j/21j)",
        "",
    ...
    out = ROOT / "results" / "nonml_momentum_decile_spread_vol_targeting_overlay_robustness.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
