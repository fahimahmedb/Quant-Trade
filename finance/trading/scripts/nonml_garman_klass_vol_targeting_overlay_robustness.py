"""Robustesse — Overlay vol-targeting estimateur Garman-Klass. Grille CAP
...
from nonml_garman_klass_vol_targeting_overlay_backtest import COST_BPS, TARGET_VOL_ANNUAL, ANNUALIZATION, MARKETS  # noqa: E402
...
def position_for(df, cap: float, window: int) -> np.ndarray:
    var_pct = garman_klass_var_pct(df)
...
def main():
    ...
    lines = [
        "# Robustesse — Overlay vol-targeting Garman-Klass (grilles CAP et fenêtre, PAS un retuning)",
        "",
    ...
    out = ROOT / "results" / "nonml_garman_klass_vol_targeting_overlay_robustness.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
