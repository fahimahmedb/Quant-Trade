"""Backtest — Overlay levé fenêtre élargie vendredi-lundi (spécification
pré-enregistrée dans PREREG_friday_monday_overlay.md, committée avant ce
...
def fri_mon_mask(dates: pd.Series) -> np.ndarray:
...
def main():
    lines = [
        "# Résultat — Overlay levé fenêtre vendredi-lundi (pré-enregistré, règle renforcée)",
        "",
        f"Position 1.0x en permanence, CAP={CAP}x les vendredis et lundis.",
        "",
    ...
    out = ROOT / "results" / "nonml_friday_monday_overlay_result.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
