...
January Barometer (NDX), ~3 derniers mois. Spécification pré-enregistrée
...
from nonml_january_barometer_vol_targeting_overlay_backtest import (  # noqa: E402
    january_gate_mask, combined_position, COST_BPS, CAP, VOL_WINDOW, TARGET_VOL_ANNUAL,
)
...
def main():
    ...
    pos_full_all = combined_position(close_full, bh_full_all, gate)
    ...
    lines = [
        "# Simulation — 300 EUR, overlay vol-targeting gaté par le January Barometer (NDX, ~3 derniers mois)",
        "",
        ...
        f"CAP={CAP}x (2026 a eu un janvier positif sur NDX -> porte active toute la fenêtre, "
        "avril-juillet étant hors janvier).",
        "",
        "| | Capital final | Rendement période | MDD | Sharpe ann. |",
        "|---|---|---|---|---|",
        f"| BuyHold | {equity_bh[-1]:.2f} EUR | {100*(equity_bh[-1]/CAPITAL0-1):+.1f}% | "
        f"{mdd(equity_bh):.1f}% | {me_bh['sharpe_ann']:+.2f} |",
        f"| **Overlay vol-targeting January Barometer** | **{equity_ov[-1]:.2f} EUR** | "
        ...
        "du backtest complet (PASS 4/5 marchés) et de la robustesse (plateau 4-5/5 sur les "
        "grilles CAP 1.5x-3.0x et fenêtre 15-30j)."
    ]

    out = ROOT / "results" / "nonml_january_barometer_vol_targeting_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
