"""Simulation — 300 EUR, panel élargi 4 signaux (défaut carte + NFCI +
BAA10Y + corrélation NDX-DAX, cycle #303) sur les ~3 derniers mois
...
def main():
    ...
    lines = [
        f"# Simulation — 300 EUR, panel élargi 4 signaux (défaut carte+NFCI+BAA10Y+corr NDX-DAX, cycle #303), {MARKET_NAME} (~3 derniers mois)",
        "",
        f"Période : {dates_w[0].date()} → {dates_w[-1].date()} ({len(r)} séances). "
        f"{100*frac_cut:.0f}% de la fenêtre est en régime coupé (au moins 3 des 4 signaux de stress "
        ...
        f"| **Overlay panel élargi ≥3/4** | **{eq_ov[-1]:.2f} EUR** | "
    ...
    out = ROOT / "results" / "nonml_delinquency_nfci_baa10y_corr_majority_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
