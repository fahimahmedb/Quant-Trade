"""Backtest — Overlay de vol-targeting avec estimateur Rogers-Satchell
(spécification pré-enregistrée dans
PREREG_rogers_satchell_vol_targeting_overlay.md, committée avant ce
script). Variante du #46 (close-to-close), #50 (Parkinson) et #215
(Garman-Klass), robuste au drift intra-séance. n_trials=1, aucune
...
def rogers_satchell_vol_position(df) -> np.ndarray:
    """Renvoie la position continue clip(vol_cible / vol_rs(t-1), 0, CAP),
    alignee sur les rendements clot-a-clot r = log(close[1:]/close[:-1])
    (meme longueur, meme convention causale que le #46/#50/#215)."""
    var_pct = rogers_satchell_var_pct(df)  # index = df["date"].iloc[1:], longueur T-1, en %^2
    var_roll = var_pct.rolling(VOL_WINDOW).mean().values  # moyenne roulante causale, %^2
    vol_ann = np.sqrt(np.clip(var_roll, 0.0, None)) * ANNUALIZATION / 100.0  # %->fraction, annualise
...
def main():
    lines = [
        ...
        "",
        f"Position(t) = clip({TARGET_VOL_ANNUAL:.0%} / vol_RogersSatchell_{VOL_WINDOW}j(t-1), 0.0, {CAP}x) "
        f"— variante robuste au drift intra-séance du #46/#50/#215. Échantillon testable = à partir de la "
    ...
    for name, fname in MARKETS.items():
        ...
        pos_full = rogers_satchell_vol_position(df)  # longueur T-1, aligne sur bh_full
        start = VOL_WINDOW
        bh_t = bh_full[start:]
        pos = pos_full[start:]
        ...
        if name == "NDX (40 ans)":
            ...
            np.savez(ROOT / "results" / "nonml_rogers_satchell_vol_targeting_overlay_pnl.npz",
    ...
    out = ROOT / "results" / "nonml_rogers_satchell_vol_targeting_overlay_result.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
