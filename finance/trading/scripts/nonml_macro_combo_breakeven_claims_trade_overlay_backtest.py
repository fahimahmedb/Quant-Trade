"""Backtest — Combinaison majoritaire (≥2/3) de trois signaux macro
déjà testés individuellement cette session : breakeven inflation
(#200, seul PASS net), demandes continues de chômage (#322), balance
commerciale (#327) (spécification pré-enregistrée dans
PREREG_macro_combo_breakeven_claims_trade_overlay.md, committée avant
...
de porte déjà validées des trois scripts d'origine : #200
(nonml_inflation_breakeven_overlay_backtest.py), #322
(nonml_continuing_claims_overlay_backtest.py), #327
(nonml_trade_balance_overlay_backtest.py). Logique majoritaire ≥2/3
déjà utilisée au #299, Règle 7.
"""
import sys
...
def majority_2_of_3(g1: np.ndarray, g2: np.ndarray, g3: np.ndarray) -> np.ndarray:
    votes = (g1 == CUT).astype(int) + (g2 == CUT).astype(int) + (g3 == CUT).astype(int)
    return votes >= 2
...
def main():
    ...
    lines = [
        ...
        "",
        f"`position(t) = {CUT}x` si AU MOINS 2 des 3 gates (#200 T10YIE tercile haut, #322 CCSA tercile "
        f"haut, #327 BOPGSTB tercile bas) sont actifs, `1.0x` sinon. Design purement défensif. "
    ...
    for name, fname in MARKETS.items():
        ...
        pos_active = majority_2_of_3(gate1[start:], gate2[start:], gate3[start:])
        ...
        if name == "NDX (40 ans)":
            ...
            np.savez(ROOT / "results" / "nonml_macro_combo_breakeven_claims_trade_overlay_pnl.npz",
    ...
    out = ROOT / "results" / "nonml_macro_combo_breakeven_claims_trade_overlay_result.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
    return verdict
