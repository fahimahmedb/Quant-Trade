"""Robustesse — Low-Vol Tilt + overlay proximité plus haut 52-semaines
indice. Grille CAP ET grille de seuil (PAS un retuning), autour des
valeurs pré-enregistrées (CAP=2.0x, seuil=95%).
"""
import sys
...
from nonml_lowvol_index52w_high_overlay_backtest import (  # noqa: E402
    load_prices, VOL_WINDOW, REBAL_EVERY, TERCILE, COST_BPS, INDEX_LOOKBACK,
)
...
def evaluate(P, R, weights_lowvol, cap, threshold):
    start = VOL_WINDOW
...
def main():
    ...
    lines = [
        "# Robustesse — Low-Vol + overlay proximité plus haut 52-semaines indice (grilles CAP et seuil, PAS un retuning)",
        "",
    ...
    for cap in CAP_GRID:
        sharpe_ok, ret_ok, sharpe, ret, mdd = evaluate(P, R, weights_lowvol, cap, 0.95)
    ...
    for thr in THRESHOLD_GRID:
        sharpe_ok, ret_ok, sharpe, ret, mdd = evaluate(P, R, weights_lowvol, 2.0, thr)
        marker = " ← seuil pré-enregistré" if thr == 0.95 else ""
        lines.append(f"| {100*thr:.0f}% | {'OUI' if sharpe_ok else 'non'} | {'OUI' if ret_ok else 'non'} | "
                     f"{sharpe:+.2f} | {100*ret:+.1f}% | {mdd:.1f}%{marker} |")

    out = ROOT / "results" / "nonml_lowvol_index52w_high_overlay_robustness.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
