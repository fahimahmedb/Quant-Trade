...
breadth nette hauts-bas NDX-100, ~3 derniers mois. Spécification
...
from nonml_net_breadth_vol_targeting_overlay_backtest import (  # noqa: E402
    compute_net_breadth_series, combined_position, CAP, COST_BPS,
)
...
def main():
    ...
    lines = [
        ...
        f"| **Overlay breadth nette** | **{equity_ov[-1]:.2f} EUR** | "
    ...
    out = ROOT / "results" / "nonml_net_breadth_vol_targeting_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
