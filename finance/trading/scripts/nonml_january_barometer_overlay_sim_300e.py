"""Simulation — 300 EUR dans l'overlay January Barometer (NDX), ~3
...
from nonml_january_barometer_overlay_backtest import position_for, COST_BPS, CAP  # noqa: E402
...
def main():
    ...
    lines = [
        "# Simulation — 300 EUR, overlay January Barometer (NDX, ~3 derniers mois)",
        "",
        f"Période : {pd.Timestamp(dates[1]).date()} → {pd.Timestamp(dates[-1]).date()} "
        f"({len(bh_full)} séances). CAP={CAP}x (2026 a eu un janvier positif sur NDX -> "
        "levier actif toute la fenêtre, avril-juillet étant en dehors de janvier).",
        "",
        "| | Capital final | Rendement période | MDD | Sharpe ann. |",
        "|---|---|---|---|---|",
        f"| BuyHold | {equity_bh[-1]:.2f} EUR | {100*(equity_bh[-1]/CAPITAL0-1):+.1f}% | "
        f"{mdd(equity_bh):.1f}% | {me_bh['sharpe_ann']:+.2f} |",
        f"| **Overlay January Barometer** | **{equity_ov[-1]:.2f} EUR** | "
        ...
        f"est de {pos.mean():.2f}x — illustration seulement (la fenêtre entière est hors janvier, "
        "le signal est donc constant sur toute la période simulée), le verdict statistique reste "
        "celui du backtest complet (PASS 5/5 marchés) et de la robustesse (5/5 à CAP 1.5x-2.0x, "
        "4/5 à 2.5x-3.0x)."
    ]

    out = ROOT / "results" / "nonml_january_barometer_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
