"""Audit indépendant — Taux réel TIPS 10 ans (DFII10).
...
aussi l'absence de fuite (troncature de l'historique).
"""
import sys
...
from nonml_real_rate_overlay_backtest import (  # noqa: E402
    COST_BPS, CUT, MARKETS, load_dfii10_lag, expanding_tercile_cut_high,
)


def manual_dfii10_lag(dates: pd.DatetimeIndex) -> np.ndarray:
    raw = pd.read_csv(REPO_ROOT / "data" / "dfii10_daily.csv")
    raw["observation_date"] = pd.to_datetime(raw["observation_date"])
    obs = raw.dropna(subset=["DFII10"]).sort_values("observation_date")
    obs_dates = obs["observation_date"].values
    vals = obs["DFII10"].astype(float).values
...
def main():
    lines = ["# Audit — Taux réel TIPS 10 ans (DFII10)", ""]
    all_ok = True
    ...
    for name, fname in MARKETS.items():
        ...
        dfii10_lag_official = load_dfii10_lag(dates)[1:]
        pos_official = expanding_tercile_cut_high(dfii10_lag_official)

        dfii10_lag_manual = manual_dfii10_lag(dates)[1:]
        diff_align = np.nanmax(np.abs(dfii10_lag_official - dfii10_lag_manual))

        start = int(np.argmax(np.isfinite(dfii10_lag_manual)))
        T = len(dfii10_lag_manual)
        pos_manual = np.full(T, np.nan)
        for t in range(start, T):
            if not np.isfinite(dfii10_lag_manual[t]):
                continue
            hist = dfii10_lag_manual[start:t + 1]
            hist = hist[np.isfinite(hist)]
            thresh = manual_percentile_high_tercile(hist)
            pos_manual[t] = CUT if dfii10_lag_manual[t] >= thresh else 1.0
        ...
        for i in np.where(mask)[0][pos_diff_mask]:
            local_diff = abs(dfii10_lag_official[i] - dfii10_lag_manual[i])
            if local_diff >= 1e-10:
        ...
        if n_pos_diff == 0:
            ...
            lines.append(f"- **{n_pos_diff}/{n_total} désaccords de position** — sensibilité de bord flottante "
                         f"documentée (même pattern que #193/#195/#196/#197/#198/#199/#200) : {'confirmé' if boundary_tie_ok else 'NON confirmé, écart réel'}.")
    ...
    dates = pd.DatetimeIndex(df["date"].values)
    dfii10_lag_full = load_dfii10_lag(dates)[1:]
    valid_start = int(np.argmax(np.isfinite(dfii10_lag_full)))
    T_CUT = valid_start + 2000
    pos_full = expanding_tercile_cut_high(dfii10_lag_full)
    pos_truncated = expanding_tercile_cut_high(dfii10_lag_full[:T_CUT])
    ...
    out = ROOT / "results" / "nonml_real_rate_overlay_audit.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
    return all_ok
