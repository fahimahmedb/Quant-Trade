"""Backtest — Overlay vol-targeting gaté par la breadth de rebond rapide
post-creux (seuil absolu +10% sous 20j, distinct du #111 profondeur et
du #100/#112 dispersion/spread) (spécification pré-enregistrée dans
PREREG_rebound_speed_breadth_vol_targeting_overlay.md, committée avant
...
LOW_LOOKBACK = 20
REBOUND_THRESHOLD = 1.10  # close >= 1.10 * plus bas glissant 20j => rebond >= 10%
MEDIAN_WINDOW = 252
COST_BPS = 5.0
CAP = 2.0
VOL_WINDOW = 20
...
def load_all_prices():
    ...
    for path in sorted(PRICES_DIR.glob("*.json")):
        ...
        if len(close) > LOW_LOOKBACK + 21:
            series[path.stem] = close
    return series


def compute_rebound_breadth_series() -> pd.Series:
    """Renvoie Breadth_Rebound(t) = fraction des titres COTES ce jour-la
    (prix du jour fini) dont le prix est >=10% au-dessus de leur plus
    bas glissant 20j (fenetre PLEINE requise), indexee sur le calendrier
    UNION des tickers. Denominateur = tous les titres listes (meme
    convention que #89/#111)."""
    ...
    rolling_low = np.full((T, n_tickers), np.nan)
    has_full = np.zeros((T, n_tickers), dtype=bool)
    for i in range(LOW_LOOKBACK, T):
        window = close[i - LOW_LOOKBACK + 1:i + 1]
        has_full[i] = np.isfinite(window).all(axis=0)
        with np.errstate(all="ignore"):
            rolling_low[i] = np.nanmin(window, axis=0)
    fast_rebound = np.where(has_full, close >= REBOUND_THRESHOLD * rolling_low, False)

    n_listed = exists.sum(axis=1)
    n_rebound = (fast_rebound & exists).sum(axis=1)
    with np.errstate(divide="ignore", invalid="ignore"):
        breadth = np.where(n_listed > 0, n_rebound / n_listed, np.nan)
...
def main():
    ...
    breadth_rb = compute_rebound_breadth_series()
    median_rb = breadth_rb.rolling(MEDIAN_WINDOW).median()
    gate_series = (breadth_rb >= median_rb)
    ...
    lines = [
        ...
        f"si Breadth_Rebound(t) (fraction des titres NDX-100 ≥10% au-dessus de leur plus bas "
        f"glissant 20j) ≥ sa médiane glissante {MEDIAN_WINDOW}j, sinon 1.0x. {len(bh_t)} séances "
        f"testables (échantillon restreint à la période où la breadth titre-par-titre est disponible).",
        "",
        f"%j porte rebond rapide active : {100*gate_active.mean():.1f}%",
        f"Position moyenne : {pos.mean():.2f}x",
        f"Breadth rebond rapide moyenne (toute la période) : {100*np.nanmean(breadth_rb.values):.1f}%",
        "",
        "| | Sharpe ann. | Rendement total net | MDD |",
        "|---|---|---|---|",
        f"| Buy&Hold (NDX) | {me_bh['sharpe_ann']:+.2f} | {100*ret_bh:+.1f}% | {me_bh['max_drawdown_pct']:.1f}% |",
        f"| **Overlay vol-targeting gaté rebond rapide** | **{me_ov['sharpe_ann']:+.2f}** | "
    ...
    if verdict:
        ...
        lines.append("**PASS niveau 1 seulement -- pas un verdict final (Règle 9). Doit encore passer "
                     "`nonml_pass_validation_battery.py rebound_speed_breadth_vol_targeting_overlay` "
                     "(stress coûts/crise, stabilité temporelle, SPA, DSR à n_trials=backlog).**")

    out = ROOT / "results" / "nonml_rebound_speed_breadth_vol_targeting_overlay_result.md"
    out.write_text("\n".join(lines) + "\n")

    if verdict:
        dates_used = dates_idx.values[1:][start:]
        np.savez(
            ROOT / "results" / "nonml_rebound_speed_breadth_vol_targeting_overlay_pnl.npz",
