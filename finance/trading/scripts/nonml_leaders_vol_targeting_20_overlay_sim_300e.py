"""Simulation — 300 EUR, Leaders 52-semaines + overlay vol-targeting
cible 20%, ~3 derniers mois. Spécification pré-enregistrée (CAP=2.0x,
fenêtre=20j), aucun paramètre retouché après les résultats précédents.
"""
import sys
...
from nonml_leaders_vol_targeting_20_overlay_backtest import (  # noqa: E402
    load_prices, vol_target_exposure, LOOKBACK, REBAL_EVERY, TERCILE, COST_BPS, CAP,
)
...
def main():
    ...
    lines = [
        "# Simulation — 300 EUR, Leaders + overlay vol-targeting cible 20% (~3 derniers mois)",
        "",
        ...
        f"| **Leaders + overlay vol-targeting 20%** | **{equity_lev[-1]:.2f} EUR** | "
        ...
        "robustesse, qui est SEULEMENT partielle ici : le résultat tient au CAP/fenêtre "
        "pré-enregistrés et se dégrade en-dessous (CAP=1.5x échoue le Sharpe, "
        "fenêtre=15j échoue les deux jambes) — signalé honnêtement, à distinguer des "
        "cycles à plateau parfait comme #37/#38."
    ]

    out = ROOT / "results" / "nonml_leaders_vol_targeting_20_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
