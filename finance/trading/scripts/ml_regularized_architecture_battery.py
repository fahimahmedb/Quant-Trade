"""ML-3 -- batterie de validation renforcee (5 controles a-e du §2.4 de
...
ml_regularized_architecture_backtest.py (positions OOS, rendements log
...
Usage : python3 ml_regularized_architecture_battery.py [tag]
"""
import sys
import warnings
...
TAG = sys.argv[1] if len(sys.argv) > 1 else "ml_regularized_architecture"
...
def criterion(mc, mb):
    """Critere niveau 1 du PREREG §7 (ML-3) : (Sharpe ET rendement) OU Calmar."""
...
L = []
w = L.append
w(f"# ML-3 — Batterie de validation renforcée ({TAG})\n")
...
w("| Coût (bps) | Sharpe HistGB-ES | Sharpe BH | Rdt ann. HistGB-ES | Rdt ann. BH | Calmar HistGB-ES | Calmar BH | Critère |")
w("|---|---|---|---|---|---|---|---|")
ok_a = True
for mult in (1, 3, 5):
...
# --------------------------------------------------------- b. stress de crise
w("## b. Stress de crise (MDD HistGB-ES vs Buy & Hold)\n")
w("| Fenêtre | Séances | MDD HistGB-ES | MDD BH | Pas pire que BH |")
w("|---|---|---|---|---|")
...
w("| Fold | Séances | Période | Sharpe HistGB-ES | Sharpe BH | HistGB-ES > BH |")
w("|---|---|---|---|---|---|")
T = len(r)
flen = T // N_FOLDS
n_beat = n_scored = 0
...
spa = spa_test({"HistGB-ES": -pnl_c, "BuyHold": -pnl_b}, bench="BuyHold")
