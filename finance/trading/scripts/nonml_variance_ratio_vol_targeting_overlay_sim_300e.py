"""Simulation — 300 EUR dans l'overlay vol-targeting gaté par le ratio
de variance de Lo-MacKinlay glissant (NDX), ~3 derniers mois.
Spécification pré-enregistrée (Q=5, VR_WINDOW=252, CAP=2.0x,
...
from nonml_variance_ratio_vol_targeting_overlay_backtest import (  # noqa: E402
    momentum_regime_mask, combined_position, COST_BPS, CAP, VOL_WINDOW,
    TARGET_VOL_ANNUAL, Q, VR_WINDOW,
)
...
def main():
    ...
    gate_all = momentum_regime_mask(bh_full_all)
    ...
    lines = [
        ...
        f"({len(bh_full)} séances). VR(q={Q}) sur fenêtre {VR_WINDOW}j, vol cible "
        ...
        f"| **Overlay vol-targeting VR gaté** | **{equity_ov[-1]:.2f} EUR** | "
        ...
        "DAX échoue) et de la robustesse (plateau correct mais pas parfait sur les grilles CAP et "
        "fenêtre de vol, notamment 2/5 à fenêtre=15j)."
    ]

    out = ROOT / "results" / "nonml_variance_ratio_vol_targeting_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
