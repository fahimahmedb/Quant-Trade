"""Audit indépendant — Corrélation cross-marché NDX-Russell 2000 (domestique).
...
from nonml_cross_market_correlation_ndx_russell_overlay_backtest import (  # noqa: E402
...
def manual_corr_series() -> pd.Series:
    ...
    russell_v = russell_r.reindex(common).values
    T = len(common)
    out = np.full(T, np.nan)
    for t in range(CORR_WINDOW - 1, T):
        x = ndx_v[t - CORR_WINDOW + 1:t + 1]
        y = russell_v[t - CORR_WINDOW + 1:t + 1]
...
def main():
    lines = ["# Audit — Corrélation cross-marché NDX-Russell 2000 (domestique)", ""]
    all_ok = True
    ...
    for name, fname in MARKETS.items():
        ...
        if n_pos_diff == 0:
            ...
            lines.append(f"- **{n_pos_diff}/{n_total} désaccords de position** — tous localisés à un point où "
                         f"`corr(t)` tombe à <1e-10 du seuil de tercile (sensibilité de bord flottante, même "
                         f"pattern que les #193/#195 : {'confirmé' if boundary_tie_ok else 'NON confirmé, écart réel'}).")
    ...
    out = ROOT / "results" / "nonml_cross_market_correlation_ndx_russell_overlay_audit.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
    return all_ok
