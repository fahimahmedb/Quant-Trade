"""Audit indépendant — cycle #184 (Halloween x mid-term multiplicatif).
...
from nonml_halloween_midterm_multiplicative_overlay_backtest import CAP, MARKETS  # noqa: E402
...
def independent_position(dates: pd.Series) -> np.ndarray:
    ...
    for i, ts in enumerate(d):
        ...
        factor_m = CUT if mid else 1.0
        out[i] = 1.0 * factor_h * factor_m
    return out


def main():
    lines = ["# Audit indépendant — cycle #184 (Halloween x mid-term multiplicatif)", ""]
    all_ok = True
    ...
    out = ROOT / "results" / "nonml_halloween_midterm_multiplicative_overlay_audit.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
