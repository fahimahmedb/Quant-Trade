"""Backtest — Overlay vol-targeting gaté par la kurtosis (aplatissement)
glissante (spécification pré-enregistrée dans
PREREG_kurtosis_vol_targeting_overlay.md, committée avant ce script).
Nouveau type de porte (moment statistique d'ordre 4), suite au VR (#217,
PASS) et à la skewness (#218, FAIL). n_trials=1, aucune dépendance ML.
...
def rolling_kurtosis_series(r: np.ndarray) -> np.ndarray:
    """Exces de kurtosis glissant calcule sur r[k-KURT_WINDOW:k]
    (EXCLUANT r[k] lui-meme, connu seulement a la cloture du jour k+1).
    NaN avant que la fenetre soit disponible."""
    n = len(r)
    kt = np.full(n, np.nan)
    for k in range(KURT_WINDOW, n):
        window = r[k - KURT_WINDOW:k]
        kt[k] = kurtosis(window, fisher=True, bias=False)
    return kt


def calm_kurtosis_mask(r: np.ndarray) -> np.ndarray:
    """Porte = kurtosis glissante <= sa mediane glissante MEDIAN_WINDOW
    jours (queues moins epaisses que la norme recente = regime calme).
    Deja causale par construction (rolling_kurtosis_series exclut r[k])."""
    kt = rolling_kurtosis_series(r)
    med = pd.Series(kt).rolling(MEDIAN_WINDOW).median().values
    gate = kt <= med
...
def combined_position(r: np.ndarray, gate: np.ndarray) -> np.ndarray:
    ...
    (causale par construction, comme les #217/#218)."""
...
def main():
    lines = [
        ...
        f"si l'excès de kurtosis glissant {KURT_WINDOW}j est ≤ sa médiane glissante {MEDIAN_WINDOW}j, "
        f"sinon 1.0x. Échantillon testable = à partir de la {max(KURT_WINDOW, MEDIAN_WINDOW)+2}e séance.",
        "",
    ...
    for name, fname in MARKETS.items():
        ...
        gate = calm_kurtosis_mask(bh_full)
        ...
        if name == "NDX (40 ans)":
            ...
            np.savez(ROOT / "results" / "nonml_kurtosis_vol_targeting_overlay_pnl.npz",
    ...
    out = ROOT / "results" / "nonml_kurtosis_vol_targeting_overlay_result.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
