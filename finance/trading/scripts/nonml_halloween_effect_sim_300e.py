"""Simulation — 300 EUR dans l'overlay Halloween (NDX), ~3 derniers mois.
Spécification pré-enregistrée (CAP=2.0x), aucun paramètre retouché après
...
def main():
    ...
    month_full = dates_full.dt.month.values

    close = close_full[-(WINDOW_DAYS + 1):]
    month = month_full[-(WINDOW_DAYS + 1):]
    dates = df["date"].values[-(WINDOW_DAYS + 1):]

    bh_full = np.log(close[1:] / close[:-1])
    is_winter = (month[1:] >= 11) | (month[1:] <= 4)
    ...
    lines = [
        "# Simulation — 300 EUR, overlay Halloween (NDX, ~3 derniers mois)",
        "",
        f"Période : {pd.Timestamp(dates[1]).date()} → {pd.Timestamp(dates[-1]).date()} "
        f"({len(bh_full)} séances). CAP=2.0x nov-avril, 1.0x sinon.",
        "",
        "| | Capital final | Rendement période | MDD | Sharpe ann. |",
        "|---|---|---|---|---|",
        f"| BuyHold | {equity_bh[-1]:.2f} EUR | {100*(equity_bh[-1]/CAPITAL0-1):+.1f}% | "
        f"{mdd(equity_bh):.1f}% | {me_bh['sharpe_ann']:+.2f} |",
        f"| **Overlay Halloween** | **{equity_ov[-1]:.2f} EUR** | "
        f"**{100*(equity_ov[-1]/CAPITAL0-1):+.1f}%** | {mdd(equity_ov):.1f}% | {me_ov['sharpe_ann']:+.2f} |",
        "",
        "**Lecture honnête** : la fenêtre (avril-juillet) est presque entièrement en régime "
        "\"été\" (1.0x), donc cette simulation courte teste peu l'effet réel — illustration "
        "seulement, le verdict statistique reste celui du backtest complet (PASS 4/5 marchés) "
        "et de la robustesse (plateau 4,4,3,3 selon CAP)."
    ]

    out = ROOT / "results" / "nonml_halloween_effect_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
