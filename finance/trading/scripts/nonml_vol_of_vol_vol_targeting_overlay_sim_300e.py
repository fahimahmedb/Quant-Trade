...
vol-de-la-vol glissante (NDX), ~3 derniers mois. Spécification
pré-enregistrée (VOV_WINDOW=252, MEDIAN_WINDOW=252, CAP=2.0x,
...
from nonml_vol_of_vol_vol_targeting_overlay_backtest import (  # noqa: E402
    stable_vol_regime_mask, combined_position, COST_BPS, CAP, VOL_WINDOW,
    TARGET_VOL_ANNUAL, VOV_WINDOW, MEDIAN_WINDOW,
)
...
def main():
    ...
    gate_all = stable_vol_regime_mask(bh_full_all)
    ...
    lines = [
        ...
        f"({len(bh_full)} séances). Fenêtre vol-de-la-vol/médiane {VOV_WINDOW}j/{MEDIAN_WINDOW}j, "
        ...
        f"| **Overlay vol-targeting vol-de-la-vol gaté** | **{equity_ov[-1]:.2f} EUR** | "
        ...
        "Russell 2000 échoue) et de la robustesse (plateau correct, CAP stable à excellent 4/5-5/5, "
        "fenêtre de vol un peu plus sensible 3/5-4/5)."
    ]

    out = ROOT / "results" / "nonml_vol_of_vol_vol_targeting_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
