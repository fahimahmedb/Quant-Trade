"""Simulation — 300 EUR, Low-Vol Tilt + overlay SMA200 levé, ~3 derniers
mois. Spécification pré-enregistrée (CAP=2.0x), aucun paramètre retouché
...
def main():
    ...
    lines = [
        "# Simulation — 300 EUR, Low-Vol Tilt + overlay SMA200 levé (~3 derniers mois)",
        "",
        ...
        f"| **Low-Vol + overlay SMA200 x{CAP}** | **{equity_lev[-1]:.2f} EUR** | "
    ...
    out = ROOT / "results" / "nonml_lowvol_sma200_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
