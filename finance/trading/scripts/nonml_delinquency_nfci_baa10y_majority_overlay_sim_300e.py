"""Simulation — 300 EUR, porte majoritaire (≥2/3) défaut carte + NFCI
+ BAA10Y (cycle #299) sur les ~3 derniers mois disponibles (NDX).
Aucun paramètre retouché. Gabarit repris tel quel du #289/#286/#296
(Règle 7).
"""
import sys
...
def main():
    ...
    lines = [
        f"# Simulation — 300 EUR, porte majoritaire ≥2/3 défaut carte+NFCI+BAA10Y (cycle #299), {MARKET_NAME} (~3 derniers mois)",
        "",
        f"Période : {dates_w[0].date()} → {dates_w[-1].date()} ({len(r)} séances). "
        f"{100*frac_cut:.0f}% de la fenêtre est en régime coupé (au moins 2 des 3 signaux de stress "
        ...
        f"| **Overlay porte majoritaire ≥2/3** | **{eq_ov[-1]:.2f} EUR** | "
    ...
    out = ROOT / "results" / "nonml_delinquency_nfci_baa10y_majority_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
