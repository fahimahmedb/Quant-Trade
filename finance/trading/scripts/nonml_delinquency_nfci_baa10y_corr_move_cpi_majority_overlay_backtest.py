"""Backtest — Panel élargi à 6 signaux (défaut carte #286, NFCI #291,
BAA10Y #199, corrélation cross-marché NDX-DAX #193, volatilité
implicite obligataire MOVE #357, inflation réalisée CPI #338), vote
majoritaire ≥5/6, overlay défensif (spécification pré-enregistrée dans
PREREG_delinquency_nfci_baa10y_corr_move_cpi_majority_overlay.md,
...
build_corr_series/load_corr_lag du #193, load_move_series/load_move_lag
du #357, build_cpi_growth_series/load_cpi_growth_lag du #338,
expanding_tercile_gate_high du #296 (générique, étendue ici à une 6e
série sans modification).
"""
import sys
...
def main():
    ...
    lines = [
        "# Résultat — Panel élargi à 6 signaux (défaut carte #286 + NFCI #291 + BAA10Y #199 + "
        "corrélation NDX-DAX #193 + MOVE #357 + CPI #338), vote ≥5/6, overlay défensif (pré-enregistré)",
        "",
        f"`position(t) = {CUT}x` si AU MOINS 5 des 6 signaux sont dans leur tercile expanding "
        f"le plus HAUT, `1.0x` sinon. Design purement défensif. Coûts {COST_BPS:.0f} bps.",
        "",
        "| Marché | Séances test. | % temps coupé (≥5/6) | BH Sharpe | BH Rdt total | BH MDD | Overlay Sharpe | Overlay Rdt total | Overlay MDD | Sharpe>BH | Rdt>BH |",
        "|---|---|---|---|---|---|---|---|---|---|---|",
    ]
    ...
    for name, fname in MARKETS.items():
        ...
        valid_t = valid[start:]
        bh_t = bh_full[start:]
        assert valid_t.all(), "les six series doivent etre valides sur toute la fenetre testable"
        ...
        if name == "NDX (40 ans)":
            ...
            np.savez(ROOT / "results" / "nonml_delinquency_nfci_baa10y_corr_move_cpi_majority_overlay_pnl.npz",
    ...
    out = ROOT / "results" / "nonml_delinquency_nfci_baa10y_corr_move_cpi_majority_overlay_result.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
    return verdict
