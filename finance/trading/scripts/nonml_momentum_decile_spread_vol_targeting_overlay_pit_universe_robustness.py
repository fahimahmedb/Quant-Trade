...
(`nonml_momentum_decile_spread_vol_targeting_overlay_robustness.py`) : CAP de
...
Usage : python3 scripts/nonml_momentum_decile_spread_vol_targeting_overlay_pit_universe_robustness.py
"""
import sys
...
OUT = ROOT / "results" / "nonml_momentum_decile_spread_vol_targeting_overlay_pit_universe_robustness.md"
...
def main():
    ...
    spread, _cov = bt.compute_decile_spread_series_pit()
    median_b = spread.rolling(bt.MEDIAN_WINDOW).median()
    # meme masque explicite qu'au backtest (piege du #396)
    signal_defined = spread.notna() & median_b.notna()
    gate_series = (spread >= median_b).where(signal_defined)
    ...
    L = ["# Robustesse — spread décile de momentum, univers POINT-IN-TIME", ""]
    ...
    L.append("`MEDIAN_WINDOW` (252) et `DECILE_FRACTION` (0,10) définissent l'hypothèse et restent figés.")
    L.append("")
    ...
    L.append("`results/nonml_momentum_decile_spread_vol_targeting_overlay_robustness.md`.")
    L.append("")
