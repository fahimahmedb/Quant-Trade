"""Audit indépendant — Positionnement spéculatif net CFTC (COT, NASDAQ-100).

Recalcule NetPctSpéc_lag(t), le tercile expanding et la position par
...
backtest. Vérifie le décalage de publication (5j) + causal (1j) sur
...
from nonml_cot_positioning_overlay_backtest import (  # noqa: E402
    PUBLICATION_LAG_DAYS, build_cot_net_pct_series, load_cot_lag,
)


def manual_cot_lag(dates: pd.DatetimeIndex) -> np.ndarray:
    ...
    raw = pd.read_csv(REPO_ROOT / "data" / "nasdaq100_cot_positioning_weekly.csv")
...
def main():
    lines = ["# Audit — Positionnement spéculatif net CFTC (COT, NASDAQ-100)", ""]
    all_ok = True

    cot_series = build_cot_net_pct_series()
    ...
    for name, fname in MARKETS.items():
        ...
        cot_official = load_cot_lag(dates, cot_series)[1:]
        pos_official = expanding_tercile_cut_high(cot_official)

        cot_manual = manual_cot_lag(dates)[1:]
        ...
        if n_pos_diff == 0:
            ...
            lines.append(f"- **{n_pos_diff}/{n_total} désaccords de position** — sensibilité de bord flottante "
                         f"documentée (même pattern que #193/#195/.../#359) : "
                         f"{'confirmé' if boundary_tie_ok else 'NON confirmé, écart réel'}.")
        lines.append(f"- **{'OK' if ok else 'ÉCART DÉTECTÉ'}**")
        lines.append("")

    # Verification specifique du decalage de publication (5j) + causal (1j)
    # sur 5 dates CONSECUTIVES se terminant sur la derniere transition de
    # valeur reelle.
    raw = pd.read_csv(REPO_ROOT / "data" / "nasdaq100_cot_positioning_weekly.csv")
    raw.columns = [c.strip() for c in raw.columns]
    raw["as_of_date"] = pd.to_datetime(raw["as_of_date"])
    ...
    cot_check = load_cot_lag(check_dates, cot_series)
    ...
    lines.append(f"- NetPctSpéc_lag({d_avail.date()}) = {cot_check[2]:.6f} (doit valoir {v_prev:.6f}, JAMAIS {v_last:.6f} — "
                 f"la valeur n'est censée devenir visible qu'au jour SUIVANT sa disponibilité, via shift(1))")
    lines.append(f"- NetPctSpéc_lag({(d_avail + pd.Timedelta(days=1)).date()}) = {cot_check[3]:.6f} "
    ...
    dates = pd.DatetimeIndex(df["date"].values)
    cot_full = load_cot_lag(dates, cot_series)[1:]
    ...
    out = ROOT / "results" / "nonml_cot_positioning_overlay_audit.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
    return all_ok
