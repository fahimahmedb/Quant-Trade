"""Simulation — 300 EUR, Momentum de constance + overlay SMA200 levé,
...
from nonml_momentum_consistency_sma200_overlay_backtest import (  # noqa: E402
    load_all_prices, index_trend_series, consistency_at, LOOKBACK, REBAL_EVERY, TERCILE, COST_BPS, CAP,
)
...
def main():
    ...
    pnl_base_full = (weights_cons * R).sum(axis=1)
    turn_base = np.abs(np.diff(weights_cons, axis=0, prepend=weights_cons[:1])).sum(axis=1) / 2.0
    ...
    lines = [
        ...
        "Référence = momentum de constance 1.0x (cycle #82), pas Buy&Hold classique.",
        "",
        "| | Capital final | Rendement période | MDD | Sharpe ann. |",
        "|---|---|---|---|---|",
        f"| Momentum de constance 1.0x (référence) | {equity_base[-1]:.2f} EUR | "
        f"{100*(equity_base[-1]/CAPITAL0-1):+.1f}% | {mdd(equity_base):.1f}% | {me_base['sharpe_ann']:+.2f} |",
        f"| **Momentum de constance + overlay SMA200 x{CAP}** | **{equity_lev[-1]:.2f} EUR** | "
    ...
    out = ROOT / "results" / "nonml_momentum_consistency_sma200_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
