"""Backtest — Leaders 52-semaines + overlay levé sur fenêtre ToM
(spécification pré-enregistrée dans PREREG_leaders_tom_overlay.md,
...
def main(causal=True):
    ...
    lines = [
        "# Résultat — Leaders 52-semaines + overlay levé ToM (pré-enregistré, combinaison #4+#8)",
        "",
        ...
        f"| **Leaders + overlay ToM x{CAP}** | **{me_lev['sharpe_ann']:+.2f}** | "
    ...
    out = ROOT / "results" / "nonml_leaders_tom_overlay_result.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
