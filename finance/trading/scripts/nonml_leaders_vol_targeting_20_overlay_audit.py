"""Audit adversarial — Leaders 52-semaines + overlay de vol-targeting
continu, cible 20%.

Même protocole que l'audit du #45 (recalcul indépendant ddof=1, test
anti-lookahead, exposition totale conforme).
"""
import sys
...
def main():
    ...
    lines = [
        ...
        "",
        "**Lecture** : confirme l'hypothèse -- en relevant la cible de vol à 20% (au lieu de "
        "15% au #45), l'exposition moyenne passe de 0,91x à 1,21x, ce qui referme l'écart de "
        "rendement (+98,7%→+116,4%) tout en gardant un Sharpe quasi stable. Le MDD se dégrade "
        "légèrement (-21,3%→-22,9%), cohérent avec une exposition moyenne désormais >1.0x.",
    ]

    out = ROOT / "results" / "nonml_leaders_vol_targeting_20_overlay_audit.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
