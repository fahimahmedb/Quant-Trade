...
confirmation multi-marché élargie (NDX), ~3 derniers mois.
...
from nonml_multimarket_breadth_vol_targeting_overlay_backtest import (  # noqa: E402
    compute_multimarket_breadth_series, combined_position, BREADTH_THRESHOLD, CAP, COST_BPS,
)
...
def main():
    ...
    close_full = df_ndx["close"].values
    dates_full = df_ndx["date"].values
    ...
    lines = [
        "# Simulation — 300 EUR, overlay vol-targeting gaté par la confirmation multi-marché élargie (NDX, ~3 derniers mois)",
        "",
        ...
        f"| **Overlay breadth 5-marchés** | **{equity_ov[-1]:.2f} EUR** | "
    ...
    out = ROOT / "results" / "nonml_multimarket_breadth_vol_targeting_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
