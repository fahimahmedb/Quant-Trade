"""Robustesse — cycle #363 (panel élargi 5 signaux +MOVE, vote ≥4/5),
perturbation ±20% du plancher CUT=0.5x, seul paramètre non central au
critère de succès (le critère porte sur la règle de vote ≥4/5, pas sur
...
quelle du #200/#284/#289/#296/#299/#304 (Règle 7).
"""
import sys
...
def main():
    ...
    lines = [
        "# Robustesse — cycle #363, perturbation ±20% du plancher CUT",
        "",
        f"Point pré-enregistré : CUT = {CUT}x. Grille : {{{', '.join(f'{c}x' for c in CUT_GRID)}}}. "
        "La règle de vote elle-même (≥4/5 signaux dans leur tercile expanding le plus haut) n'est "
    ...
    out = ROOT / "results" / "nonml_delinquency_nfci_baa10y_corr_move_majority_overlay_robustness.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
