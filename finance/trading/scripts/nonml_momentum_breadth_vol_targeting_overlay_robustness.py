"""Robustesse — Overlay vol-targeting gaté par la breadth de momentum
NDX-100. Grille CAP ET grille de fenêtre de vol (PAS un retuning de la
porte BREADTH_THRESHOLD=50%/LOOKBACK=252/SKIP=21), autour des valeurs
...
from nonml_momentum_breadth_vol_targeting_overlay_backtest import (  # noqa: E402
    compute_momentum_breadth_series, BREADTH_THRESHOLD, COST_BPS, TARGET_VOL_ANNUAL,
...
def main():
    ...
    lines = [
        "# Robustesse — Overlay vol-targeting gaté par la breadth de momentum (grilles CAP et fenêtre, PAS un retuning de la porte)",
        "",
    ...
    for cap in CAP_GRID:
        start = max(first_valid, LOOKBACK, 20)
    ...
    for window in WINDOW_GRID:
        start = max(first_valid, LOOKBACK, window)
    ...
    out = ROOT / "results" / "nonml_momentum_breadth_vol_targeting_overlay_robustness.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
