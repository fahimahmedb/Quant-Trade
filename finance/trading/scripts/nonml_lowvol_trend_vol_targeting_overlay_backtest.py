"""Backtest — Low-Volatility Tilt + overlay combiné tendance +
vol-targeting (spécification pré-enregistrée dans
PREREG_lowvol_trend_vol_targeting_overlay.md, committée avant ce
script). Combine les cycles #15 et #47. n_trials=1, aucune dépendance
ML. Règle de succès renforcée -- référence = Low-Vol 1.0x (cycle #15),
pas Buy&Hold.

CORRECTION 05/08/2026 (cycle #255) -- fuite d'exécution « même barre »
(voir `results/nonml_same_bar_execution_audit.md`, patch #166/#167/#253/
#254 appliqué ici, jamais fait avant). `main(causal=True)` décale
...
def load_prices():
    ...
    for path in sorted(PRICES_DIR.glob("*.json")):
        ...
        if len(close) > LOWVOL_WINDOW + REBAL_EVERY:
...
def main(causal=True):
    ...
    pnl_lowvol_raw = (weights_lowvol * R).sum(axis=1)
    vol_ann = pd.Series(pnl_lowvol_raw).rolling(VOL_WINDOW).std().values * ANNUALIZATION
    ...
    pnl_base = (weights_base[start2:] * R[start2:]).sum(axis=1)
    ...
    lines = [
        "# Résultat — Low-Vol Tilt + overlay combiné tendance + vol-targeting (pré-enregistré, combinaison #15+#47)",
        "",
        ...
        f"| **Low-Vol + overlay tendance+vol-targeting** | **{me_lev['sharpe_ann']:+.2f}** | "
    ...
    np.savez(ROOT / "results" / "nonml_lowvol_trend_vol_targeting_overlay_pnl.npz",
    ...
    out = ROOT / "results" / "nonml_lowvol_trend_vol_targeting_overlay_result.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
