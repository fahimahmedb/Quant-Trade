"""Backtest — Overlay de vol-targeting continu, cible 20% (spécification
pré-enregistrée dans PREREG_vol_targeting_20_overlay.md, committée avant
ce script). Variante du #43 (vol cible 15%). n_trials=1, aucune
...
def vol_target_position(r: np.ndarray) -> np.ndarray:
    """Identique au mecanisme du #43, seule TARGET_VOL_ANNUAL change (20%
    au lieu de 15%)."""
...
def main():
    lines = [
        ...
        f"— variante du #43 (vol cible 15%). Échantillon testable = à partir de la {VOL_WINDOW+1}e séance.",
        "",
    ...
    for name, fname in MARKETS.items():
        ...
        if name == "NDX (40 ans)":
            ...
            np.savez(ROOT / "results" / "nonml_vol_targeting_20_overlay_pnl.npz",
    ...
    out = ROOT / "results" / "nonml_vol_targeting_20_overlay_result.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
