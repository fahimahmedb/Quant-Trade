"""Robustesse — Empilement diversification+ensemble3moteurs (#139),
grille de la maturité de référence (identique au #134/#136/#137, PAS
un retuning de la position équity qui reste celle du #124).
"""
import sys
...
def main():
    ...
    lines = [
        "# Robustesse — Empilement diversification+ensemble3moteurs (#139), grille de maturité (PAS un retuning)",
        "",
        "Maturité pré-enregistrée = 10 ans. Grille 5-20 ans (position équity #124 strictement inchangée).",
        "",
    ...
    out = ROOT / "results" / "nonml_diversification_bond_triple_engine_stack_robustness.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
