"""Backtest — Overlay levé sur rebond après choc 1 séance, niveau indice
(spécification pré-enregistrée dans PREREG_single_day_shock_rebound.md,
...
def rebound_exposure(close: np.ndarray) -> np.ndarray:
    ...
    trigger = day_ret <= SHOCK_THRESHOLD
...
def main():
    lines = [
        "# Résultat — Overlay levé sur rebond après choc 1 séance, niveau indice (pré-enregistré, règle renforcée)",
        "",
        f"Choc = rendement d'une seule séance ≤ {100*SHOCK_THRESHOLD:.0f}% -> CAP={CAP}x pendant "
        f"{REBOUND_LEN}j suivants (relance si nouveau choc pendant la fenêtre), 1.0x sinon.",
        "",
    ...
    out = ROOT / "results" / "nonml_single_day_shock_rebound_result.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
