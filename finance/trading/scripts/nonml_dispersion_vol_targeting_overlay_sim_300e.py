"""Simulation — 300 EUR dans l'overlay vol-targeting gaté par dispersion
cross-sectionnelle NDX-100, ~3 derniers mois. Spécification
...
from nonml_dispersion_vol_targeting_overlay_backtest import (  # noqa: E402
    compute_dispersion_series, combined_position, MEDIAN_WINDOW, COST_BPS, CAP, VOL_WINDOW, TARGET_VOL_ANNUAL,
)
...
def main():
    ...
    lines = [
        ...
        f"CAP={CAP}x quand la dispersion cross-sectionnelle est au-dessus de sa médiane 252j.",
        "",
        "| | Capital final | Rendement période | MDD | Sharpe ann. |",
        "|---|---|---|---|---|",
        f"| BuyHold | {equity_bh[-1]:.2f} EUR | {100*(equity_bh[-1]/CAPITAL0-1):+.1f}% | "
        f"{mdd(equity_bh):.1f}% | {me_bh['sharpe_ann']:+.2f} |",
        f"| **Overlay vol-targeting dispersion** | **{equity_ov[-1]:.2f} EUR** | "
        ...
        "(plateau parfait 8/8 sur les grilles CAP 1.5x-3.0x et fenêtre 15-30j)."
    ]

    out = ROOT / "results" / "nonml_dispersion_vol_targeting_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
