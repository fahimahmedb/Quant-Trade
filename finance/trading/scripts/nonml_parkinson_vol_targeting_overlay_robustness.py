"""Robustesse — Overlay vol-targeting estimateur Parkinson. Grille CAP
...
from nonml_parkinson_vol_targeting_overlay_backtest import COST_BPS, TARGET_VOL_ANNUAL, ANNUALIZATION, MARKETS  # noqa: E402
...
def position_for(df, cap: float, window: int) -> np.ndarray:
    var_pct = parkinson_var_pct(df)
    var_roll = var_pct.rolling(window).mean().values
    vol_ann = np.sqrt(var_roll) * ANNUALIZATION / 100.0
...
def main():
    ...
    lines = [
        "# Robustesse — Overlay vol-targeting Parkinson (grilles CAP et fenêtre, PAS un retuning)",
        "",
    ...
    out = ROOT / "results" / "nonml_parkinson_vol_targeting_overlay_robustness.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
