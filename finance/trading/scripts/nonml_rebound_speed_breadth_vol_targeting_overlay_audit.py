...
rebond rapide post-creux.

1. Recalcul indépendant de la breadth (boucle Python explicite par
   titre, sans réutiliser les opérations vectorisées du backtest) à un
...
from nonml_rebound_speed_breadth_vol_targeting_overlay_backtest import (  # noqa: E402
    load_all_prices, LOW_LOOKBACK, REBOUND_THRESHOLD, compute_rebound_breadth_series,
)
...
def independent_breadth_at(aligned: dict, tickers: list, i: int) -> float:
    ...
    for t in tickers:
        ...
        if not np.isfinite(px):
            continue
        n_listed += 1
        window = aligned[t][i - LOW_LOOKBACK + 1:i + 1]
        if not np.isfinite(window).all():
            continue
        if px >= REBOUND_THRESHOLD * np.min(window):
            n_rebound += 1
    return n_rebound / n_listed if n_listed > 0 else np.nan
...
def main():
    ...
    orig = compute_rebound_breadth_series()
    check_idx = list(range(LOW_LOOKBACK, T, 400))

    lines = ["# Audit adversarial — Overlay vol-targeting gaté par la breadth de rebond rapide post-creux", "",
    ...
    out = ROOT / "results" / "nonml_rebound_speed_breadth_vol_targeting_overlay_audit.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
