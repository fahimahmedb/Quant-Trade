"""Backtest — Taux de défaut hypothécaire US (DRSFRMACBS), overlay
défensif (spécification pré-enregistrée dans
PREREG_mortgage_delinquency_overlay.md, committée avant ce script).
...
def build_delinquency_series() -> pd.Series:
    ...
    raw = raw.dropna(subset=["DRSFRMACBS"]).drop_duplicates("observation_date").sort_values("observation_date")
    vals = raw["DRSFRMACBS"].astype(float).values
...
def main():
    ...
    lines = [
        ...
        "",
        f"`position(t) = {CUT}x` si DRSFRMACBS_lag(t-1) est dans son tercile expanding le plus HAUT, "
    ...
    for name, fname in MARKETS.items():
        ...
        if name == "NDX (40 ans)":
            ...
            np.savez(ROOT / "results" / "nonml_mortgage_delinquency_overlay_pnl.npz",
    ...
    out = ROOT / "results" / "nonml_mortgage_delinquency_overlay_result.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
    return verdict
