"""Audit adversarial — Empilement diversification obligataire + ensemble 3 moteurs (#139).

1. Recalcul indépendant du rendement combiné à un échantillon de dates.
2. Test anti-lookahead (mécanisme obligataire identique au #134/#136/
   #137, déjà audité -- pas de nouvelle surface de fuite).
"""
import sys
...
def main():
    ...
    lines = ["# Audit adversarial — Empilement diversification obligataire + ensemble 3 moteurs (#139)", "",
    ...
    all_ok = True
    for t in check_idx:
        nd = dates_used[t] if not hasattr(dates_used, "iloc") else dates_used.iloc[t]
        idx = None
    ...
    lines.append("")
    lines.append("Mécanisme obligataire strictement identique au #134/#136/#137 (déjà audité, 0 fuite "
                  "détectée). La position équity provient du #124 (déjà auditée au cycle #124). Aucune "
                  "nouvelle surface de fuite introduite par cet empilement — pas ré-audité en double.")

    out = ROOT / "results" / "nonml_diversification_bond_triple_engine_stack_audit.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
