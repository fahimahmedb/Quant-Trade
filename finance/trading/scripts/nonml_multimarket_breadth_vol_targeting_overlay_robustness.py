"""Robustesse — Overlay vol-targeting gaté par la confirmation
multi-marché élargie. Grille CAP ET grille de fenêtre de vol (PAS un
retuning de la porte SMA_WINDOW=200/BREADTH_THRESHOLD=0.6), autour des
...
from nonml_multimarket_breadth_vol_targeting_overlay_backtest import (  # noqa: E402
    compute_multimarket_breadth_series, BREADTH_THRESHOLD, COST_BPS, TARGET_VOL_ANNUAL,
...
def main():
    ...
    lines = [
        "# Robustesse — Overlay vol-targeting gaté par la confirmation multi-marché élargie (grilles CAP et fenêtre, PAS un retuning de la porte)",
        "",
    ...
    out = ROOT / "results" / "nonml_multimarket_breadth_vol_targeting_overlay_robustness.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
