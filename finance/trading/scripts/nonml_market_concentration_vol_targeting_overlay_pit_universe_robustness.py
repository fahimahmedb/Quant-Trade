...
(`nonml_market_concentration_vol_targeting_overlay_robustness.py`) : CAP de
l'overlay et fenêtre de volatilité. Reprises telles quelles, elles sont donc
fixées avant toute exécution.

**Ce n'est PAS un retuning** : `CONC_WINDOW` (60),
...
Usage : python3 scripts/nonml_market_concentration_vol_targeting_overlay_pit_universe_robustness.py
"""
import sys
...
OUT = ROOT / "results" / "nonml_market_concentration_vol_targeting_overlay_pit_universe_robustness.md"
...
def main():
    ...
    L = ["# Robustesse — concentration du marché (HHI), univers POINT-IN-TIME", ""]
    L.append("Grilles **identiques** à celles du cycle d'origine, donc fixées avant exécution.")
    L.append("**Pas un retuning** : `CONC_WINDOW` (60),")
    ...
    L.append("`results/nonml_market_concentration_vol_targeting_overlay_robustness.md`.")
    L.append("")
