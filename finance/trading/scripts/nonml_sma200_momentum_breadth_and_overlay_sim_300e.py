"""Simulation — 300 EUR dans l'overlay double porte AND breadth SMA200
+ breadth de momentum (NDX), ~3 derniers mois. Spécification
...
from nonml_sma200_momentum_breadth_and_overlay_backtest import (  # noqa: E402
    compute_sma200_breadth_series, compute_momentum_breadth_series, combined_position,
    SMA_BREADTH_THRESHOLD, MOM_BREADTH_THRESHOLD, CAP, COST_BPS,
)
...
def main():
    ...
    lines = [
        "# Simulation — 300 EUR, double porte AND breadth SMA200 + breadth de momentum (NDX, ~3 derniers mois)",
        "",
        ...
        f"| **Overlay AND SMA200∩momentum** | **{equity_ov[-1]:.2f} EUR** | "
    ...
    out = ROOT / "results" / "nonml_sma200_momentum_breadth_and_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
