"""Backtest — Empiler diversification obligataire (#134) sur l'ensemble à
3 moteurs de volatilité (#124) (spécification pré-enregistrée dans
PREREG_diversification_bond_triple_engine_stack.md, committée avant ce
...
def main():
    d = np.load(ROOT / "results" / "nonml_ewma_defensive_overlay_and_triple_engine_pnl.npz", allow_pickle=True)
    ...
    lines = [
        ...
        "",
        f"Position équity #124 (moyenne réalisé+GARCH+EWMA, peut légèrement dépasser 1,0x) ; fraction "
        f"(1-pos_eq) allouée au proxy obligataire DGS10 du #134. {len(r_ndx)} séances "
        f"(fenêtre commune #124 ∩ DGS10).",
        "",
        f"Fraction du temps avec levier léger (pos_eq>1,0x) : {100*frac_leveraged:.1f}%",
        "",
        ...
        f"| **#124 + diversification obligataire (#134)** | **{me_ov['sharpe_ann']:+.2f}** | "
    ...
    if verdict:
        ...
        lines.append("**PASS niveau 1 seulement -- pas un verdict final (Règle 9). Doit encore passer "
                     "`nonml_pass_validation_battery.py diversification_bond_triple_engine_stack` "
                     "(stress coûts/crise, stabilité temporelle, SPA, DSR à n_trials=backlog).**")

    out = ROOT / "results" / "nonml_diversification_bond_triple_engine_stack_result.md"
    out.write_text("\n".join(lines) + "\n")

    if verdict:
        np.savez(
            ROOT / "results" / "nonml_diversification_bond_triple_engine_stack_pnl.npz",
