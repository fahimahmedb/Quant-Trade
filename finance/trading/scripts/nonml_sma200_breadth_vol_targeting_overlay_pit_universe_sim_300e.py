"""Simulation — 300 EUR, porte breadth SMA200 univers point-in-time,
...
from nonml_sma200_breadth_vol_targeting_overlay_pit_universe_backtest import (  # noqa: E402
    compute_sma200_breadth_series_pit, combined_position, BREADTH_THRESHOLD, COST_BPS,
)
...
def main():
    ...
    lines = [
        "# Simulation — 300 EUR, porte breadth SMA200 univers point-in-time (NDX, ~3 derniers mois)",
        "",
        ...
        f"| **Overlay breadth SMA200 (PIT)** | **{equity_ov[-1]:.2f} EUR** | "
    ...
    out = ROOT / "results" / "nonml_sma200_breadth_vol_targeting_overlay_pit_universe_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
