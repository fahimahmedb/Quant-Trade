...
calendrier (NDX), ~3 derniers mois. Spécification pré-enregistrée
...
from nonml_calendar_vol_targeting_overlay_backtest import combined_position, COST_BPS, CAP, VOL_WINDOW, TARGET_VOL_ANNUAL  # noqa: E402
...
def main():
    ...
    pos_full_all = combined_position(close_full, df["date"])
    ...
    lines = [
        "# Simulation — 300 EUR, overlay vol-targeting gaté par le calendrier (NDX, ~3 derniers mois)",
        "",
        ...
        "du backtest complet (PASS 4/5 marchés) et de la robustesse (plateau 4-5/5 sur les deux "
        "grilles CAP 1.5x-3.0x et fenêtre 15-30j)."
    ]

    out = ROOT / "results" / "nonml_calendar_vol_targeting_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
