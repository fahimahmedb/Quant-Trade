"""Backtest — Positionnement spéculatif net CFTC (COT, futures
NASDAQ-100 Consolidated), overlay défensif (spécification
pré-enregistrée dans PREREG_cot_positioning_overlay.md, committée
...
le plus HAUT = défensif) du #291 (NFCI, Règle 7), et le principe de
décalage causal de publication hebdomadaire déjà validé au #204/#322
(ICSA, PUBLICATION_LAG_DAYS), adapté ici à 5 jours (rapport COT publié
le vendredi, ~3j après la date "en date du" mardi, marge de sécurité).
"""
import sys
...
def build_cot_net_pct_series() -> pd.Series:
...
def load_cot_lag(dates: pd.DatetimeIndex, cot_series: pd.Series) -> np.ndarray:
...
def main():
    ...
    lines = [
        ...
        "",
        f"`position(t) = {CUT}x` si NetPctSpéc_lag(t) est dans son tercile expanding le "
        f"plus HAUT (positionnement spéculatif net-long le plus extrême, trade « crowded »), "
    ...
    for name, fname in MARKETS.items():
        ...
        cot_lag_full = load_cot_lag(dates, cot_series)
        ...
        if name == "NDX (40 ans)":
            ...
            np.savez(ROOT / "results" / "nonml_cot_positioning_overlay_pnl.npz",
    ...
    out = ROOT / "results" / "nonml_cot_positioning_overlay_result.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
    return verdict
