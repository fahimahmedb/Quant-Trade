...
ensemble 3 moteurs (#139), ~3 derniers mois. Aucun paramètre retouché
...
def main():
    d = np.load(ROOT / "results" / "nonml_diversification_bond_triple_engine_stack_pnl.npz", allow_pickle=True)
    ...
    lines = [
        ...
        f"| **#124 + diversification obligataire** | **{equity_ov[-1]:.2f} EUR** | "
        ...
        "parfait 3/3 sur la grille de maturité). Doit encore passer la batterie Règle 9 "
        "(`nonml_pass_validation_battery.py diversification_bond_triple_engine_stack`).",
    ]

    out = ROOT / "results" / "nonml_diversification_bond_triple_engine_stack_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
