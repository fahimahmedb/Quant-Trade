"""Audit indépendant — Croissance du PIB réel US (FRED GDPC1).

Recalcule GDPGrowth_lag(t), l'alignement causal (avec décalage de 3
...
spécifiquement le décalage de 3 mois (sur une transition réelle) et
...
from nonml_real_gdp_overlay_backtest import (  # noqa: E402
    PUBLICATION_LAG_MONTHS, YOY_QUARTERS, build_gdp_growth_series, load_gdp_growth_lag,
)


def manual_gdp_growth_lag(dates: pd.DatetimeIndex) -> np.ndarray:
    """Recalcul par boucle explicite + searchsorted, sans
    pandas.reindex/ffill/DateOffset."""
    raw = pd.read_csv(REPO_ROOT / "data" / "real_gdp_quarterly.csv")
    raw["observation_date"] = pd.to_datetime(raw["observation_date"])
    raw = raw.dropna(subset=["GDPC1"]).drop_duplicates("observation_date").sort_values("observation_date")
    obs_dates = raw["observation_date"].values
    vals = raw["GDPC1"].astype(float).values
...
def main():
    lines = ["# Audit — Croissance du PIB réel US (FRED GDPC1)", ""]
    all_ok = True

    gdp_series = build_gdp_growth_series()
    ...
    for name, fname in MARKETS.items():
        ...
        gdp_lag_official = load_gdp_growth_lag(dates, gdp_series)[1:]
        pos_official = expanding_tercile_cut_low(gdp_lag_official)

        gdp_lag_manual = manual_gdp_growth_lag(dates)[1:]
        diff_align = np.nanmax(np.abs(gdp_lag_official - gdp_lag_manual))

        start = int(np.argmax(np.isfinite(gdp_lag_manual)))
        T = len(gdp_lag_manual)
        pos_manual = np.full(T, np.nan)
        for t in range(start, T):
            if not np.isfinite(gdp_lag_manual[t]):
                continue
            hist = gdp_lag_manual[start:t + 1]
            hist = hist[np.isfinite(hist)]
            thresh = manual_percentile_low_tercile(hist)
            pos_manual[t] = CUT if gdp_lag_manual[t] <= thresh else 1.0
        ...
        for i in np.where(mask)[0][pos_diff_mask]:
            local_diff = abs(gdp_lag_official[i] - gdp_lag_manual[i])
            if local_diff >= 1e-10:
    ...
    # Verification specifique du decalage de 3 mois : transition ou la
    # valeur CHANGE reellement.
    raw = pd.read_csv(REPO_ROOT / "data" / "real_gdp_quarterly.csv")
    raw["observation_date"] = pd.to_datetime(raw["observation_date"])
    raw = raw.dropna(subset=["GDPC1"]).sort_values("observation_date")
    vals = raw["GDPC1"].astype(float).values
    obs_dates = raw["observation_date"].values
    ...
    gdp_check = load_gdp_growth_lag(check_dates, gdp_series)
    ...
    lines.append(f"- GDPGrowth_lag({check_dates[1].date()}) = {gdp_check[1]:.6f}, "
                 f"GDPGrowth_lag({check_dates[2].date()}) = {gdp_check[2]:.6f} "
                 f"(doivent valoir {g_prev:.6f}, JAMAIS {g_last:.6f})")
    lines.append(f"- GDPGrowth_lag({check_dates[3].date()}) = {gdp_check[3]:.6f}, "
                 f"GDPGrowth_lag({check_dates[4].date()}) = {gdp_check[4]:.6f} "
                 f"({check_dates[3].date()} doit être le premier jour où {g_last:.6f} apparaît, via shift(1))")
    ok_shift = (not np.isclose(g_prev, g_last)) and np.isclose(gdp_check[1], g_prev) and \
        np.isclose(gdp_check[2], g_prev) and np.isclose(gdp_check[3], g_last) and np.isclose(gdp_check[4], g_last)
    all_ok &= ok_shift
    ...
    dates = pd.DatetimeIndex(df["date"].values)
    gdp_lag_full = load_gdp_growth_lag(dates, gdp_series)[1:]
    pos_full = expanding_tercile_cut_low(gdp_lag_full)
    valid_start = int(np.argmax(np.isfinite(gdp_lag_full)))
    T_CUT = valid_start + 2000
    pos_truncated = expanding_tercile_cut_low(gdp_lag_full[:T_CUT])
    ...
    out = ROOT / "results" / "nonml_real_gdp_overlay_audit.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
    return all_ok
