"""Simulation — 300 EUR, overlay inflation réalisée CPI (cycle #336)
...
from nonml_m2_growth_overlay_backtest import COST_BPS  # noqa: E402
...
def main():
    ...
    lines = [
        f"# Simulation — 300 EUR, overlay inflation réalisée CPI (cycle #336), {MARKET_NAME} (~3 derniers mois)",
        "",
        f"Période : {dates_w[0].date()} → {dates_w[-1].date()} ({len(r)} séances). "
        f"{100*frac_cut:.0f}% de la fenêtre est en régime coupé (inflation réalisée élevée).",
        "",
        "| | Capital final | Rendement période | MDD | Sharpe ann. |",
        "|---|---|---|---|---|",
        f"| Buy & Hold | {eq_bh[-1]:.2f} EUR | {100 * (eq_bh[-1] / CAPITAL0 - 1):+.1f}% | "
        f"{mdd_pct(eq_bh):.1f}% | {me_bh['sharpe_ann']:+.2f} |",
        f"| **Overlay CPI** | **{eq_ov[-1]:.2f} EUR** | "
        ...
        "et de la grille de robustesse (15/15, plateau parfait).",
    ]

    out = ROOT / "results" / "nonml_cpi_inflation_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
