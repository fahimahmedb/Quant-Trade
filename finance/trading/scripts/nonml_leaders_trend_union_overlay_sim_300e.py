"""Simulation — 300 EUR, Leaders 52-semaines + overlay union
SMA200∪52w-high, ~3 derniers mois. Spécification pré-enregistrée
(CAP=2.0x), aucun paramètre retouché après les résultats précédents.

NOTE (voir audit) : reproduit mathématiquement la simulation du #33
(52w-high ⊆ SMA200 sur cette donnée).
"""
import sys
...
from nonml_leaders_trend_union_overlay_backtest import (  # noqa: E402
    load_prices, index_trend_union_series, LOOKBACK, REBAL_EVERY, TERCILE, COST_BPS, CAP,
)
...
def main():
    ...
    lines = [
        "# Simulation — 300 EUR, Leaders + overlay union SMA200∪52w-high (~3 derniers mois)",
        "",
        ...
        f"| **Leaders + overlay union SMA200∪52w-high x{CAP}** | **{equity_lev[-1]:.2f} EUR** | "
        f"**{100*(equity_lev[-1]/CAPITAL0-1):+.1f}%** | {mdd(equity_lev):.1f}% | {me_lev['sharpe_ann']:+.2f} |",
        "",
        "**Lecture honnête** : ce résultat reproduit mathématiquement la simulation du "
        "#33 (voir audit : le signal 52w-high est un sous-ensemble strict de SMA200 sur "
        "cette donnée, l'union n'apporte donc aucune valeur ajoutée). Fenêtre courte "
        "(~3 mois) illustrative — le verdict statistique réel reste celui du backtest "
        "complet (2022-2026, PASS) et de la robustesse (plateau croissant CAP 1.5x-3.0x)."
    ]

    out = ROOT / "results" / "nonml_leaders_trend_union_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
