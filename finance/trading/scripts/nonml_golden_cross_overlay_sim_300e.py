"""Simulation — 300 EUR dans l'overlay Golden Cross SMA50/SMA200 (NDX),
...
from nonml_golden_cross_overlay_backtest import golden_cross_mask, COST_BPS, CAP, SMA_SHORT, SMA_LONG  # noqa: E402
...
def main():
    ...
    golden_full = golden_cross_mask(close_full)

    close = close_full[-(WINDOW_DAYS + 1):]
    golden = golden_full[-(WINDOW_DAYS + 1):]
    ...
    lines = [
        "# Simulation — 300 EUR, overlay Golden Cross SMA50/SMA200 (NDX, ~3 derniers mois)",
        "",
        f"Période : {pd.Timestamp(dates[1]).date()} → {pd.Timestamp(dates[-1]).date()} "
        f"({len(bh_full)} séances). CAP={CAP}x si SMA{SMA_SHORT} > SMA{SMA_LONG}, 1.0x sinon.",
        "",
        "| | Capital final | Rendement période | MDD | Sharpe ann. |",
        "|---|---|---|---|---|",
        f"| BuyHold | {equity_bh[-1]:.2f} EUR | {100*(equity_bh[-1]/CAPITAL0-1):+.1f}% | "
        f"{mdd(equity_bh):.1f}% | {me_bh['sharpe_ann']:+.2f} |",
        f"| **Overlay Golden Cross** | **{equity_ov[-1]:.2f} EUR** | "
        ...
        "reste celui du backtest complet (PASS 4/5 marchés) et de la robustesse (plateau stable "
        "4/5 sur la grille CAP 1.5x-3.0x)."
    ]

    out = ROOT / "results" / "nonml_golden_cross_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
