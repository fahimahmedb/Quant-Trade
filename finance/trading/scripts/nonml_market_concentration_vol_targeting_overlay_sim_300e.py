...
concentration du marché NDX-100, ~3 derniers mois. Spécification
...
from nonml_market_concentration_vol_targeting_overlay_backtest import (  # noqa: E402
    compute_concentration_series, combined_position, MEDIAN_WINDOW, CAP, COST_BPS,
)
...
def main():
    ...
    gate_series = (concentration <= median_conc)
    ...
    lines = [
        "# Simulation — 300 EUR, overlay vol-targeting gaté par la concentration du marché (NDX, ~3 derniers mois)",
        "",
        ...
        f"| **Overlay concentration** | **{equity_ov[-1]:.2f} EUR** | "
        ...
        "statistique réel reste celui du backtest complet (PASS) et de la robustesse (6/8, "
        "moins solide que le plateau parfait d'autres portes de ce backlog)."
    ]

    out = ROOT / "results" / "nonml_market_concentration_vol_targeting_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
