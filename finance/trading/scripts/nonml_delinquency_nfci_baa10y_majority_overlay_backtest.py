"""Backtest — Porte majoritaire (≥2 sur 3) défaut carte de crédit
(#286) + NFCI (#291) + spread de crédit BAA10Y (#199), overlay
défensif (spécification pré-enregistrée dans
PREREG_delinquency_nfci_baa10y_majority_overlay.md, committée avant ce
...
expanding_tercile_gate_high du #296 (générique, étendue ici à une 3e
...
def main():
    ...
    lines = [
        ...
        "",
        f"`position(t) = {CUT}x` si AU MOINS 2 des 3 signaux (DRCCLACBS_lag, NFCI_lag, BAA10Y_lag) "
        f"sont dans leur tercile expanding le plus HAUT, `1.0x` sinon. Design purement défensif. "
    ...
    for name, fname in MARKETS.items():
        ...
        if name == "NDX (40 ans)":
            ...
            np.savez(ROOT / "results" / "nonml_delinquency_nfci_baa10y_majority_overlay_pnl.npz",
    ...
    out = ROOT / "results" / "nonml_delinquency_nfci_baa10y_majority_overlay_result.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
    return verdict
