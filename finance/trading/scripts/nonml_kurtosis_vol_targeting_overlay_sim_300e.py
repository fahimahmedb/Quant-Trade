...
kurtosis glissante (NDX), ~3 derniers mois. Spécification pré-enregistrée
(KURT_WINDOW=252, MEDIAN_WINDOW=252, CAP=2.0x, VOL_WINDOW=20), aucun
...
from nonml_kurtosis_vol_targeting_overlay_backtest import (  # noqa: E402
    calm_kurtosis_mask, combined_position, COST_BPS, CAP, VOL_WINDOW,
    TARGET_VOL_ANNUAL, KURT_WINDOW, MEDIAN_WINDOW,
)
...
def main():
    ...
    gate_all = calm_kurtosis_mask(bh_full_all)
    ...
    lines = [
        ...
        f"({len(bh_full)} séances). Fenêtre kurtosis/médiane {KURT_WINDOW}j/{MEDIAN_WINDOW}j, "
        ...
        f"| **Overlay vol-targeting kurtosis gaté** | **{equity_ov[-1]:.2f} EUR** | "
        ...
        "DAX échoue) et de la robustesse (plateau parfait 8/8 sur les deux grilles CAP 1.5x-3.0x et "
        "fenêtre 15j-30j)."
    ]

    out = ROOT / "results" / "nonml_kurtosis_vol_targeting_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
