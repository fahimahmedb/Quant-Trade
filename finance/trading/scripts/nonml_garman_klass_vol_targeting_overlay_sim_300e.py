...
Garman-Klass (NDX), ~3 derniers mois. Spécification pré-enregistrée
...
from nonml_garman_klass_vol_targeting_overlay_backtest import garman_klass_vol_position, COST_BPS, CAP, VOL_WINDOW, TARGET_VOL_ANNUAL  # noqa: E402
...
def main():
    ...
    pos_full_all = garman_klass_vol_position(df)
    ...
    lines = [
        ...
        f"| **Overlay vol-targeting Garman-Klass** | **{equity_ov[-1]:.2f} EUR** | "
    ...
    out = ROOT / "results" / "nonml_garman_klass_vol_targeting_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
