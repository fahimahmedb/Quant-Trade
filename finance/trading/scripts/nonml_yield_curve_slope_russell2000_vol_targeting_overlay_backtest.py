"""Backtest — Pente de la courbe des taux US (T10Y2Y) appliquée au
Russell 2000 (mécanisme IDENTIQUE au #114/#126, seul le marché piloté
change, spécification pré-enregistrée dans
PREREG_yield_curve_slope_russell2000_vol_targeting_overlay.md,
committée avant ce script). n_trials=1 pour ce marché précis. Règle de
...
MARKET_FILE = "russell2000_daily.txt"
...
def main():
    ...
    gate_active = (pos > 1.0)
    lines = [
        "# Résultat — Pente de la courbe des taux US (T10Y2Y) appliquée au Russell 2000 (pré-enregistré, règle renforcée niveau 1)",
        "",
        f"Mécanisme identique au #114 (NDX) et #126 (S&P 500), seul le marché piloté change "
        f"(Russell 2000). {len(bh_t)} séances testables.",
        "",
        ...
        "|---|---|---|---|",
        f"| Buy&Hold (Russell 2000) | {me_bh['sharpe_ann']:+.2f} | {100*ret_bh:+.1f}% | {me_bh['max_drawdown_pct']:.1f}% |",
    ...
    if verdict:
        ...
        lines.append("**PASS niveau 1 seulement -- pas un verdict final (Règle 9). Doit encore passer "
                     "`nonml_pass_validation_battery.py yield_curve_slope_russell2000_vol_targeting_overlay` "
                     "(stress coûts/crise, stabilité temporelle, SPA, DSR à n_trials=backlog).**")

    out = ROOT / "results" / "nonml_yield_curve_slope_russell2000_vol_targeting_overlay_result.md"
    out.write_text("\n".join(lines) + "\n")

    if verdict:
        dates_used = dates_idx.values[1:][start:]
        np.savez(
            ROOT / "results" / "nonml_yield_curve_slope_russell2000_vol_targeting_overlay_pnl.npz",
