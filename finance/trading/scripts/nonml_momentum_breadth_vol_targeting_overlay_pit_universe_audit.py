...
from nonml_momentum_breadth_vol_targeting_overlay_pit_universe_backtest import (  # noqa: E402
    load_prices_pit, compute_momentum_breadth_series_pit, LOOKBACK, SKIP, COMPOSITION_START,
)
...
def independent_breadth_at(P: pd.DataFrame, tickers, date):
    ...
    for tk in tickers:
        if tk not in members:
            continue
        c_skip = P[tk].iloc[idx - SKIP]
        c_lookback = P[tk].iloc[idx - LOOKBACK]
...
def main():
    ...
    lines = ["# Audit adversarial — Porte breadth de momentum, univers point-in-time", "",
             "## 1. Recalcul indépendant de la breadth PIT à un échantillon de dates", "",
    ...
    out = ROOT / "results" / "nonml_momentum_breadth_vol_targeting_overlay_pit_universe_audit.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
