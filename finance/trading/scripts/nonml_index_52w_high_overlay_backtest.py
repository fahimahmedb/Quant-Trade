"""Backtest — Overlay levé proximité du plus haut 52-semaines, niveau
indice (spécification pré-enregistrée dans
PREREG_index_52w_high_overlay.md, committée avant ce script). n_trials=1,
...
def near_high_mask(close: np.ndarray, threshold: float = THRESHOLD) -> np.ndarray:
    rolling_max = pd.Series(close).rolling(LOOKBACK).max().values
    return close >= threshold * rolling_max


def main():
    lines = [
        "# Résultat — Overlay levé proximité plus haut 52-semaines, niveau indice (pré-enregistré, règle renforcée)",
        "",
        f"Position 1.0x en permanence, CAP={CAP}x quand clôture ≥ {100*THRESHOLD:.0f}% du plus haut "
        f"glissant {LOOKBACK}j, 1.0x sinon. Échantillon testable = à partir de la {LOOKBACK+1}e séance.",
        "",
    ...
    for name, fname in MARKETS.items():
        ...
        near_high = near_high_mask(close)
        ...
        if fname == "nasdaq100_daily.txt":
            np.savez(ROOT / "results" / "nonml_index_52w_high_overlay_pnl.npz",
                     pos=pos, r_asset=bh_full,
                     dates=pd.to_datetime(df["date"]).values[start:][1:],
                     cost_bps=COST_BPS)
    ...
    out = ROOT / "results" / "nonml_index_52w_high_overlay_result.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
