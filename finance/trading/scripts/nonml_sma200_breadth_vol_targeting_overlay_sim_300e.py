...
breadth SMA200 NDX-100, ~3 derniers mois. Spécification
...
from nonml_sma200_breadth_vol_targeting_overlay_backtest import (  # noqa: E402
    compute_sma200_breadth_series, combined_position, BREADTH_THRESHOLD, CAP, COST_BPS,
)
...
def main():
    ...
    lines = [
        "# Simulation — 300 EUR, overlay vol-targeting gaté par la breadth SMA200 (NDX, ~3 derniers mois)",
        "",
        ...
        f"| **Overlay breadth SMA200** | **{equity_ov[-1]:.2f} EUR** | "
    ...
    out = ROOT / "results" / "nonml_sma200_breadth_vol_targeting_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
