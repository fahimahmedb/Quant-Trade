"""Backtest — Indice OVX (volatilité implicite du marché pétrolier,
options sur l'ETF USO, via Yahoo Finance), overlay défensif
(spécification pré-enregistrée dans PREREG_oil_market_volatility_overlay.md,
...
à celle du #357 (MOVE) et du #341 (SKEW), niveau brut sans transformation
momentum.
"""
import sys
...
def load_ovx_series() -> pd.Series:
    raw = pd.read_csv(REPO_ROOT / "data" / "oil_vol_ovx_daily.csv")
    raw["observation_date"] = pd.to_datetime(raw["observation_date"])
    raw["OVX"] = pd.to_numeric(raw["OVX"], errors="coerce")
    s = raw.dropna(subset=["OVX"]).set_index("observation_date")["OVX"]
    s = s[~s.index.duplicated(keep="first")].sort_index()
    return s


def load_ovx_lag(dates: pd.DatetimeIndex, ovx_series: pd.Series) -> np.ndarray:
    y = ovx_series.reindex(dates, method="ffill")
...
def main():
    ...
    lines = [
        ...
        "",
        f"`position(t) = {CUT}x` si OVX_lag(t) est dans son tercile expanding le "
        f"plus HAUT (stress pétrolier implicite le plus élevé), `1.0x` sinon. "
    ...
    for name, fname in MARKETS.items():
        ...
        ovx_lag_full = load_ovx_lag(dates, ovx_series)
        ovx_lag = ovx_lag_full[1:]
        pos_full = expanding_tercile_cut_high(ovx_lag)
        ...
        if name == "NDX (40 ans)":
            ...
            np.savez(ROOT / "results" / "nonml_oil_market_volatility_overlay_pnl.npz",
    ...
    out = ROOT / "results" / "nonml_oil_market_volatility_overlay_result.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
    return verdict
