"""Simulation — 300 EUR dans l'overlay vol-targeting gaté par double
porte dispersion+tendance (NDX), ~3 derniers mois. Spécification
pré-enregistrée (CAP=2.0x, fenêtre=20j), aucun paramètre retouché après
...
from nonml_dispersion_trend_vol_targeting_overlay_backtest import (  # noqa: E402
    near_high_mask, combined_position, COST_BPS, CAP, VOL_WINDOW, TARGET_VOL_ANNUAL,
)
...
def main():
    ...
    trend_gate = near_high_mask(close_full)
    ...
    pos_full_all = combined_position(close_full, bh_full_all, gate_combined)
    ...
    lines = [
        ...
        f"CAP={CAP}x quand tendance 52w-high ET dispersion cross-sectionnelle sont simultanément actives.",
        "",
        "| | Capital final | Rendement période | MDD | Sharpe ann. |",
        "|---|---|---|---|---|",
        f"| BuyHold | {equity_bh[-1]:.2f} EUR | {100*(equity_bh[-1]/CAPITAL0-1):+.1f}% | "
        f"{mdd(equity_bh):.1f}% | {me_bh['sharpe_ann']:+.2f} |",
        f"| **Overlay vol-targeting double porte** | **{equity_ov[-1]:.2f} EUR** | "
        ...
        "(7/8 sur les grilles CAP 1.5x-3.0x et fenêtre 15-30j, seule la fenêtre 30j échoue)."
    ]

    out = ROOT / "results" / "nonml_dispersion_trend_vol_targeting_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
