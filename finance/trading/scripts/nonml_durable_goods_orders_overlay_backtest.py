"""Backtest — Nouvelles commandes de biens durables US (FRED DGORDER),
overlay défensif (spécification pré-enregistrée dans
PREREG_durable_goods_orders_overlay.md, committée avant ce script).
...
(commandes de biens durables au lieu de masse monétaire).
"""
import sys
...
def build_durable_goods_growth_series() -> pd.Series:
    ...
    # Les commandes de biens durables du mois M ne sont connaissables
    # qu'apres publication (~3-4 semaines apres la fin du mois, Census
    # Bureau) : decalage d'un mois calendaire complet AVANT le ffill
    # (meme convention conservatrice que le #195/#203).
...
def load_durable_goods_growth_lag(dates: pd.DatetimeIndex, dgo_series: pd.Series) -> np.ndarray:
    y = dgo_series.reindex(dates, method="ffill")
...
def main():
    ...
    lines = [
        ...
        "",
        f"`position(t) = {CUT}x` si DGOGrowth_lag(t-1)=log(DGORDER(t)/DGORDER(t-12)) est dans son tercile "
        f"expanding le plus BAS (contraction des commandes = défavorable), `1.0x` sinon. "
    ...
    for name, fname in MARKETS.items():
        ...
        dgo_lag_full = load_durable_goods_growth_lag(dates, dgo_series)
        dgo_lag = dgo_lag_full[1:]
        pos_full = expanding_tercile_cut_low(dgo_lag)
        ...
        if name == "NDX (40 ans)":
            ...
            np.savez(ROOT / "results" / "nonml_durable_goods_orders_overlay_pnl.npz",
    ...
    out = ROOT / "results" / "nonml_durable_goods_orders_overlay_result.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
    return verdict
