"""Robustesse — Overlay vol-targeting gaté par la concentration du
marché NDX-100. Grille CAP ET grille de fenêtre de vol (PAS un
retuning de la porte CONC_WINDOW=60/MEDIAN_WINDOW=252), autour des
...
from nonml_market_concentration_vol_targeting_overlay_backtest import (  # noqa: E402
    compute_concentration_series, MEDIAN_WINDOW, COST_BPS, TARGET_VOL_ANNUAL, ANNUALIZATION,
)
...
def main():
    ...
    lines = [
        "# Robustesse — Overlay vol-targeting gaté par la concentration du marché (grilles CAP et fenêtre, PAS un retuning de la porte)",
        "",
    ...
    out = ROOT / "results" / "nonml_market_concentration_vol_targeting_overlay_robustness.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
