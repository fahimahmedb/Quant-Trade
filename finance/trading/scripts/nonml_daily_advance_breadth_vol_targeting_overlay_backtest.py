"""Backtest — Overlay vol-targeting gaté par la breadth d'avance
journalière NDX-100 (spécification pré-enregistrée dans
PREREG_daily_advance_breadth_vol_targeting_overlay.md, committée avant
ce script). Breadth = fraction des titres NDX-100 en hausse, moyennée
sur une fenêtre TRÈS COURTE (5j, granularité quotidienne). n_trials=1,
...
def load_all_prices():
    ...
    for path in sorted(PRICES_DIR.glob("*.json")):
        ...
        if len(close) > ADV_WINDOW + 21:
            series[path.stem] = close
    return series


def compute_daily_advance_breadth_series() -> pd.Series:
    """Renvoie Breadth(t) = fraction des titres cotes en hausse
    aujourd'hui (close(t) > close(t-1)), moyennee sur une fenetre
    roulante ADV_WINDOW, indexee sur le calendrier UNION des tickers."""
    ...
    advance = np.full((T, n_tickers), np.nan)
    diff = close[1:] - close[:-1]
    valid = np.isfinite(close[1:]) & np.isfinite(close[:-1])
    advance[1:] = np.where(valid, diff > 0, np.nan)

    n_calc = np.isfinite(advance).sum(axis=1)
    n_up = np.where(np.isfinite(advance), advance, False).sum(axis=1)
    with np.errstate(divide="ignore", invalid="ignore"):
        daily_breadth = np.where(n_calc > 0, n_up / n_calc, np.nan)

    breadth_ma = pd.Series(daily_breadth).rolling(ADV_WINDOW).mean().values
    return pd.Series(breadth_ma, index=P.index)
...
def combined_position(close: np.ndarray, r: np.ndarray, gate_aligned: np.ndarray) -> np.ndarray:
    ...
    breadth d'avance alignee sur close (longueur T, booleenne). Renvoie
    la position pour chaque rendement de r."""
...
def main():
    ...
    breadth = compute_daily_advance_breadth_series()
    ...
    lines = [
        ...
        f"si Breadth d'avance moyenne {ADV_WINDOW}j(t) ≥{100*BREADTH_THRESHOLD:.0f}%, sinon 1.0x. "
        f"{len(bh_t)} séances testables.",
        "",
        f"%j porte breadth d'avance active : {100*gate_active.mean():.1f}%",
        f"Position moyenne : {pos.mean():.2f}x",
        f"Breadth d'avance moyenne (toute la période) : {100*np.nanmean(breadth.values):.1f}%",
        "",
        "| | Sharpe ann. | Rendement total net | MDD |",
        "|---|---|---|---|",
        f"| Buy&Hold (NDX) | {me_bh['sharpe_ann']:+.2f} | {100*ret_bh:+.1f}% | {me_bh['max_drawdown_pct']:.1f}% |",
        f"| **Overlay vol-targeting gaté breadth d'avance** | **{me_ov['sharpe_ann']:+.2f}** | "
    ...
    np.savez(ROOT / "results" / "nonml_daily_advance_breadth_vol_targeting_overlay_pnl.npz",
             pos=pos, r_asset=bh_t, dates=dates_idx.values[1:][start:], cost_bps=COST_BPS)

    out = ROOT / "results" / "nonml_daily_advance_breadth_vol_targeting_overlay_result.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
