"""Simulation — 300 EUR, overlay conditions financières NFCI
(cycle #289) sur les ~3 derniers mois disponibles (NDX). Aucun
paramètre retouché.
"""
import sys
...
from nonml_financial_conditions_overlay_backtest import (  # noqa: E402
    COST_BPS, build_nfci_series, load_nfci_lag, expanding_tercile_cut_high,
)
...
def main():
    ...
    lines = [
        f"# Simulation — 300 EUR, overlay conditions financières NFCI (cycle #289), {MARKET_NAME} (~3 derniers mois)",
        "",
        f"Période : {dates_w[0].date()} → {dates_w[-1].date()} ({len(r)} séances). "
        f"{100*frac_cut:.0f}% de la fenêtre est en régime coupé (conditions financières tendues).",
        "",
        "| | Capital final | Rendement période | MDD | Sharpe ann. |",
        "|---|---|---|---|---|",
        f"| Buy & Hold | {eq_bh[-1]:.2f} EUR | {100 * (eq_bh[-1] / CAPITAL0 - 1):+.1f}% | "
        f"{mdd_pct(eq_bh):.1f}% | {me_bh['sharpe_ann']:+.2f} |",
        f"| **Overlay conditions financières NFCI** | **{eq_ov[-1]:.2f} EUR** | "
    ...
    out = ROOT / "results" / "nonml_financial_conditions_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
