"""Backtest — Overlay vol-targeting gaté par la breadth NETTE (hauts − bas),
univers de titres POINT-IN-TIME (spécification pré-enregistrée dans
`PREREG_net_breadth_vol_targeting_overlay_pit_universe.md`, committée avant ce
script).

Réutilisation stricte (Règle 7) du cycle d'origine (#90) : **aucun paramètre
...
Particularité de ce candidat : la porte a un **seuil absolu** (breadth > 0), donc
elle n'est pas invariante par translation du signal — contrairement aux portes à
médiane glissante des #405 et #407.
"""
import json
import sys
...
def compute_net_breadth_series_pit():
    """Breadth nette(t) = (n_proche_haut − n_proche_bas) / n_cotes, calculee sur
    les seuls MEMBRES du NDX-100 a la date t.
    ...
    for i in range(T):
        ...
        if n_listed == 0:
            continue
        n_high = int((near_high[i] & listed).sum())
        n_low = int((near_low[i] & listed).sum())
        breadth[i] = (n_high - n_low) / n_listed
...
def main():
    ...
    breadth_net, cov = compute_net_breadth_series_pit()
    ...
    gate_aligned = np.where(np.isnan(breadth_raw), False, breadth_raw > GATE_THRESHOLD)
    ...
    lines = [
        ...
        "",
        "**Particularité** : la porte a un **seuil absolu** (breadth > 0), donc elle n'est "
        "pas invariante par translation du signal — contrairement aux portes à médiane "
        "glissante des #405 et #407.",
        "",
        ...
        f"Breadth nette moyenne : {100*np.nanmean(breadth_net.values):+.1f} pts",
        "",
        "| | Sharpe ann. | Rendement total net | MDD |",
        "|---|---|---|---|",
        f"| Buy&Hold (NDX) | {me_bh['sharpe_ann']:+.2f} | {100*ret_bh:+.1f}% | "
        f"{me_bh['max_drawdown_pct']:.1f}% |",
        f"| **Overlay gaté breadth nette (PIT)** | **{me_ov['sharpe_ann']:+.2f}** | "
    ...
    out = ROOT / "results" / "nonml_net_breadth_vol_targeting_overlay_pit_universe_result.md"
    out.write_text("\n".join(lines), encoding="utf-8")

    np.savez(
        ROOT / "results" / "nonml_net_breadth_vol_targeting_overlay_pit_universe_pnl.npz",
