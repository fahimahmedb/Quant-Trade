"""Robustesse — cycle #299 (porte majoritaire ≥2/3 défaut carte+NFCI+
BAA10Y), perturbation ±20% du plancher CUT=0.5x, seul paramètre non
central au critère de succès (le critère porte sur la règle de vote
≥2/3, pas sur la valeur exacte du plancher). Ce n'est PAS un retuning :
le verdict reste celui du point pré-enregistré 0.5x. Grille réutilisée
...
def main():
    ...
    lines = [
        "# Robustesse — cycle #299, perturbation ±20% du plancher CUT",
        "",
        f"Point pré-enregistré : CUT = {CUT}x. Grille : {{{', '.join(f'{c}x' for c in CUT_GRID)}}}. "
        "La règle de vote elle-même (≥2/3 signaux dans leur tercile expanding le plus haut) n'est "
    ...
    out = ROOT / "results" / "nonml_delinquency_nfci_baa10y_majority_overlay_robustness.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
