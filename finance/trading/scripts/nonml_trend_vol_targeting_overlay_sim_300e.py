"""Simulation — 300 EUR dans l'overlay combiné tendance + vol-targeting
...
from nonml_trend_vol_targeting_overlay_backtest import combined_position, COST_BPS, CAP, INDEX_LOOKBACK, INDEX_THRESHOLD, VOL_WINDOW, TARGET_VOL_ANNUAL  # noqa: E402
...
def main():
    ...
    lines = [
        "# Simulation — 300 EUR, overlay combiné tendance + vol-targeting (NDX, ~3 derniers mois)",
        "",
        ...
        f"seuil tendance {100*INDEX_THRESHOLD:.0f}%/{INDEX_LOOKBACK}j, CAP={CAP}x.",
        "",
        "| | Capital final | Rendement période | MDD | Sharpe ann. |",
        "|---|---|---|---|---|",
        f"| BuyHold | {equity_bh[-1]:.2f} EUR | {100*(equity_bh[-1]/CAPITAL0-1):+.1f}% | "
        f"{mdd(equity_bh):.1f}% | {me_bh['sharpe_ann']:+.2f} |",
        f"| **Overlay tendance+vol-targeting** | **{equity_ov[-1]:.2f} EUR** | "
        ...
        "grilles CAP 1.5x-3.0x et fenêtre de vol 15-30j)."
    ]

    out = ROOT / "results" / "nonml_trend_vol_targeting_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
