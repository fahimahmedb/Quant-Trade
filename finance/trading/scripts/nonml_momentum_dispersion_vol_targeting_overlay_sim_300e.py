...
dispersion du momentum NDX-100, ~3 derniers mois. Spécification
...
from nonml_momentum_dispersion_vol_targeting_overlay_backtest import (  # noqa: E402
    compute_momentum_dispersion_series, combined_position, MEDIAN_WINDOW, CAP, COST_BPS,
)
...
def main():
    ...
    lines = [
        "# Simulation — 300 EUR, overlay vol-targeting gaté par la dispersion du momentum (NDX, ~3 derniers mois)",
        "",
        ...
        f"| **Overlay dispersion momentum** | **{equity_ov[-1]:.2f} EUR** | "
    ...
    out = ROOT / "results" / "nonml_momentum_dispersion_vol_targeting_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
