"""Robustesse — Overlay vol-targeting gaté par dispersion cross-sectionnelle
NDX-100. Grille CAP ET grille de fenêtre de vol (PAS un retuning de la
porte MEDIAN_WINDOW=252/MIN_LISTED=10), autour des valeurs
...
from nonml_dispersion_vol_targeting_overlay_backtest import (  # noqa: E402
    compute_dispersion_series, MEDIAN_WINDOW, COST_BPS, TARGET_VOL_ANNUAL, ANNUALIZATION,
)
...
def position_for(r: np.ndarray, gate_aligned: np.ndarray, cap: float, vol_window: int) -> np.ndarray:
...
def run_one(r: np.ndarray, gate_aligned: np.ndarray, start: int, cap: float, vol_window: int) -> bool:
...
def main():
    ...
    lines = [
        "# Robustesse — Overlay vol-targeting gaté par dispersion cross-sectionnelle (grilles CAP et fenêtre, PAS un retuning de la porte)",
        "",
    ...
    out = ROOT / "results" / "nonml_dispersion_vol_targeting_overlay_robustness.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
