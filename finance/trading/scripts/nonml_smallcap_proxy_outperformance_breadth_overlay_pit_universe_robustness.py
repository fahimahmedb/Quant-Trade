...
(`nonml_smallcap_proxy_outperformance_breadth_overlay_robustness.py`) : CAP de
l'overlay et fenêtre de volatilité. Reprises telles quelles, elles sont donc
fixées avant toute exécution.

**Ce n'est PAS un retuning** : `IDIO_VOL_WINDOW` (60), `MOM_WINDOW` (21),
...
Usage : python3 scripts/nonml_smallcap_proxy_outperformance_breadth_overlay_pit_universe_robustness.py
"""
import sys
...
OUT = ROOT / "results" / "nonml_smallcap_proxy_outperformance_breadth_overlay_pit_universe_robustness.md"
...
def main():
    ...
    breadth, _cov = bt.compute_smallcap_breadth_series_pit()
    median_b = breadth.rolling(bt.MEDIAN_WINDOW).median()
    # meme masque explicite qu'au backtest (piege du #396)
    signal_defined = breadth.notna() & median_b.notna()
    gate_series = (breadth >= median_b).where(signal_defined)
    ...
    L = ["# Robustesse — breadth « petites caps » proxy, univers POINT-IN-TIME", ""]
    L.append("Grilles **identiques** à celles du cycle d'origine, donc fixées avant exécution.")
    L.append("**Pas un retuning** : `IDIO_VOL_WINDOW` (60), `MOM_WINDOW` (21),")
    L.append("`MEDIAN_WINDOW` (252) et le seuil médian définissent l'hypothèse et restent figés.")
    L.append("")
    ...
    L.append(f"**{n_cap}/{len(CAP_GRID)} cellules PASS sur la grille CAP, "
             ...
             f"sans réajustement.")
    L.append("")
    L.append("Pour mémoire, le cycle d'origine (univers biaisé par le survivant) obtient")
    L.append("**3/4** sur chacune des deux grilles après la correction du #404.")
    L.append("")
