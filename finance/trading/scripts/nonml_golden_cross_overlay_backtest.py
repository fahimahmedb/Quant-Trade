"""Backtest — Overlay levé "Golden Cross" SMA50/SMA200 (spécification
pré-enregistrée dans PREREG_golden_cross_overlay.md, committée avant ce
...
def main():
    lines = [
        "# Résultat — Overlay levé Golden Cross SMA50/SMA200 (pré-enregistré, règle renforcée)",
        "",
        f"Position 1.0x en permanence, CAP={CAP}x quand SMA{SMA_SHORT} > SMA{SMA_LONG}, 1.0x sinon. "
        f"Échantillon testable = à partir de la {SMA_LONG+1}e séance.",
        "",
    ...
    for name, fname in MARKETS.items():
        ...
        golden = golden_cross_mask(close)
        ...
        if fname == "nasdaq100_daily.txt":
            np.savez(ROOT / "results" / "nonml_golden_cross_overlay_pnl.npz",
    ...
    out = ROOT / "results" / "nonml_golden_cross_overlay_result.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
