"""Simulation — 300 EUR dans l'overlay pente courbe des taux US sur S&P
500, ~3 derniers mois. Spécification pré-enregistrée (CAP=2.0x), aucun
...
from nonml_yield_curve_slope_vol_targeting_overlay_backtest import (  # noqa: E402
    load_t10y2y, combined_position, MEDIAN_WINDOW, COST_BPS,
)
from nonml_yield_curve_slope_sp500_vol_targeting_overlay_backtest import MARKET_FILE  # noqa: E402
...
def main():
    ...
    lines = [
        ...
        f"| **Overlay pente courbe des taux** | **{equity_ov[-1]:.2f} EUR** | "
        ...
        "statistique réel n'est PAS encore établi à ce stade (Règle 9) : ce PASS niveau 1 a une "
        "robustesse FAIBLE (2/4 sur les deux grilles, pas un plateau) et un MDD dégradé par rapport à "
        "Buy&Hold sur l'historique complet — signaux d'alerte déjà notables avant même la batterie. "
        "Doit encore passer `nonml_pass_validation_battery.py "
        "yield_curve_slope_sp500_vol_targeting_overlay`."
    ]

    out = ROOT / "results" / "nonml_yield_curve_slope_sp500_vol_targeting_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
