"""Robustesse — cycle #357 (volatilité obligataire MOVE), perturbation
...
from nonml_financial_conditions_overlay_backtest import COST_BPS, CUT, MARKETS, TERCILE_PCT  # noqa: E402
...
def main():
    ...
    lines = [
        "# Robustesse — cycle #357, perturbation ±20% du plancher CUT",
        "",
    ...
    for name, fname in MARKETS.items():
        ...
        for cut in CUT_GRID:
            pos_full = expanding_tercile_cut_high(move_lag_full, cut)
    ...
    out = ROOT / "results" / "nonml_bond_market_volatility_overlay_robustness.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
