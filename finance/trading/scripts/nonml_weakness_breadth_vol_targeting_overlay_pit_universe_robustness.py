...
(`nonml_weakness_breadth_vol_targeting_overlay_robustness.py`) : CAP de
l'overlay et fenêtre de volatilité. Reprises telles quelles, elles sont donc
fixées avant toute exécution.

**Ce n'est PAS un retuning** : `INDEX_LOOKBACK` (252), les seuils de proximité (0,95 / 1,05)
...
Usage : python3 scripts/nonml_weakness_breadth_vol_targeting_overlay_pit_universe_robustness.py
"""
import sys
...
OUT = ROOT / "results" / "nonml_weakness_breadth_vol_targeting_overlay_pit_universe_robustness.md"
...
def main():
    ...
    # meme separation serie brute / porte qu'au backtest (piege du #396)
    ...
    L = ["# Robustesse — breadth de faiblesse, univers POINT-IN-TIME", ""]
    L.append("Grilles **identiques** à celles du cycle d'origine, donc fixées avant exécution.")
    L.append("**Pas un retuning** : `INDEX_LOOKBACK` (252), les seuils de proximité (0,95 / 1,05)")
    L.append("et le **seuil de porte (50 %)** définissent l'hypothèse et restent figés.")
    L.append("")
    L.append(f"CAP pré-enregistré = {bt.CAP}×, fenêtre de vol pré-enregistrée = {bt.VOL_WINDOW} j.")
    L.append("")
    L.append("**Lecture obligatoire avant les tableaux** : le verdict de ce candidat est")
    L.append("étiqueté **NON INFORMATIF** (porte brute active 0,45 % du temps), et l'audit a")
    L.append("établi que l'exposition ne dépasse jamais 1,0×. Toutes les cellules ci-dessous")
    L.append("sont donc mécaniquement identiques à Buy & Hold, quel que soit le CAP : faire")
    L.append("varier un plafond que la stratégie n'atteint jamais ne teste rien. Cette grille")
    L.append("est exécutée pour le montrer, pas pour valider un plateau.")
    L.append("")
    ...
    n_cap = 0
    for cap in CAP_GRID:
        start = max(first_valid, bt.INDEX_LOOKBACK, bt.VOL_WINDOW)
    ...
    for w in WINDOW_GRID:
        start = max(first_valid, bt.INDEX_LOOKBACK, w)
    ...
    L.append("`results/nonml_weakness_breadth_vol_targeting_overlay_robustness.md`.")
    L.append("")
