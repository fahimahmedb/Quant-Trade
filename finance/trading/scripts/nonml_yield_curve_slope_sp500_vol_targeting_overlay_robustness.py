"""Robustesse — Pente de la courbe des taux US appliquée au S&P 500.
Grille CAP ET grille de fenêtre de vol (PAS un retuning de
MEDIAN_WINDOW=252j), autour des valeurs pré-enregistrées.
"""
import sys
...
from nonml_yield_curve_slope_vol_targeting_overlay_backtest import (  # noqa: E402
    load_t10y2y, MEDIAN_WINDOW, COST_BPS, TARGET_VOL_ANNUAL, ANNUALIZATION,
)
...
def main():
    ...
    lines = [
        "# Robustesse — Pente courbe des taux US sur S&P 500 (grilles CAP et fenêtre, PAS un retuning de MEDIAN_WINDOW=252j)",
        "",
    ...
    out = ROOT / "results" / "nonml_yield_curve_slope_sp500_vol_targeting_overlay_robustness.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
