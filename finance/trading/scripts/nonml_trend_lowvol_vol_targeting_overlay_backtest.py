"""Backtest — Overlay vol-targeting gaté par double porte tendance ET
régime de vol faible (spécification pré-enregistrée dans
PREREG_trend_lowvol_vol_targeting_overlay.md, committée avant ce
script). Combine les cycles #47 (porte tendance) et #58 (porte vol
faible) en INTERSECTION. n_trials=1, aucune dépendance ML. Règle de
succès renforcée.
"""
import sys
...
def combined_position(close: np.ndarray, r: np.ndarray) -> np.ndarray:
    ...
    si tendance haussiere ET vol_lagged(t) < mediane glissante (double
    porte, intersection), 1.0 sinon."""
    trend = near_high_mask(close)  # longueur T, aligne comme #47
    trend_r = trend[:-1]  # meme convention causale que #47 (trend[:-1], pas trend[1:])
    ...
    lowvol_gate = vol_lagged < median_vol

    gate = trend_r & lowvol_gate
...
def main():
    lines = [
        ...
        f"si tendance haussière (≥{100*INDEX_THRESHOLD:.0f}% du plus haut {INDEX_LOOKBACK}j) ET vol "
        f"réalisée sous sa médiane glissante {MEDIAN_WINDOW}j, sinon 1.0x.",
        "",
    ...
    for name, fname in MARKETS.items():
        ...
        start = INDEX_LOOKBACK + MEDIAN_WINDOW
        bh_t = bh_full[start:]
        pos = pos_full[start:]
        ...
        if fname == "nasdaq100_daily.txt":
            np.savez(ROOT / "results" / "nonml_trend_lowvol_vol_targeting_overlay_pnl.npz",
    ...
    out = ROOT / "results" / "nonml_trend_lowvol_vol_targeting_overlay_result.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
