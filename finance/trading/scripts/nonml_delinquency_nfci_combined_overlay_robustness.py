"""Robustesse — cycle #296 (porte combinée ET défaut carte + NFCI),
perturbation ±20% du plancher CUT=0.5x, seul paramètre non central au
critère de succès (le critère porte sur la porte ET tercile expanding,
...
telle quelle du #200/#284/#289 (Règle 7).
"""
import sys
...
def main():
    ...
    lines = [
        "# Robustesse — cycle #296, perturbation ±20% du plancher CUT",
        "",
        f"Point pré-enregistré : CUT = {CUT}x. Grille : {{{', '.join(f'{c}x' for c in CUT_GRID)}}}. "
        "La porte ET elle-même (tercile expanding le plus haut des deux composantes) n'est pas "
        "perturbée — seul le plancher de position l'est.",
        "",
    ...
    for name, fname in MARKETS.items():
        ...
        gate_combined = expanding_tercile_gate_high(delinq_lag) & expanding_tercile_gate_high(nfci_lag)
    ...
    out = ROOT / "results" / "nonml_delinquency_nfci_combined_overlay_robustness.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
