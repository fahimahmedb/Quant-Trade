"""Audit adversarial — Panel élargi à 5 signaux (défaut carte + NFCI +
BAA10Y + corrélation NDX-DAX + MOVE), vote majoritaire ≥4/5.
...
2. Vérifie les relations d'inclusion attendues : la porte ET à 5 (les
   5 votes) est un SOUS-ENSEMBLE de la porte majoritaire (≥4/5), qui
   est elle-même un SOUS-ENSEMBLE de l'union à 5 (au moins 1 vote).
...
def independent_gate_at(level: np.ndarray, t: int) -> bool:
    ...
    meme logique que l'audit du #282/#296/#298/#299/#301/#304. Utilise le
...
def main():
    ...
    lines = ["# Audit adversarial — Panel élargi à 5 signaux (défaut carte + NFCI + BAA10Y + corrélation NDX-DAX + MOVE), vote ≥4/5", "",
             ...
             "à chaque pas serait prohibitif — même logique d'échantillonnage que l'audit du #282/#296/#298/#299/#301/#304).",
             "",
    ...
    for name, fname in MARKETS.items():
        ...
        votes = (gate_delinq.astype(int) + gate_nfci.astype(int)
                 ...
                 + gate_move.astype(int))
        gate_majority = votes >= 4
        gate_and5 = votes == 5
        gate_or5 = votes >= 1

        subset_and = bool(np.all(~gate_and5[start:] | gate_majority[start:]))
        subset_maj_or = bool(np.all(~gate_majority[start:] | gate_or5[start:]))
        ...
        lines.append(f"| {name} | {100*gate_majority[start:].mean():.1f}% | "
                     f"{100*gate_and5[start:].mean():.1f}% | {100*gate_or5[start:].mean():.1f}% | "
                     f"{'OUI' if subset_and else 'NON'} | {'OUI' if subset_maj_or else 'NON'} | "
                     f"{len(sample_idx)} | {n_diff} |")

    lines.append("")
    lines.append(f"**{'OK — vote majoritaire confirmé cohérent avec les relations ET(5)⊆majorité⊆OU(5), recalcul indépendant identique sur l’échantillon (0 désaccord).' if all_ok else 'ÉCHEC — incohérence détectée.'}**")
    ...
    out = ROOT / "results" / "nonml_delinquency_nfci_baa10y_corr_move_majority_overlay_audit.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
