...
(`nonml_net_breadth_vol_targeting_overlay_robustness.py`) : CAP de
...
Usage : python3 scripts/nonml_net_breadth_vol_targeting_overlay_pit_universe_robustness.py
"""
import sys
...
OUT = ROOT / "results" / "nonml_net_breadth_vol_targeting_overlay_pit_universe_robustness.md"
...
def main():
    ...
    breadth_net, _cov = bt.compute_net_breadth_series_pit()
    ...
    gate_aligned = np.where(np.isnan(breadth_raw), False, breadth_raw > bt.GATE_THRESHOLD)
    first_valid = int(np.argmax(~np.isnan(breadth_raw)))

    L = ["# Robustesse — breadth nette hauts-bas, univers POINT-IN-TIME", ""]
    ...
    L.append("et le **seuil de porte (0,0)** définissent l'hypothèse et restent figés.")
    L.append("")
    ...
    L.append("`results/nonml_net_breadth_vol_targeting_overlay_robustness.md`.")
    L.append("")
