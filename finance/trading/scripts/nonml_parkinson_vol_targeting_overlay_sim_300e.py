"""Simulation — 300 EUR dans l'overlay vol-targeting estimateur Parkinson
...
from nonml_parkinson_vol_targeting_overlay_backtest import parkinson_vol_position, COST_BPS, CAP, VOL_WINDOW, TARGET_VOL_ANNUAL  # noqa: E402
...
def main():
    ...
    pos_full_all = parkinson_vol_position(df)
    ...
    lines = [
        "# Simulation — 300 EUR, overlay vol-targeting estimateur Parkinson (NDX, ~3 derniers mois)",
        "",
        ...
        f"| **Overlay vol-targeting Parkinson** | **{equity_ov[-1]:.2f} EUR** | "
    ...
    out = ROOT / "results" / "nonml_parkinson_vol_targeting_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
