"""Robustesse — Overlay vol-targeting gaté par le January Barometer.
Grille CAP ET grille de fenêtre de vol (PAS un retuning de la porte
annuelle), autour des valeurs pré-enregistrées (CAP=2.0x, fenêtre=20j).
"""
import sys
...
from nonml_january_barometer_vol_targeting_overlay_backtest import (  # noqa: E402
    january_gate_mask, COST_BPS, TARGET_VOL_ANNUAL, ANNUALIZATION, MARKETS,
)
...
def position_for(r: np.ndarray, gate: np.ndarray, cap: float, vol_window: int) -> np.ndarray:
    gate_r = gate[:-1]
...
def run_one(r: np.ndarray, gate: np.ndarray, cap: float, vol_window: int) -> bool:
    pos_full = position_for(r, gate, cap, vol_window)
    start = vol_window
    r_t = r[start:]
    pos = pos_full[start:]
...
def main():
    ...
    lines = [
        "# Robustesse — Overlay vol-targeting gaté par January Barometer (grilles CAP et fenêtre, PAS un retuning de la porte)",
        "",
    ...
    out = ROOT / "results" / "nonml_january_barometer_vol_targeting_overlay_robustness.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
