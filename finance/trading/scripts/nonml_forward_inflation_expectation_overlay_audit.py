"""Audit indépendant — Anticipation d'inflation à long terme (5 ans dans
5 ans, T5YIFR).

Recalcule l'alignement causal et le tercile expanding par une méthode
...
le décalage causal d'un jour sur une transition réelle et l'absence de
fuite (troncature de l'historique).
"""
import sys
...
from nonml_forward_inflation_expectation_overlay_backtest import (  # noqa: E402
    CUT, MARKETS, load_t5yifr_lag,
)
from nonml_inflation_breakeven_overlay_backtest import expanding_tercile_cut_high  # noqa: E402


def manual_t5yifr_lag(dates: pd.DatetimeIndex) -> np.ndarray:
    raw = pd.read_csv(REPO_ROOT / "data" / "t5yifr_daily.csv")
    raw["observation_date"] = pd.to_datetime(raw["observation_date"])
    raw["T5YIFR"] = pd.to_numeric(raw["T5YIFR"], errors="coerce")
    obs = raw.dropna(subset=["T5YIFR"]).sort_values("observation_date")
    obs_dates = obs["observation_date"].values
    vals = obs["T5YIFR"].astype(float).values
...
def main():
    lines = ["# Audit — Anticipation d'inflation à long terme (5 ans dans 5 ans, T5YIFR)", ""]
    all_ok = True
    ...
    for name, fname in MARKETS.items():
        ...
        t5yifr_official = load_t5yifr_lag(dates)[1:]
        pos_official = expanding_tercile_cut_high(t5yifr_official)

        t5yifr_manual = manual_t5yifr_lag(dates)[1:]
        diff_align = np.nanmax(np.abs(t5yifr_official - t5yifr_manual))

        start = int(np.argmax(np.isfinite(t5yifr_manual)))
        T = len(t5yifr_manual)
        pos_manual = np.full(T, np.nan)
        for t in range(start, T):
            if not np.isfinite(t5yifr_manual[t]):
                continue
            hist = t5yifr_manual[start:t + 1]
            hist = hist[np.isfinite(hist)]
            thresh = manual_percentile_high_tercile(hist)
            pos_manual[t] = CUT if t5yifr_manual[t] >= thresh else 1.0
        ...
        for i in np.where(mask)[0][pos_diff_mask]:
            local_diff = abs(t5yifr_official[i] - t5yifr_manual[i])
            if local_diff >= 1e-10:
        ...
        if n_pos_diff == 0:
            ...
            lines.append(f"- **{n_pos_diff}/{n_total} désaccords de position** — sensibilité de bord flottante "
                         f"documentée (même pattern que #193/#195/.../#342) : "
                         f"{'confirmé' if boundary_tie_ok else 'NON confirmé, écart réel'}.")
        lines.append(f"- **{'OK' if ok else 'ÉCART DÉTECTÉ'}**")
        lines.append("")

    # Verification specifique du decalage causal d'un jour, 5 dates
    # CONSECUTIVES se terminant sur la derniere transition de valeur
    # reelle (evite l'artefact de bord positionnel de shift(1) deja
    # documente au #320/#321/#340/#341).
    raw = pd.read_csv(REPO_ROOT / "data" / "t5yifr_daily.csv")
    raw["observation_date"] = pd.to_datetime(raw["observation_date"])
    raw["T5YIFR"] = pd.to_numeric(raw["T5YIFR"], errors="coerce")
    raw = raw.dropna(subset=["T5YIFR"]).sort_values("observation_date")
    vals = raw["T5YIFR"].astype(float).values
    ...
    t5yifr_check = load_t5yifr_lag(check_dates)

    lines.append(f"## Vérification spécifique du décalage causal d'un jour (transition {d_prev.date()}→{d_last.date()})")
    lines.append(f"- T5YIFR brut jour précédent ({d_prev.date()}) = {v_prev:.6f}, "
                 f"T5YIFR brut jour de la transition ({d_last.date()}) = {v_last:.6f} "
                 f"(valeurs distinctes vérifiées : {'OUI' if not np.isclose(v_prev, v_last) else 'NON — transition aveugle, coïncidence à éviter'})")
    lines.append(f"- T5YIFR_lag({d_last.date()}) = {t5yifr_check[4]:.6f} (doit valoir {v_prev:.6f}, JAMAIS {v_last:.6f})")
    lines.append(f"- T5YIFR_lag({(d_last + pd.Timedelta(days=1)).date()}) = {t5yifr_check[5]:.6f} "
                 f"(doit être le premier jour où {v_last:.6f} apparaît, via shift(1))")
    ok_shift = (not np.isclose(v_prev, v_last)) and np.isclose(t5yifr_check[4], v_prev) and \
        np.isclose(t5yifr_check[5], v_last)
    all_ok &= ok_shift
    ...
    dates = pd.DatetimeIndex(df["date"].values)
    t5yifr_lag_full = load_t5yifr_lag(dates)[1:]
    valid_start = int(np.argmax(np.isfinite(t5yifr_lag_full)))
    T_CUT = valid_start + 2000
    pos_full = expanding_tercile_cut_high(t5yifr_lag_full)
    pos_truncated = expanding_tercile_cut_high(t5yifr_lag_full[:T_CUT])
    ...
    out = ROOT / "results" / "nonml_forward_inflation_expectation_overlay_audit.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
    return all_ok
