"""Robustesse — Overlay vol-targeting gaté par la position moyenne dans
le range annuel NDX-100. Grille CAP ET grille de fenêtre de vol (PAS un
retuning de la porte RANGE_LOOKBACK=252/MEDIAN_WINDOW=252), autour des
...
from nonml_range_position_vol_targeting_overlay_backtest import (  # noqa: E402
    compute_range_position_series, MEDIAN_WINDOW, COST_BPS, TARGET_VOL_ANNUAL, ANNUALIZATION,
)
...
def main():
    ...
    lines = [
        "# Robustesse — Overlay vol-targeting gaté par la position moyenne dans le range annuel (grilles CAP et fenêtre, PAS un retuning de la porte)",
        "",
    ...
    out = ROOT / "results" / "nonml_range_position_vol_targeting_overlay_robustness.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
