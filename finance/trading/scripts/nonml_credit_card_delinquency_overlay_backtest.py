"""Backtest — Taux de défaut cartes de crédit US (DRCCLACBS), overlay
défensif (spécification pré-enregistrée dans
PREREG_credit_card_delinquency_overlay.md, committée avant ce script).
n_trials=1, aucune dépendance ML. Règle de succès renforcée sur 5
marchés (≥4/5).

Réutilise CUT=0,5, le NIVEAU brut (pas une croissance, comme le spread
de crédit BAA10Y #199) et la technique de tercile expanding déjà
établies aux #169/#177/#183/#191/#192/#193/#195/#196/#197/#198/#199,
Règle 7. Décalage de publication d'un TRIMESTRE (extension
proportionnelle du décalage d'un mois des séries mensuelles #195/#203/
#204/#205/#206/#283 à une fréquence trimestrielle).
"""
import sys
...
def build_delinquency_series() -> pd.Series:
    ...
    raw = raw.dropna(subset=["DRCCLACBS"]).drop_duplicates("observation_date").sort_values("observation_date")
    vals = raw["DRCCLACBS"].astype(float).values
...
def main():
    ...
    lines = [
        ...
        "",
        f"`position(t) = {CUT}x` si DRCCLACBS_lag(t-1) est dans son tercile expanding le plus HAUT, "
    ...
    for name, fname in MARKETS.items():
        ...
        if name == "NDX (40 ans)":
            ...
            np.savez(ROOT / "results" / "nonml_credit_card_delinquency_overlay_pnl.npz",
    ...
    out = ROOT / "results" / "nonml_credit_card_delinquency_overlay_result.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
    return verdict
