...
Yang-Zhang (NDX), ~3 derniers mois. Spécification pré-enregistrée
...
from nonml_yang_zhang_vol_targeting_overlay_backtest import yang_zhang_vol_position, COST_BPS, CAP, VOL_WINDOW, TARGET_VOL_ANNUAL  # noqa: E402
...
def main():
    ...
    pos_full_all = yang_zhang_vol_position(df)
    ...
    lines = [
        "# Simulation — 300 EUR, overlay vol-targeting estimateur Yang-Zhang (NDX, ~3 derniers mois)",
        "",
        ...
        f"| **Overlay vol-targeting Yang-Zhang** | **{equity_ov[-1]:.2f} EUR** | "
        ...
        "du backtest complet (PASS 5/5 marchés) et de la robustesse (plateau correct mais pas "
        "parfait, dip à 3/5 marchés à fenêtre=15j)."
    ]

    out = ROOT / "results" / "nonml_yang_zhang_vol_targeting_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
