...
variance de Garman-Klass elle-même à partir de open/high/low/close,
indépendamment de data_loader.garman_klass_var_pct) et test
anti-lookahead.
"""
import sys
...
from nonml_garman_klass_vol_targeting_overlay_backtest import (  # noqa: E402
    garman_klass_vol_position, VOL_WINDOW, TARGET_VOL_ANNUAL, CAP, ANNUALIZATION, MARKETS,
)


def independent_gk_position(df) -> np.ndarray:
    """Recalcul totalement independant : variance de Garman-Klass calculee
    directement depuis open/high/low/close (pas via
    data_loader.garman_klass_var_pct), puis moyenne roulante et position
    par boucle explicite."""
    o = df["open"].values
    high = df["high"].values
    low = df["low"].values
    ...
    hl = np.log(high / low)
    co = np.log(close / o)
    v = 0.5 * (100.0 * hl) ** 2 - (2.0 * np.log(2.0) - 1.0) * (100.0 * co) ** 2  # %^2
    v = v[1:]  # aligne comme garman_klass_var_pct (drop 1re obs)
...
def main():
    lines = ["# Audit adversarial — Overlay de vol-targeting estimateur Garman-Klass", "",
    ...
    for name, fname in MARKETS.items():
        ...
        quality_report(df)
        pos_orig = garman_klass_vol_position(df)
        pos_indep = independent_gk_position(df)
        start = VOL_WINDOW + 1
    ...
    for name, fname in MARKETS.items():
        ...
        df = load_ohlc(str(path))
        quality_report(df)
        pos_before = garman_klass_vol_position(df)
        ...
        pos_after = garman_klass_vol_position(df_pert)
    ...
    out = ROOT / "results" / "nonml_garman_klass_vol_targeting_overlay_audit.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
