"""Robustesse — Porte breadth SMA200, univers point-in-time (cycle
#271). Grille CAP et grille de fenêtre de vol (PAS un retuning de la
porte BREADTH_THRESHOLD=50%/SMA_WINDOW=200), grille réutilisée telle
quelle du #96 original.
"""
import sys
...
from nonml_sma200_breadth_vol_targeting_overlay_pit_universe_backtest import (  # noqa: E402
    compute_sma200_breadth_series_pit, BREADTH_THRESHOLD, COST_BPS, TARGET_VOL_ANNUAL,
    ANNUALIZATION, SMA_WINDOW,
)
...
def main():
    ...
    lines = [
        "# Robustesse — Porte breadth SMA200, univers point-in-time (grilles CAP et fenêtre, PAS un retuning de la porte)",
        "",
        "Grille réutilisée telle quelle du #96 original. CAP pré-enregistré = 2.0x, "
    ...
    results = []
    for cap in CAP_GRID:
        start = max(first_valid, SMA_WINDOW, 20)
    ...
    for window in WINDOW_GRID:
        start = max(first_valid, SMA_WINDOW, window)
    ...
    out = ROOT / "results" / "nonml_sma200_breadth_vol_targeting_overlay_pit_universe_robustness.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
