"""Batterie de validation renforcée (Règle 9) — #266 (momentum de
constance, univers point-in-time), ADAPTÉE au format PORTEFEUILLE
multi-actifs (spécification pré-enregistrée dans
`PREREG_momentum_consistency_pit_universe_pass_validation_battery.md`,
committée avant ce script).

Réutilise STRICTEMENT (Règle 7) les fonctions de contrôle a-e déjà
écrites et validées au #259 -- génériques sur des paires (rendement
brut, turnover) candidat/référence, aucune modification. Seule la
reconstruction des séries change (momentum de constance univers PIT vs
Buy&Hold équipondéré univers PIT).
"""
import sys
...
def build_raw_series():
    ...
    start = first_rebal
    raw_cons = (weights_cons[start:] * R_safe[start:]).sum(axis=1)
    ...
    dates = P.index[start:]
    return raw_cons, turn_cons, raw_bh, turn_bh, dates


def main():
    name = "momentum_consistency_pit_universe"
    raw_cons, turn_cons, raw_bh, turn_bh, dates = build_raw_series()
    cost_bps = COST_BPS

    lines = [f"# Batterie de validation renforcée — {name} (adaptée au format portefeuille)",
             "",
             f"Coût pré-enregistré : {cost_bps:.1f} bps. {len(raw_cons)} séances. "
             "Candidat = momentum de constance, univers point-in-time (#266). "
    ...
    lines.append("|---|---|---|---|---|---|")
    ok_a, rows_a = check_a_cost_stress(raw_cons, turn_cons, raw_bh, turn_bh, cost_bps)
    ...
    lines.append("|---|---|---|---|---|")
    ok_b, rows_b, any_window = check_b_crisis_stress(raw_cons, turn_cons, raw_bh, turn_bh, dates, cost_bps)
    ...
    lines.append("|---|---|---|---|---|")
    ok_c, rows_c, n_beat, n_scored = check_c_temporal_stability(raw_cons, turn_cons, raw_bh, turn_bh, cost_bps)
    ...
    lines.append("")
    ok_d, p_spa = check_d_spa(raw_cons, turn_cons, raw_bh, turn_bh, cost_bps)
    ...
    lines.append("")
    e_res = check_e_dsr(raw_cons, turn_cons, cost_bps)
    if e_res is None:
        ...
        ok_e = d >= 0.95
        lines.append(f"n_trials = {n_trials} (taille du backlog après le #268), "
    ...
    out = ROOT / "results" / "nonml_momentum_consistency_pit_universe_pass_validation_battery.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
