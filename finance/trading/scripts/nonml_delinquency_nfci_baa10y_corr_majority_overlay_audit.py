"""Audit adversarial — Panel élargi à 4 signaux (défaut carte + NFCI +
BAA10Y + corrélation NDX-DAX), vote majoritaire ≥3/4.
...
2. Vérifie les relations d'inclusion attendues : la porte ET à 4 (les
   4 votes) est un SOUS-ENSEMBLE de la porte majoritaire (≥3/4), qui
   est elle-même un SOUS-ENSEMBLE de l'union à 4 (au moins 1 vote).
...
def independent_gate_at(level: np.ndarray, t: int) -> bool:
    ...
    meme logique que l'audit du #282/#296/#298/#299/#301. Utilise le
...
def main():
    ...
    lines = ["# Audit adversarial — Panel élargi à 4 signaux (défaut carte + NFCI + BAA10Y + corrélation NDX-DAX), vote ≥3/4", "",
             ...
             "à chaque pas serait prohibitif — même logique d'échantillonnage que l'audit du #282/#296/#298/#299/#301).",
             "",
    ...
    for name, fname in MARKETS.items():
        ...
        gate_majority = votes >= 3
        gate_and4 = votes == 4
        gate_or4 = votes >= 1

        subset_and = bool(np.all(~gate_and4[start:] | gate_majority[start:]))
        subset_maj_or = bool(np.all(~gate_majority[start:] | gate_or4[start:]))
        ...
        lines.append(f"| {name} | {100*gate_majority[start:].mean():.1f}% | "
                     f"{100*gate_and4[start:].mean():.1f}% | {100*gate_or4[start:].mean():.1f}% | "
                     f"{'OUI' if subset_and else 'NON'} | {'OUI' if subset_maj_or else 'NON'} | "
                     f"{len(sample_idx)} | {n_diff} |")

    lines.append("")
    lines.append(f"**{'OK — vote majoritaire confirmé cohérent avec les relations ET(4)⊆majorité⊆OU(4), recalcul indépendant identique sur l’échantillon (0 désaccord).' if all_ok else 'ÉCHEC — incohérence détectée.'}**")
    ...
    gate_full = votes_full >= 3

    N_CHECK = 2000
    TRUNC_POINTS = [4000, 5500, 6400]
    all_trunc_ok = True
    ...
    out = ROOT / "results" / "nonml_delinquency_nfci_baa10y_corr_majority_overlay_audit.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
