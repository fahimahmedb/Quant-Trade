"""Robustesse — Overlay vol-targeting gaté par le calendrier. Grille CAP
ET grille de fenêtre de vol (PAS un retuning de la cible de vol ni des
fenêtres calendaires), autour des valeurs pré-enregistrées (CAP=2.0x,
fenêtre vol=20j).
"""
import sys
...
from nonml_calendar_vol_targeting_overlay_backtest import (  # noqa: E402
    tom_mask, halloween_mask, COST_BPS, TARGET_VOL_ANNUAL, ANNUALIZATION, MARKETS,
)
...
def position_for(close: np.ndarray, dates, cap: float, vol_window: int) -> np.ndarray:
    ...
    return np.where(calendar, vt, 1.0)
...
def main():
    ...
    lines = [
        "# Robustesse — Overlay vol-targeting gaté par le calendrier (grilles CAP et fenêtre, PAS un retuning)",
        "",
    ...
    out = ROOT / "results" / "nonml_calendar_vol_targeting_overlay_robustness.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
