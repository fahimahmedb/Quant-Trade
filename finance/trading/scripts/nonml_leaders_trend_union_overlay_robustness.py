"""Robustesse — Leaders 52-semaines + overlay union SMA200∪52w-high.
Grille CAP (PAS un retuning), autour du CAP pré-enregistré (2.0x).

NOTE (voir audit) : le signal 52w-high est un sous-ensemble strict du
signal SMA200 sur cette donnée -- cette grille reproduit donc
mathématiquement celle déjà obtenue au cycle #33 (Leaders + SMA200
seul). Exécutée quand même pour la traçabilité complète du protocole.
"""
import sys
...
from nonml_leaders_trend_union_overlay_backtest import (  # noqa: E402
    load_prices, index_trend_union_series, LOOKBACK, REBAL_EVERY, TERCILE, COST_BPS,
)
...
def main():
    ...
    lines = [
        "# Robustesse — Leaders + overlay union SMA200∪52w-high (grille CAP, PAS un retuning)",
        "",
        "CAP pré-enregistré = 2.0x. **Note** : reproduit mathématiquement la grille du #33 "
        "(52w-high ⊆ SMA200 sur cette donnée, voir audit).",
        "",
    ...
    out = ROOT / "results" / "nonml_leaders_trend_union_overlay_robustness.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
