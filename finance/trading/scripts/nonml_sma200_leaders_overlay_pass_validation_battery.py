"""Batterie de validation renforcée (Règle 9) — #33 (SMA200 + portefeuille
Leaders 52-semaines), ADAPTÉE au format PORTEFEUILLE multi-actifs
(spécification pré-enregistrée dans
`PREREG_sma200_leaders_overlay_pass_validation_battery.md`, committée
avant ce script).

Réutilisation stricte (Règle 7) du patron déjà établi au #259 (batterie
du #258, même problème structurel résolu) : les 5 fonctions de contrôle
sont importées directement (génériques, ne dépendent que des tableaux
raw/turn/dates/coût).
"""
import sys
...
from nonml_sma200_leaders_overlay_backtest import (  # noqa: E402
    load_prices, index_trend_series, lag_one_day,
    LOOKBACK, REBAL_EVERY, COST_BPS, TERCILE, CAP,
)


def build_raw_series():
    """Reconstruit (rendement brut, turnover, dates) pour le candidat
    (Leaders + overlay SMA200) et la reference (Leaders seul), avant
    application des couts -- identique en structure a build_raw_series()
    du #259, mais avec la porte SMA200 du #33 au lieu du double-tri
    turnover."""
    series = load_prices()
...
def main():
    ...
    lines = [f"# Batterie de validation renforcée — {name} (adaptée au format portefeuille)",
             ...
             "Candidat = Leaders 52-semaines + overlay SMA200 (#33). "
    ...
    if e_res is None:
        ...
        ok_e = d >= 0.95
        lines.append(f"n_trials = {n_trials} (taille du backlog après le #33), "
    ...
    out = ROOT / "results" / "nonml_sma200_leaders_overlay_pass_validation_battery.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
