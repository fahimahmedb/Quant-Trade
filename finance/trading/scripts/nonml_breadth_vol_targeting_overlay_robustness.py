"""Robustesse — Overlay vol-targeting gaté par breadth NDX+Russell2000.
Grille CAP ET grille de fenêtre de vol (PAS un retuning du seuil de
breadth 95%/252j, laissé fixe comme au #52), autour des valeurs
...
from nonml_breadth_vol_targeting_overlay_backtest import (  # noqa: E402
    breadth_gate, COST_BPS, INDEX_LOOKBACK, TARGET_VOL_ANNUAL, ANNUALIZATION,
)
...
def position_for(close: np.ndarray, r: np.ndarray, gate: np.ndarray, cap: float, vol_window: int) -> np.ndarray:
    gate_r = gate[:-1]
...
def run_one(close: np.ndarray, r: np.ndarray, gate: np.ndarray, cap: float, vol_window: int) -> bool:
    pos_full = position_for(close, r, gate, cap, vol_window)
...
def main():
    ...
    lines = [
        ...
        "",
        "CAP pré-enregistré = 2.0x, fenêtre pré-enregistrée = 20j. Seuil breadth 95%/252j "
        "laissé fixe (identique au #52, non retesté ici).",
        "",
    ...
    for cap in CAP_GRID:
        ok = run_one(close, r, gate, cap, 20)
    ...
    for window in WINDOW_GRID:
        ok = run_one(close, r, gate, 2.0, window)
        marker = " ← fenêtre pré-enregistrée" if window == 20 else ""
        lines.append(f"| {window}j | {'OUI' if ok else 'non'}{marker} |")

    out = ROOT / "results" / "nonml_breadth_vol_targeting_overlay_robustness.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
