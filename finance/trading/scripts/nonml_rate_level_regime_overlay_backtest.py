"""Backtest — Régime de NIVEAU des taux courts DGS3MO, overlay coupé/levé
(spécification pré-enregistrée dans PREREG_rate_level_regime_overlay.md,
committée avant ce script). n_trials=1, aucune dépendance ML. Règle de
succès renforcée sur 5 marchés (≥4/5).

Réutilise l'alignement causal DGS3MO (ffill + shift(1)) déjà validé aux
Règle 10 des #142/#166 (`load_short_rate`), Règle 7.
"""
import sys
...
COST_BPS = 5.0
N = 63  # ~1 trimestre, fenêtre standard macro du backlog
CUT = 0.5
CAP = 2.0
...
def load_rate_lag(dates: pd.DatetimeIndex) -> np.ndarray:
    ...
    meme convention que la Regle 10 des #142/#166."""
...
def main():
    lines = [
        "# Résultat — Régime de niveau des taux courts DGS3MO (pré-enregistré)",
        "",
    ...
    out = ROOT / "results" / "nonml_rate_level_regime_overlay_result.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
    return verdict
