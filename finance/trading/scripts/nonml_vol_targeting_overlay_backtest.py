"""Backtest — Overlay de vol-targeting continu (spécification
pré-enregistrée dans PREREG_vol_targeting_overlay.md, committée avant ce
script). n_trials=1, aucune dépendance ML (vol réalisée simple, pas de
GARCH). Règle de succès renforcée.
"""
import sys
...
def vol_target_position(r: np.ndarray) -> np.ndarray:
    ...
    pos = np.nan_to_num(pos, nan=1.0)  # avant que la vol roulante soit definie -> neutre (non teste, hors echantillon)
    return pos
...
def main():
    ...
    out = ROOT / "results" / "nonml_vol_targeting_overlay_result.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
