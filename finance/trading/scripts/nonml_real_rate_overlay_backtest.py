"""Backtest — Taux réel TIPS 10 ans (DFII10), overlay défensif
(spécification pré-enregistrée dans PREREG_real_rate_overlay.md,
...
#175/#178/#186/#187/#191/#192/#193/#195/#196/#197/#198/#199/#200, CUT=0,5 et
la technique de tercile expanding déjà établie aux #169/#177/#183/#191/
#192/#193/#195/#196/#197/#198/#199/#200, Règle 7.
"""
import sys
...
def load_dfii10_lag(dates: pd.DatetimeIndex) -> np.ndarray:
    raw = pd.read_csv(REPO_ROOT / "data" / "dfii10_daily.csv")
    raw["observation_date"] = pd.to_datetime(raw["observation_date"])
    s = raw.set_index("observation_date")["DFII10"].astype(float).dropna()
...
def main():
    lines = [
        ...
        "",
        f"`position(t) = {CUT}x` si DFII10_lag(t) est dans son tercile expanding le plus haut, "
    ...
    for name, fname in MARKETS.items():
        ...
        dfii10_lag = load_dfii10_lag(dates)[1:]
        pos_full = expanding_tercile_cut_high(dfii10_lag)
    ...
    out = ROOT / "results" / "nonml_real_rate_overlay_result.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
    return verdict
