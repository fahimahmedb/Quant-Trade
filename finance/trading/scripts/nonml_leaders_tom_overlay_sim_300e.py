"""Simulation — 300 EUR, Leaders 52-semaines + overlay ToM levé, ~3
...
from nonml_leaders_tom_overlay_backtest import (  # noqa: E402
    load_prices, tom_mask, LOOKBACK, REBAL_EVERY, TERCILE, COST_BPS, CAP,
)
...
def main():
    ...
    lines = [
        "# Simulation — 300 EUR, Leaders 52-semaines + overlay ToM levé (~3 derniers mois)",
        "",
        ...
        f"| **Leaders + overlay ToM x{CAP}** | **{equity_lev[-1]:.2f} EUR** | "
    ...
    out = ROOT / "results" / "nonml_leaders_tom_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
