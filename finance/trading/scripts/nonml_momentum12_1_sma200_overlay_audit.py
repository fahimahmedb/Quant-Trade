"""Audit adversarial — Momentum 12-1 + overlay SMA200.
...
from nonml_momentum12_1_sma200_overlay_backtest import (  # noqa: E402
    load_prices, index_trend_series, LOOKBACK, SKIP, REBAL_EVERY, TERCILE, CAP, SMA_WINDOW,
)
...
def main():
    ...
    weights_lev = weights_mom * exposure_target[:, None]

    start = LOOKBACK
    total_exposure = weights_lev[start:].sum(axis=1)
    expected = exposure_target[start:]
    has_position = weights_mom[start:].sum(axis=1) > 1e-9
    ...
    out = ROOT / "results" / "nonml_momentum12_1_sma200_overlay_audit.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
