"""Robustesse — cycle #301 (position graduée par nombre de votes),
perturbation ±20% du plancher CUT=0.5x (le plancher atteint à 3/3
votes), seul paramètre non central au critère de succès (le critère
porte sur la fonction linéaire elle-même, pas sur la valeur exacte du
plancher). Ce n'est PAS un retuning : le verdict reste celui du point
pré-enregistré 0.5x. Grille réutilisée telle quelle du #200/#284/#289/
#296/#299 (Règle 7).
"""
import sys
...
def main():
    ...
    lines = [
        "# Robustesse — cycle #301, perturbation ±20% du plancher CUT",
        "",
        f"Point pré-enregistré : CUT = {CUT}x (position = 1,0 - CUT×votes/3). "
        f"Grille : {{{', '.join(f'{c}x' for c in CUT_GRID)}}}. "
        "La fonction linéaire elle-même (sizing continu par nombre de votes) n'est pas "
        "perturbée — seule la pente/le plancher à 3 votes l'est.",
        "",
    ...
    for name, fname in MARKETS.items():
        ...
        for cut in CUT_GRID:
            pos = 1.0 - cut * (votes_t.astype(float) / 3.0)
    ...
    out = ROOT / "results" / "nonml_delinquency_nfci_baa10y_graduated_overlay_robustness.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
