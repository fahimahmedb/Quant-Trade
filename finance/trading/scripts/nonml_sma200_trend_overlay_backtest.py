"""Backtest — Overlay levé filtre de tendance SMA200 (spécification
pré-enregistrée dans PREREG_sma200_trend_overlay.md, committée avant ce
...
def above_sma_mask(close: np.ndarray) -> np.ndarray:
...
def main():
    lines = [
        "# Résultat — Overlay levé filtre de tendance SMA200 (pré-enregistré, règle renforcée)",
        "",
        f"Position 1.0x en permanence, CAP={CAP}x quand la clôture est au-dessus de sa SMA{SMA_WINDOW}j, 1.0x sinon. "
        f"Échantillon testable = à partir de la {SMA_WINDOW+1}e séance.",
        "",
    ...
    for name, fname in MARKETS.items():
        ...
        start = SMA_WINDOW  # premiere seance avec SMA200 valide
        close_t = close[start:]
        bh_full = np.log(close_t[1:] / close_t[:-1])
        mask = above[start:-1]  # decision a t (SMA connue a t), appliquee au rendement t->t+1
        ...
        if name == "NDX (40 ans)":
            # Sauvegarde ajoutee retroactivement (cycle #312, batterie Regle 9) pour
            ...
            np.savez(ROOT / "results" / "nonml_sma200_trend_overlay_pnl.npz",
    ...
    out = ROOT / "results" / "nonml_sma200_trend_overlay_result.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
