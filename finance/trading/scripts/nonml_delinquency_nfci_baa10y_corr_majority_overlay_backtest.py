"""Backtest — Panel élargi à 4 signaux (défaut carte #286, NFCI #291,
BAA10Y #199, corrélation cross-marché NDX-DAX #193), vote majoritaire
≥3/4, overlay défensif (spécification pré-enregistrée dans
PREREG_delinquency_nfci_baa10y_corr_majority_overlay.md, committée
...
build_corr_series/load_corr_lag du #193, expanding_tercile_gate_high
du #296 (générique, étendue ici à une 4e série sans modification).
"""
import sys
...
def main():
    ...
    lines = [
        ...
        "",
        f"`position(t) = {CUT}x` si AU MOINS 3 des 4 signaux sont dans leur tercile expanding "
    ...
    for name, fname in MARKETS.items():
        ...
        valid_t = valid[start:]
        bh_t = bh_full[start:]
        assert valid_t.all(), "les quatre series doivent etre valides sur toute la fenetre testable"
        ...
        if name == "NDX (40 ans)":
            ...
            np.savez(ROOT / "results" / "nonml_delinquency_nfci_baa10y_corr_majority_overlay_pnl.npz",
    ...
    out = ROOT / "results" / "nonml_delinquency_nfci_baa10y_corr_majority_overlay_result.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
    return verdict
