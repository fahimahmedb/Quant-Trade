"""Simulation — 300 EUR, Low-Vol Tilt + overlay proximité plus haut
52-semaines indice, ~3 derniers mois. Spécification pré-enregistrée
(CAP=2.0x, seuil=95%), aucun paramètre retouché après les résultats
précédents.
"""
import sys
...
from nonml_lowvol_index52w_high_overlay_backtest import (  # noqa: E402
    load_prices, index_trend_series, VOL_WINDOW, REBAL_EVERY, TERCILE, COST_BPS, CAP,
)
...
def main():
    ...
    pnl_base_full = (weights_lowvol * R).sum(axis=1)
    turn_base = np.abs(np.diff(weights_lowvol, axis=0, prepend=weights_lowvol[:1])).sum(axis=1) / 2.0
    ...
    lines = [
        "# Simulation — 300 EUR, Low-Vol Tilt + overlay proximité plus haut 52-semaines indice (~3 derniers mois)",
        "",
        f"Période : {dates[0].date()} → {dates[-1].date()} ({len(pnl_base)} séances). "
        "Référence = Low-Vol 1.0x (cycle #15), pas Buy&Hold classique.",
        "",
        "| | Capital final | Rendement période | MDD | Sharpe ann. |",
        "|---|---|---|---|---|",
        f"| Low-Vol 1.0x (référence) | {equity_base[-1]:.2f} EUR | "
        f"{100*(equity_base[-1]/CAPITAL0-1):+.1f}% | {mdd(equity_base):.1f}% | {me_base['sharpe_ann']:+.2f} |",
        f"| **Low-Vol + overlay 52w-high indice x{CAP}** | **{equity_lev[-1]:.2f} EUR** | "
        ...
        "robustesse (plateau stable sur les deux grilles CAP 1.5x-3.0x et seuil 90-97%)."
    ]

    out = ROOT / "results" / "nonml_lowvol_index52w_high_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
