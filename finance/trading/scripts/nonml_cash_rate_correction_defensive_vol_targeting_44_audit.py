...
def main():
    ...
    pos_full = vol_target_position(r_ndx_full)
    ...
    lines = ["# Audit adversarial — Correction taux réaliste appliquée au #44 (#149)", "",
             "## 1. Recalcul indépendant de la position équity (formule fermée indépendante)", "",
             "| Indice séance | Original | Indépendant | Concorde |",
             "|---|---|---|---|"]
    check_idx = list(range(VOL_WINDOW + 10, len(r_ndx_full), max(1, len(r_ndx_full) // 8)))
    all_ok = True
    for t in check_idx:
        indep = independent_pos_eq_at(r_ndx_full, t, VOL_WINDOW)
        if indep is None:
            continue
    ...
    lines.append("")
    lines.append("Mécanisme obligataire strictement identique au #134/#136/#137/#139/#141/#146 (déjà "
                  "audité, 0 fuite détectée). Position équity recalculée indépendamment ci-dessus "
                  "(formule identique au #115/#134, déjà auditée à l'origine sous ce même schéma). "
                  "Pas de nouvelle surface de fuite introduite par cette correction.")

    out = ROOT / "results" / "nonml_cash_rate_correction_defensive_vol_targeting_44_audit.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
