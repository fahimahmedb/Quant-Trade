"""Robustesse — Overlay vol-targeting gaté par la breadth SMA200
NDX-100. Grille CAP ET grille de fenêtre de vol (PAS un retuning de la
porte BREADTH_THRESHOLD=50%/SMA_WINDOW=200), autour des valeurs
...
from nonml_sma200_breadth_vol_targeting_overlay_backtest import (  # noqa: E402
    compute_sma200_breadth_series, BREADTH_THRESHOLD, COST_BPS, TARGET_VOL_ANNUAL,
...
def main():
    ...
    lines = [
        "# Robustesse — Overlay vol-targeting gaté par la breadth SMA200 (grilles CAP et fenêtre, PAS un retuning de la porte)",
        "",
    ...
    out = ROOT / "results" / "nonml_sma200_breadth_vol_targeting_overlay_robustness.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
