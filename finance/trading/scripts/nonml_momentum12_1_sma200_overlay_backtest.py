"""Backtest — Momentum 12-1 + overlay levé filtre de tendance SMA200
(spécification pré-enregistrée dans
PREREG_momentum12_1_sma200_overlay.md, committée avant ce script).
...
def main(causal=True):
    ...
    lines = [
        "# Résultat — Momentum 12-1 + overlay levé filtre de tendance SMA200 (pré-enregistré, combinaison #73+#29)",
        "",
        f"Référence = portefeuille momentum 12-1 1.0x (cycle #73), PAS Buy&Hold. "
        ...
        "|---|---|---|---|",
        f"| Momentum 12-1 1.0x (référence, cycle #73) | {me_base['sharpe_ann']:+.2f} | {100*ret_base:+.1f}% | "
        f"{me_base['max_drawdown_pct']:.1f}% |",
        f"| **Momentum 12-1 + overlay SMA200 x{CAP}** | **{me_lev['sharpe_ann']:+.2f}** | "
    ...
    out = ROOT / "results" / "nonml_momentum12_1_sma200_overlay_result.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
