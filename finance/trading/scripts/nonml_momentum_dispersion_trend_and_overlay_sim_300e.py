"""Simulation — 300 EUR dans l'overlay double porte AND dispersion
momentum + tendance 52w-high (NDX), ~3 derniers mois. Spécification
...
from nonml_momentum_dispersion_trend_and_overlay_backtest import combined_position, CAP, COST_BPS  # noqa: E402
...
def main():
    ...
    pos_full_all = combined_position(bh_full_all, gate_combined)
    ...
    lines = [
        "# Simulation — 300 EUR, double porte AND dispersion momentum + tendance 52w-high (NDX, ~3 derniers mois)",
        "",
        ...
        f"| **Overlay AND dispersion∩tendance** | **{equity_ov[-1]:.2f} EUR** | "
        ...
        "statistique réel reste celui du backtest complet (PASS) et de la robustesse (plateau "
        "parfait 8/8 sur les grilles CAP et fenêtre de vol)."
    ]

    out = ROOT / "results" / "nonml_momentum_dispersion_trend_and_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
