"""Backtest — Overlay vol-targeting gaté par la dispersion du momentum
NDX-100 (spécification pré-enregistrée dans
PREREG_momentum_dispersion_vol_targeting_overlay.md, committée avant
ce script). n_trials=1, aucune dépendance ML. Règle de succès
renforcée. Cycle #100 du backlog.

NOTE (leçon du cycle #77) : la dispersion du momentum n'est disponible
que depuis le début de l'historique des titres NDX-100 (~2021 +
LOOKBACK=252j) -- l'échantillon testable est restreint à la période où
...
def compute_momentum_dispersion_series() -> pd.Series:
    """Renvoie Dispersion(t) = ecart-type cross-sectionnel (ddof=1) des
    scores de momentum 12-1 mois calculables au jour t (NaN si
    ...
    n_elig = np.isfinite(momentum).sum(axis=1)
    with np.errstate(invalid="ignore"):
        dispersion = np.nanstd(momentum, axis=1, ddof=1)
    dispersion = np.where(n_elig >= MIN_LISTED, dispersion, np.nan)
...
def combined_position(close: np.ndarray, r: np.ndarray, gate_aligned: np.ndarray) -> np.ndarray:
    ...
    dispersion momentum alignee sur close (longueur T, booleenne).
    Renvoie la position pour chaque rendement de r."""
    gate_r = gate_aligned[:-1]  # meme convention causale que #47/#57/#78/#89/#90/#94/#96/#97/#98/#99
...
def main():
    ...
    lines = [
        ...
        f"si Dispersion du momentum(t) ≥ sa médiane glissante {MEDIAN_WINDOW}j, sinon 1.0x. "
        f"{len(bh_t)} séances testables (échantillon restreint à la période où le signal est disponible).",
        "",
        f"%j porte dispersion momentum active : {100*gate_active.mean():.1f}%",
        f"Position moyenne : {pos.mean():.2f}x",
        f"Dispersion du momentum moyenne observée : {np.nanmean(dispersion.values):.3f}",
        "",
        "| | Sharpe ann. | Rendement total net | MDD |",
        "|---|---|---|---|",
        f"| Buy&Hold (NDX) | {me_bh['sharpe_ann']:+.2f} | {100*ret_bh:+.1f}% | {me_bh['max_drawdown_pct']:.1f}% |",
        f"| **Overlay vol-targeting gaté dispersion momentum** | **{me_ov['sharpe_ann']:+.2f}** | "
    ...
    out = ROOT / "results" / "nonml_momentum_dispersion_vol_targeting_overlay_result.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")

    # Sauvegarde ajoutee retroactivement (cycle #318, batterie Regle 9) pour
    ...
    np.savez(ROOT / "results" / "nonml_momentum_dispersion_vol_targeting_overlay_pnl.npz",
