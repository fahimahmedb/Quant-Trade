"""Backtest — Overlay combiné tendance + vol-targeting (spécification
pré-enregistrée dans PREREG_trend_vol_targeting_overlay.md, committée
avant ce script). Combine les cycles #37 et #46. n_trials=1, aucune
...
def combined_position(close: np.ndarray, r: np.ndarray) -> np.ndarray:
    ...
    vol_realisee(t-1), 1.0, CAP) si tendance haussiere, 1.0 sinon."""
    trend = near_high_mask(close)  # aligne sur close (longueur T)
    # r[i] = log(close[i+1]/close[i]) = rendement du jour i au jour i+1 ;
    # la decision doit utiliser trend[i] (connu a la cloture du jour i,
    # jour de decision), donc trend_r[i] = trend[i] -> trend[:-1], PAS
    # trend[1:] (qui utiliserait trend[i+1], connu seulement a la cloture
    # du jour i+1 -> fuite d'un jour, bug trouve et corrige avant tout
    # calcul committe).
    trend_r = trend[:-1]
    ...
    pos = np.where(trend_r, vt_exposure, 1.0)
    return pos
...
def main():
    lines = [
        ...
        f"si tendance haussière (≥{100*INDEX_THRESHOLD:.0f}% du plus haut {INDEX_LOOKBACK}j), sinon 1.0x. "
        f"Échantillon testable = à partir de la {INDEX_LOOKBACK+1}e séance.",
        "",
    ...
    for name, fname in MARKETS.items():
        ...
        if name == "NDX (40 ans)":
            ...
            np.savez(ROOT / "results" / "nonml_trend_vol_targeting_overlay_pnl.npz",
    ...
    out = ROOT / "results" / "nonml_trend_vol_targeting_overlay_result.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
