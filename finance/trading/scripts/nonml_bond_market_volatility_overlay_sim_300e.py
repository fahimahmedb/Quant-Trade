"""Simulation — 300 EUR, overlay volatilité obligataire MOVE
(cycle #357) sur les ~3 derniers mois disponibles (NDX). Aucun
paramètre retouché.
"""
import sys
...
from nonml_financial_conditions_overlay_backtest import COST_BPS, expanding_tercile_cut_high  # noqa: E402
...
def main():
    ...
    pos_full = expanding_tercile_cut_high(move_lag_full)
    dates_r = dates[1:]
    ...
    lines = [
        f"# Simulation — 300 EUR, overlay volatilité obligataire MOVE (cycle #357), {MARKET_NAME} (~3 derniers mois)",
        "",
        f"Période : {dates_w[0].date()} → {dates_w[-1].date()} ({len(r)} séances). "
        f"{100*frac_cut:.0f}% de la fenêtre est en régime coupé (stress obligataire élevé).",
        "",
        "| | Capital final | Rendement période | MDD | Sharpe ann. |",
        "|---|---|---|---|---|",
        f"| Buy & Hold | {eq_bh[-1]:.2f} EUR | {100 * (eq_bh[-1] / CAPITAL0 - 1):+.1f}% | "
        f"{mdd_pct(eq_bh):.1f}% | {me_bh['sharpe_ann']:+.2f} |",
        f"| **Overlay volatilité obligataire MOVE** | **{eq_ov[-1]:.2f} EUR** | "
        f"**{100 * (eq_ov[-1] / CAPITAL0 - 1):+.1f}%** | {mdd_pct(eq_ov):.1f}% | "
        f"{me_ov['sharpe_ann']:+.2f} |",
        "",
        "**Note** : la série `^MOVE` étant figée (ffill) depuis le 17/07/2026 (cf. note "
        "qualité des données du résultat principal), une partie de cette fenêtre de 63 "
        "séances utilise une valeur MOVE constante — illustratif uniquement, sans impact "
        "sur le verdict.",
        "",
    ...
    out = ROOT / "results" / "nonml_bond_market_volatility_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
