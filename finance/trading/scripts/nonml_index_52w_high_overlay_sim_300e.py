"""Simulation — 300 EUR dans l'overlay proximité plus haut 52-semaines
(NDX), ~3 derniers mois. Spécification pré-enregistrée (CAP=2.0x,
seuil=95%), aucun paramètre retouché après les résultats précédents.
"""
import sys
...
from nonml_index_52w_high_overlay_backtest import near_high_mask, COST_BPS, CAP, LOOKBACK, THRESHOLD  # noqa: E402
...
def main():
    ...
    near_high_full = near_high_mask(close_full)

    close = close_full[-(WINDOW_DAYS + 1):]
    near_high = near_high_full[-(WINDOW_DAYS + 1):]
    ...
    lines = [
        "# Simulation — 300 EUR, overlay proximité plus haut 52-semaines (NDX, ~3 derniers mois)",
        "",
        f"Période : {pd.Timestamp(dates[1]).date()} → {pd.Timestamp(dates[-1]).date()} "
        f"({len(bh_full)} séances). CAP={CAP}x si clôture ≥ {100*THRESHOLD:.0f}% du plus haut "
        f"glissant {LOOKBACK}j, 1.0x sinon.",
        "",
        "| | Capital final | Rendement période | MDD | Sharpe ann. |",
        "|---|---|---|---|---|",
        f"| BuyHold | {equity_bh[-1]:.2f} EUR | {100*(equity_bh[-1]/CAPITAL0-1):+.1f}% | "
        f"{mdd(equity_bh):.1f}% | {me_bh['sharpe_ann']:+.2f} |",
        f"| **Overlay proximité plus haut 52s** | **{equity_ov[-1]:.2f} EUR** | "
        ...
        "reste celui du backtest complet (PASS 5/5 marchés) et de la robustesse (plateau parfait "
        "5/5 sur CAP 1.5x-3.0x, robuste sur le seuil 90-95% mais sensible à 97%, signalé "
        "honnêtement)."
    ]

    out = ROOT / "results" / "nonml_index_52w_high_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
