"""Backtest — Panel élargi à 5 signaux (défaut carte #286, NFCI #291,
BAA10Y #199, corrélation cross-marché NDX-DAX #193, volatilité
implicite obligataire MOVE #357), vote majoritaire ≥4/5, overlay
défensif (spécification pré-enregistrée dans
PREREG_delinquency_nfci_baa10y_corr_move_majority_overlay.md, committée
...
du #357, expanding_tercile_gate_high du #296 (générique, étendue ici à
une 5e série sans modification).
"""
import sys
...
def main():
    ...
    lines = [
        "# Résultat — Panel élargi à 5 signaux (défaut carte #286 + NFCI #291 + BAA10Y #199 + "
        "corrélation NDX-DAX #193 + MOVE #357), vote ≥4/5, overlay défensif (pré-enregistré)",
        "",
        f"`position(t) = {CUT}x` si AU MOINS 4 des 5 signaux sont dans leur tercile expanding "
    ...
    for name, fname in MARKETS.items():
        ...
        valid_t = valid[start:]
        bh_t = bh_full[start:]
        assert valid_t.all(), "les cinq series doivent etre valides sur toute la fenetre testable"
        ...
        if name == "NDX (40 ans)":
            ...
            np.savez(ROOT / "results" / "nonml_delinquency_nfci_baa10y_corr_move_majority_overlay_pnl.npz",
    ...
    out = ROOT / "results" / "nonml_delinquency_nfci_baa10y_corr_move_majority_overlay_result.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
    return verdict
