...
Cinq contrôles indépendants du backtest :
...
2. **Recalcul indépendant de la sélection Leaders.** Le backtest calcule le ratio
   `close / plus-haut-252j` par une boucle NumPy sur fenêtres glissantes ; l'audit
   le recalcule par `pandas.rolling(...).max()`. Chemins de code disjoints, poids
   attendus identiques.

3. **Anti-lookahead par perturbation du futur.** Les prix après une coupure sont
   altérés ; les poids AVANT doivent être strictement inchangés.

4. **Respect de l'appartenance point-in-time** à la date de DÉCISION (une
   sélection avant l'entrée dans l'indice serait une fuite ; une détention après
   une sortie ne l'est pas — les deux sont comptées séparément).

5. **Causalité du décalage.** Le poids appliqué au rendement du jour t doit avoir
   été décidé en t−1.

Usage : python3 scripts/nonml_leaders_trend_union_overlay_pit_universe_audit.py
"""
import sys
...
import nonml_leaders_trend_union_overlay_pit_universe_backtest as bt  # noqa: E402
from ndx100_membership import tickers_as_of_date  # noqa: E402

OUT = ROOT / "results" / "nonml_leaders_trend_union_overlay_pit_universe_audit.md"
...
def rebuild_weights_pandas():
    ...
    252 observations valides dans la fenetre -- equivalent logique du masque
    `has_full` du backtest, mais code entierement disjoint.
    """
    ...
    roll_max = P.rolling(bt.LOOKBACK).max()
    ratio = (P / roll_max).values
...
def main():
    L = ["# Audit adversarial — Leaders + overlay union de tendance, univers point-in-time", ""]
    ...
    start_eff = max(bt.LOOKBACK, first)
    ...
    if not ok1:
        ...
        L.append("ici parce que la fenêtre est longue (11,6 ans, 138 rebalancements) et le")
        L.append("panier dispersé (tercile de momentum) : la formule rééquilibre implicitement")
        L.append("tous les jours, la détention réelle laisse les poids dériver 21 jours. Sur")
        L.append("cette durée l'écart de niveau atteint ~0,5 %/an et le franchit.")
        L.append("")

    # --- 1b. diagnostic : le verdict tient-il par le chemin en parts ? ---
    # Le controle 1 signale un ecart de NIVEAU. Question distincte : cet ecart
    # peut-il retourner le VERDICT ? Le P&L levier vaut exactement
    # exposure[t-1] * P&L base (l'exposition est un scalaire par date), on peut
    # donc rejouer les deux jambes a partir des rendements du chemin en parts.
    # Ce diagnostic ne modifie PAS le verdict pre-enregistre, quel qu'en soit le
    # resultat : le protocole fige la formule d'agregation du backtest.
    from prediction import trading_metrics  # noqa: E402

    trend = bt.index_trend_union_series()
    trend_al = trend.reindex(P_bt.index, method="ffill").fillna(False).values.astype(bool)
    exposure = np.where(trend_al, bt.CAP, 1.0)
    W_base_l = W_lag
    W_lev_l = bt.lag_one_day(W_bt * exposure[:, None])
    turn_b = np.abs(np.diff(W_base_l[start_eff:], axis=0,
                            prepend=W_base_l[start_eff:start_eff + 1])).sum(axis=1) / 2.0
    turn_l = np.abs(np.diff(W_lev_l[start_eff:], axis=0,
                            prepend=W_lev_l[start_eff:start_eff + 1])).sum(axis=1) / 2.0
    ...
    L.append("résultat : le protocole fige la formule d'agrégation du backtest. Il est écrit")
    L.append("ici pour que la limite soit connue, pas pour rouvrir la décision.")
    L.append("")
    L.append("| Chemin en parts | Sharpe ann. | Rendement total net |")
    L.append("|---|---|---|")
    L.append(f"| Leaders 1.0x | {me_b_sh['sharpe_ann']:+.2f} | {100*ret_b_sh:+.1f}% |")
    L.append(f"| Leaders + overlay {bt.CAP}x | {me_l_sh['sharpe_ann']:+.2f} | {100*ret_l_sh:+.1f}% |")
    L.append("")
    ...
    # --- 4. respect de l'appartenance PIT ---
    ...
    L.append("")
    L.append(f"**{'CONFORME — les cinq contrôles passent.' if verdict else 'RÉSERVE — le contrôle 1 dépasse sa tolérance de 5 %.'}**")
    L.append("")
    if verdict:
        L.append("Le **FAIL** du backtest est donc un résultat calculé correctement, pas un")
        L.append("artefact d'implémentation.")
    else:
        L.append("Les contrôles 2 à 5 passent : la sélection est reproductible par un chemin")
        L.append("de code disjoint, il n'y a aucune fuite du futur, aucune sélection hors")
        L.append("appartenance, et le décalage causal est exact. La seule réserve porte sur")
        L.append("l'écart de niveau du contrôle 1, dont le diagnostic 1b montre "
                 + ("qu'il ne retourne pas le verdict." if not (sh_ok and rt_ok)
