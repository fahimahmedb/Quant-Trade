"""Audit indépendant — Momentum du Bitcoin (CBBTCUSD, log-return 21j).

Recalcule BTCmom_lag(t), le tercile expanding et la position par une
...
from nonml_bitcoin_momentum_overlay_backtest import (  # noqa: E402
    CUT, load_btc_mom_lag, load_btc_series, expanding_tercile_cut_low,
)


def manual_btc_mom_lag(dates: pd.DatetimeIndex) -> np.ndarray:
    """Recalcul par boucle explicite + searchsorted (side='right'-1,
    inclusif : le prix BTC connu au plus tard a la date d), sans
    pandas.reindex/ffill."""
    raw = pd.read_csv(REPO_ROOT / "data" / "bitcoin_daily.csv")
    raw["observation_date"] = pd.to_datetime(raw["observation_date"])
    raw["CBBTCUSD"] = pd.to_numeric(raw["CBBTCUSD"], errors="coerce")
    raw = raw.dropna(subset=["CBBTCUSD"]).drop_duplicates("observation_date").sort_values("observation_date")
    obs_dates = raw["observation_date"].values
    vals = raw["CBBTCUSD"].astype(float).values
...
def main():
    lines = ["# Audit — Momentum du Bitcoin (CBBTCUSD, log-return 21j)", ""]
    all_ok = True
    ...
    for name, fname in MARKETS.items():
        ...
        mom_official = load_btc_mom_lag(dates, btc_series)[1:]
        pos_official = expanding_tercile_cut_low(mom_official)

        mom_manual = manual_btc_mom_lag(dates)[1:]
        ...
        if n_pos_diff == 0:
            ...
            lines.append(f"- **{n_pos_diff}/{n_total} désaccords de position** — sensibilité de bord flottante "
                         f"documentée (même pattern que #193/#195/.../#343) : "
    ...
    # reelle du momentum brut (avant le shift(1) externe) -- evite
    # l'artefact de bord positionnel de shift(1) deja documente au
    # #320/#321/#340/#341/#342.
    name, fname = "NDX (40 ans)", MARKETS["NDX (40 ans)"]
    df = load_ohlc(str(REPO_ROOT / "data" / fname))
    dates = pd.DatetimeIndex(df["date"].values)
    btc_aligned = btc_series.reindex(dates, method="ffill").values
    mom_raw = np.full(len(dates), np.nan)
    mom_raw[RET_WINDOW:] = np.log(btc_aligned[RET_WINDOW:] / btc_aligned[:-RET_WINDOW])
    diffs = np.diff(mom_raw[np.isfinite(mom_raw)])
    ...
    mom_lag_full = load_btc_mom_lag(dates, btc_series)
    ...
    lines.append(f"- BTCmom brut séance précédente ({d_prev.date()}) = {m_prev:.6f}, "
                 f"BTCmom brut séance de la transition ({d_last.date()}) = {m_last:.6f} "
                 f"(valeurs distinctes vérifiées : {'OUI' if not np.isclose(m_prev, m_last) else 'NON — transition aveugle, coïncidence à éviter'})")
    lines.append(f"- BTCmom_lag séance {i_last} ({d_last.date()}) = {mom_lag_full[i_last]:.6f} (doit valoir {m_prev:.6f}, JAMAIS {m_last:.6f})")
    if i_last + 1 < len(dates):
        lines.append(f"- BTCmom_lag séance {i_last+1} ({dates[i_last+1].date()}) = {mom_lag_full[i_last+1]:.6f} "
    ...
    mom_full = load_btc_mom_lag(dates, btc_series)[1:]
    ...
    out = ROOT / "results" / "nonml_bitcoin_momentum_overlay_audit.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
    return all_ok
