"""Simulation — 300 EUR dans l'overlay vol-targeting gaté par la pente
SMA200 (NDX), ~3 derniers mois. Spécification pré-enregistrée
...
from nonml_slope_vol_targeting_overlay_backtest import combined_position, COST_BPS, CAP, VOL_WINDOW, TARGET_VOL_ANNUAL  # noqa: E402
...
def main():
    ...
    pos_full_all = combined_position(close_full, bh_full_all)
    ...
    lines = [
        "# Simulation — 300 EUR, overlay vol-targeting gaté par pente SMA200 (NDX, ~3 derniers mois)",
        "",
        ...
        f"CAP={CAP}x quand la pente de la SMA200 est positive.",
        "",
        "| | Capital final | Rendement période | MDD | Sharpe ann. |",
        "|---|---|---|---|---|",
        f"| BuyHold | {equity_bh[-1]:.2f} EUR | {100*(equity_bh[-1]/CAPITAL0-1):+.1f}% | "
        f"{mdd(equity_bh):.1f}% | {me_bh['sharpe_ann']:+.2f} |",
        f"| **Overlay vol-targeting pente SMA200** | **{equity_ov[-1]:.2f} EUR** | "
    ...
    out = ROOT / "results" / "nonml_slope_vol_targeting_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
