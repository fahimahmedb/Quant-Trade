"""Audit indépendant — Inflation réalisée US (FRED CPIAUCSL).

Recalcule CPIGrowth_lag(t), l'alignement causal (avec décalage d'un
...
spécifiquement le décalage d'un mois (sur une transition réelle) et
l'absence de fuite générale (troncature de l'historique). Résultat
PASS net (5/5) : audit particulièrement rigoureux étant donné l'enjeu.
"""
import sys
...
from nonml_m2_growth_overlay_backtest import CUT, YOY_MONTHS, MARKETS  # noqa: E402
from nonml_financial_conditions_overlay_backtest import expanding_tercile_cut_high  # noqa: E402
from nonml_cpi_inflation_overlay_backtest import (  # noqa: E402
    PUBLICATION_LAG_MONTHS, build_cpi_growth_series, load_cpi_growth_lag,
)


def manual_cpi_growth_lag(dates: pd.DatetimeIndex) -> np.ndarray:
    """Recalcul par boucle explicite + searchsorted, sans
    pandas.reindex/ffill/DateOffset."""
    raw = pd.read_csv(REPO_ROOT / "data" / "cpi_monthly.csv")
    raw["observation_date"] = pd.to_datetime(raw["observation_date"])
    raw = raw.dropna(subset=["CPIAUCSL"]).drop_duplicates("observation_date").sort_values("observation_date")
    obs_dates = raw["observation_date"].values
    vals = raw["CPIAUCSL"].astype(float).values
...
def main():
    lines = ["# Audit — Inflation réalisée US (FRED CPIAUCSL)", ""]
    all_ok = True
    ...
    for name, fname in MARKETS.items():
        ...
        cpi_lag_official = load_cpi_growth_lag(dates, cpi_series)[1:]
        pos_official = expanding_tercile_cut_high(cpi_lag_official)

        cpi_lag_manual = manual_cpi_growth_lag(dates)[1:]
        diff_align = np.nanmax(np.abs(cpi_lag_official - cpi_lag_manual))

        start = int(np.argmax(np.isfinite(cpi_lag_manual)))
        T = len(cpi_lag_manual)
        pos_manual = np.full(T, np.nan)
        for t in range(start, T):
            if not np.isfinite(cpi_lag_manual[t]):
                continue
            hist = cpi_lag_manual[start:t + 1]
            hist = hist[np.isfinite(hist)]
            thresh = manual_percentile_high_tercile(hist)
            pos_manual[t] = CUT if cpi_lag_manual[t] >= thresh else 1.0
        ...
        for i in np.where(mask)[0][pos_diff_mask]:
            local_diff = abs(cpi_lag_official[i] - cpi_lag_manual[i])
            if local_diff >= 1e-10:
    ...
    raw = pd.read_csv(REPO_ROOT / "data" / "cpi_monthly.csv")
    raw["observation_date"] = pd.to_datetime(raw["observation_date"])
    raw = raw.dropna(subset=["CPIAUCSL"]).sort_values("observation_date")
    vals = raw["CPIAUCSL"].astype(float).values
    obs_dates = raw["observation_date"].values
    ...
    cpi_check = load_cpi_growth_lag(check_dates, cpi_series)
    ...
    lines.append(f"- CPIGrowth_lag({check_dates[1].date()}) = {cpi_check[1]:.6f}, "
                 f"CPIGrowth_lag({check_dates[2].date()}) = {cpi_check[2]:.6f} "
                 f"(doivent valoir {g_prev:.6f}, JAMAIS {g_last:.6f})")
    lines.append(f"- CPIGrowth_lag({check_dates[3].date()}) = {cpi_check[3]:.6f}, "
                 f"CPIGrowth_lag({check_dates[4].date()}) = {cpi_check[4]:.6f} "
                 f"({check_dates[3].date()} doit être le premier jour où {g_last:.6f} apparaît, via shift(1))")
    ok_shift = (not np.isclose(g_prev, g_last)) and np.isclose(cpi_check[1], g_prev) and \
        np.isclose(cpi_check[2], g_prev) and np.isclose(cpi_check[3], g_last) and np.isclose(cpi_check[4], g_last)
    all_ok &= ok_shift
    ...
    dates = pd.DatetimeIndex(df["date"].values)
    cpi_lag_full = load_cpi_growth_lag(dates, cpi_series)[1:]
    pos_full = expanding_tercile_cut_high(cpi_lag_full)
    valid_start = int(np.argmax(np.isfinite(cpi_lag_full)))

    # Verification anti-lookahead a PLUSIEURS points de troncature
    # (etant donne l'enjeu du resultat PASS, verification renforcee).
    all_trunc_ok = True
    for cut_offset in (2000, 4000, 6000):
        T_CUT = valid_start + cut_offset
        if T_CUT >= len(cpi_lag_full):
            continue
        pos_truncated = expanding_tercile_cut_high(cpi_lag_full[:T_CUT])
        diff = np.nanmax(np.abs(pos_full[valid_start:T_CUT] - pos_truncated[valid_start:T_CUT]))
        ok_this = diff < 1e-8
        all_trunc_ok &= ok_this
        lines.append(f"## Anti-lookahead (NDX, troncature à {T_CUT} séances)")
        lines.append(f"- Écart max position sur la zone valide, pleine série vs série tronquée : {diff:.2e}")
        lines.append(f"- **{'OK — aucune fuite' if ok_this else 'FUITE DÉTECTÉE'}**")
        lines.append("")
    all_ok &= all_trunc_ok

    lines.append(f"## Verdict global : **{'CONFORME' if all_ok else 'ÉCART DÉTECTÉ'}**")

    out = ROOT / "results" / "nonml_cpi_inflation_overlay_audit.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
    return all_ok
