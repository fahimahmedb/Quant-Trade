"""Backtest — Overlay de vol-targeting DÉFENSIF uniquement (spécification
pré-enregistrée dans PREREG_defensive_vol_targeting_overlay.md,
committée avant ce script). Variante du #43 avec CAP=1.0x (jamais de
levier). n_trials=1, aucune dépendance ML. Règle de succès renforcée.
"""
import sys
...
def vol_target_position(r: np.ndarray) -> np.ndarray:
    """r = rendements log quotidiens. Renvoie la position continue
    clip(vol_cible / vol_realisee(t-1), 0, CAP), causale (vol connue a
    t-1, appliquee au rendement t). Identique au #43 sauf CAP=1.0x."""
...
def main():
    lines = [
        ...
        f"— jamais de levier au-dessus de 1.0x (variante du #43, CAP=2.0x). "
    ...
    for name, fname in MARKETS.items():
        ...
        pos_full = vol_target_position(bh_full)
        start = VOL_WINDOW
        bh_t = bh_full[start:]
        pos = pos_full[start:]
    ...
    out = ROOT / "results" / "nonml_defensive_vol_targeting_overlay_result.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
