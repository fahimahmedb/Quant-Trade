"""Audit adversarial — Leaders 52-semaines + overlay SMA200.

Vérifie (1) l'alignement causal (ffill uniquement) entre le calendrier
indice (NDX-100 OHLC) et le calendrier portefeuille (tickers individuels,
calendrier légèrement différent), (2) l'exposition totale conforme, (3)
un test anti-lookahead sur le signal de tendance lui-même.
"""
import sys
...
from nonml_sma200_leaders_overlay_backtest import (  # noqa: E402
    load_prices, index_trend_series, LOOKBACK, REBAL_EVERY, TERCILE, CAP, SMA_WINDOW,
)
...
def main():
    ...
    # combien de dates du portefeuille ne matchent pas exactement une date de l'indice
    # (necessitent un ffill) -- mesure la qualite de l'alignement calendaire
    n_exact_match = int(P.index.isin(trend.index).sum())
    n_total = len(P.index)

    # test anti-lookahead sur le signal de tendance indice lui-meme
    ...
    out = ROOT / "results" / "nonml_sma200_leaders_overlay_audit.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
