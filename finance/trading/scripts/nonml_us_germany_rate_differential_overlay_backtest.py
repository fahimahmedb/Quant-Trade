"""Backtest — Différentiel de taux US-Allemagne (DGS10-DE10Y), overlay
défensif (spécification pré-enregistrée dans
PREREG_us_germany_rate_differential_overlay.md, committée avant ce
...
#175/#178/#186/#187/#191/#192/#193 pour DGS10, CUT=0,5 et la technique de
tercile expanding déjà établie aux #169/#177/#183/#191/#192/#193, Règle
7. La série DE10Y (mensuelle) reçoit un traitement causal supplémentaire
(décalage d'un mois calendaire avant ffill) déclaré au PREREG §3.
"""
import sys
...
def load_dgs10_lag(dates: pd.DatetimeIndex) -> np.ndarray:
...
def load_de10y_lag(dates: pd.DatetimeIndex) -> np.ndarray:
    raw = pd.read_csv(REPO_ROOT / "data" / "de10y_monthly.csv")
    raw["observation_date"] = pd.to_datetime(raw["observation_date"])
    # La moyenne du mois M n'est connaissable qu'apres la fin du mois M :
    # decalage d'un mois calendaire complet AVANT le ffill (PREREG S3).
    raw["available_date"] = raw["observation_date"] + pd.DateOffset(months=1)
    s = raw.set_index("available_date")["IRLTLT01DEM156N"].astype(float).dropna()
    s = s[~s.index.duplicated(keep="first")].sort_index()
    y = s.reindex(dates, method="ffill")
    return y.shift(1).values


def expanding_tercile_cut_high(diff: np.ndarray) -> np.ndarray:
    T = len(diff)
    pos = np.full(T, np.nan)
    start = int(np.argmax(np.isfinite(diff)))
    for t in range(start, T):
        ...
        pos[t] = CUT if diff[t] >= thresh else 1.0
    return pos
...
def main():
    lines = [
        ...
        "",
        f"`position(t) = {CUT}x` si RateDiff(t)=DGS10_lag(t)-DE10Y_lag_décalé(t) est dans son tercile "
    ...
    out = ROOT / "results" / "nonml_us_germany_rate_differential_overlay_result.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
    return verdict
