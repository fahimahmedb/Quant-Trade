"""Simulation — 300 EUR, Winners momentum + overlay proximité plus haut
...
précédents. **Prudence forte héritée du #14.**
"""
import sys
...
from nonml_winners_index52w_high_overlay_backtest import (  # noqa: E402
    load_prices, index_trend_series, SIGNAL_WINDOW, REBAL_EVERY, TERCILE, COST_BPS, CAP,
)
...
def main():
    ...
    lines = [
        "# Simulation — 300 EUR, Winners momentum + overlay proximité plus haut 52-semaines indice (~3 derniers mois)",
        "",
        ...
        f"| **Winners + overlay 52w-high indice x{CAP}** | **{equity_lev[-1]:.2f} EUR** | "
        ...
        "robustesse (plateau croissant sur les deux grilles CAP 1.5x-3.0x et seuil 90-97%). "
        "Généralisabilité incertaine hors du bull market échantillonné (même mise en garde "
        "que le #14)."
    ]

    out = ROOT / "results" / "nonml_winners_index52w_high_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
