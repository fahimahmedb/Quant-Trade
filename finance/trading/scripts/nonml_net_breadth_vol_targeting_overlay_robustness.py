"""Robustesse — Overlay vol-targeting gaté par la breadth nette
hauts-bas NDX-100. Grille CAP ET grille de fenêtre de vol (PAS un
retuning de la porte, seuil zéro naturel non ajustable), autour des
...
from nonml_net_breadth_vol_targeting_overlay_backtest import (  # noqa: E402
    compute_net_breadth_series, COST_BPS, TARGET_VOL_ANNUAL, ANNUALIZATION, INDEX_LOOKBACK,
)
...
def main():
    ...
    lines = [
        "# Robustesse — Overlay vol-targeting gaté par la breadth nette hauts-bas (grilles CAP et fenêtre, PAS un retuning de la porte)",
        "",
    ...
    out = ROOT / "results" / "nonml_net_breadth_vol_targeting_overlay_robustness.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
