"""Backtest — Mises en chantier de logements US (HOUST, glissement
annuel), overlay défensif (spécification pré-enregistrée dans
PREREG_housing_starts_overlay.md, committée avant ce script). n_trials=1,
...
#191/#192/#193/#195/#198/#199/#200/#202/#203/#204/#205/#206, et le
traitement causal mensuel (décalage d'un mois avant ffill) déjà validé
au #195/#203/#204/#205/#206, Règle 7.
"""
import sys
...
def build_houst_growth_series() -> pd.Series:
    raw = pd.read_csv(REPO_ROOT / "data" / "houst_monthly.csv")
    raw["observation_date"] = pd.to_datetime(raw["observation_date"])
    raw = raw.dropna(subset=["HOUST"]).drop_duplicates("observation_date").sort_values("observation_date")
    vals = raw["HOUST"].astype(float).values
    ...
    # Publication ~3 semaines apres la fin du mois : decalage d'un mois
    # calendaire complet avant le ffill (meme convention que #195/#203).
...
def load_houst_growth_lag(dates: pd.DatetimeIndex, houst_series: pd.Series) -> np.ndarray:
    y = houst_series.reindex(dates, method="ffill")
...
def expanding_tercile_cut_low(growth: np.ndarray) -> np.ndarray:
    ...
    (chute des mises en chantier = faiblesse economique = defavorable)."""
    T = len(growth)
...
def main():
    ...
    lines = [
        ...
        "",
        f"`position(t) = {CUT}x` si HoustGrowth(t)=log(HOUST(t)/HOUST(t-12)) est dans son tercile "
    ...
    for name, fname in MARKETS.items():
        ...
        houst_lag_full = load_houst_growth_lag(dates, houst_series)
        houst_lag = houst_lag_full[1:]
        pos_full = expanding_tercile_cut_low(houst_lag)
        ...
        if name == "NDX (40 ans)":
            ...
            np.savez(ROOT / "results" / "nonml_housing_starts_overlay_pnl.npz",
    ...
    out = ROOT / "results" / "nonml_housing_starts_overlay_result.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
    return verdict
