"""Backtest — Croissance des prêts commerciaux et industriels US
(BUSLOANS, glissement annuel), overlay défensif (spécification
pré-enregistrée dans PREREG_business_loans_overlay.md, committée avant
...
#195/#203/#283/#284/#295 (Règle 7). Canal de disponibilité du crédit
bancaire aux entreprises, distinct du spread de crédit de marché (#199)
et des défauts de consommation (#286/#288/#289).
"""
import sys
...
def build_busloans_growth_series() -> pd.Series:
    raw = pd.read_csv(REPO_ROOT / "data" / "business_loans_monthly.csv")
    raw["observation_date"] = pd.to_datetime(raw["observation_date"])
    raw = raw.dropna(subset=["BUSLOANS"]).drop_duplicates("observation_date").sort_values("observation_date")
    vals = raw["BUSLOANS"].astype(float).values
    ...
    # Decalage d'un mois calendaire avant ffill (meme convention que
    # #195/#203/#283/#295).
...
def load_busloans_growth_lag(dates: pd.DatetimeIndex, busloans_series: pd.Series) -> np.ndarray:
    y = busloans_series.reindex(dates, method="ffill")
...
def expanding_tercile_cut_low(growth: np.ndarray) -> np.ndarray:
    ...
    (credit crunch = faiblesse economique = defavorable)."""
    T = len(growth)
...
def main():
    busloans_series = build_busloans_growth_series()

    lines = [
        "# Résultat — Croissance des prêts commerciaux et industriels US (BUSLOANS, glissement annuel), overlay défensif (pré-enregistré)",
        "",
        f"`position(t) = {CUT}x` si BusLoanGrowth(t)=log(BUSLOANS(t)/BUSLOANS(t-12)) est dans son "
    ...
    for name, fname in MARKETS.items():
        ...
        busloans_lag_full = load_busloans_growth_lag(dates, busloans_series)
        busloans_lag = busloans_lag_full[1:]
        pos_full = expanding_tercile_cut_low(busloans_lag)
        ...
        if name == "NDX (40 ans)":
            ...
            np.savez(ROOT / "results" / "nonml_business_loans_overlay_pnl.npz",
    ...
    out = ROOT / "results" / "nonml_business_loans_overlay_result.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
    return verdict
