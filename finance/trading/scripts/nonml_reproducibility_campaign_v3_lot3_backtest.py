"""Campagne de reproductibilité v3, LOT 3 — dernier lot au bénéfice net (#441).
...
OUT = RESULTS / "nonml_reproducibility_campaign_v3_lot3_result.md"

SEED, SAMPLE_SIZE, TIMEOUT_S = 20260819, 24, 300
SEED_LOT1, SIZE_LOT1 = 20260817, 24   # lot 1 du #438, a exclure
SEED_LOT2, SIZE_LOT2 = 20260818, 24   # lot 2 du #440, a exclure
DENOM_PREV = 47                       # tirages retenus cumules #438 + #440
...
def run(name):
    ...
    Correction du #439 REPORTEE ICI AVANT LE TIRAGE (#440) : `subprocess.run`
    avec `timeout` ne tue que l'enfant direct ; les petits-enfants orphelins
    survivaient et ont reecrit un rapport publie APRES sa restauration.
    """
...
def main():
    pool = eligible()
    lot1 = set(random.Random(SEED_LOT1).sample(pool, SIZE_LOT1))
    rest1 = [n for n in pool if n not in lot1]
    lot2 = set(random.Random(SEED_LOT2).sample(rest1, SIZE_LOT2))
    remaining = [n for n in rest1 if n not in lot2]
    ...
    if substantive:
        ...
        b_lot = 1.0 - 0.05 ** (1.0 / denom)
        b = 1.0 - 0.05 ** (1.0 / cum)
        L.append(f"- tirages retenus **ce lot** : **{denom}** "
                 f"({len(identical)} identiques + {len(substantive)} substantielles)")
        L.append(f"- tirages retenus **aux #438 + #440** : **{DENOM_PREV}**")
        L.append("")
        L.append("| | Dénominateur | Borne à 95 % |")
        L.append("|---|---|---|")
        L.append(f"| cumul #438 + #440 | {DENOM_PREV} | 6,2 % |")
        L.append(f"| ce lot seul | {denom} | {100*b_lot:.1f} % |")
        L.append(f"| **cumul #438 + #440 + #441** | **{cum}** | **{100*b:.1f} %** |")
        L.append("")
