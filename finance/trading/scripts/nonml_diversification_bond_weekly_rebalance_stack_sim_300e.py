"""Simulation — 300 EUR dans l'empilement diversification obligataire +
rebalancement hebdomadaire (#137), ~3 derniers mois. Aucun paramètre
...
def main():
    d = np.load(ROOT / "results" / "nonml_diversification_bond_weekly_rebalance_stack_pnl.npz", allow_pickle=True)
    ...
    lines = [
        ...
        f"| **#131 + diversification obligataire** | **{equity_ov[-1]:.2f} EUR** | "
        ...
        "réel reste celui du backtest complet (PASS niveau 1 standard ET Calmar, MDD -46,5% le meilleur "
        "jamais obtenu dans ce backlog, plateau de robustesse parfait 3/3 sur la grille de maturité). "
        "Doit encore passer la batterie Règle 9 "
        "(`nonml_pass_validation_battery.py diversification_bond_weekly_rebalance_stack`).",
    ]

    out = ROOT / "results" / "nonml_diversification_bond_weekly_rebalance_stack_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
