"""Robustesse — Overlay vol-targeting gaté par la statistique de
Ljung-Box glissante. Grille CAP ET grille de fenêtre de vol (PAS un
retuning de LB_WINDOW, LB_MAXLAG ni de MEDIAN_WINDOW, les paramètres du
signal de porte lui-même), autour des valeurs pré-enregistrées (CAP=2.0x,
fenêtre=20j).
"""
import sys
...
from nonml_ljung_box_clustering_vol_targeting_overlay_backtest import (  # noqa: E402
    calm_lb_mask, COST_BPS, TARGET_VOL_ANNUAL, ANNUALIZATION,
    LB_WINDOW, MEDIAN_WINDOW, MARKETS,
)
...
def main():
    ...
    lines = [
        "# Robustesse — Overlay vol-targeting gaté par la statistique de Ljung-Box glissante "
        "(grilles CAP et fenêtre de vol, PAS un retuning de LB_WINDOW/LB_MAXLAG/MEDIAN_WINDOW)",
        "",
        "CAP pré-enregistré = 2.0x, fenêtre de vol pré-enregistrée = 20j. LB_WINDOW=252, "
        "LB_MAXLAG=22 et MEDIAN_WINDOW=252 (paramètres du signal de porte lui-même) restent "
        "fixes, comme pour toutes les portes précédentes de cette famille "
        f"(#47/#54/#57/#78/#80/#216-#223/#234/#237/#240, {len(data)} marchés).",
        "",
    ...
    out = ROOT / "results" / "nonml_ljung_box_clustering_vol_targeting_overlay_robustness.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
