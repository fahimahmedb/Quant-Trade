"""Robustesse — Overlay vol-targeting gaté par la breadth de drawdown
profond (seuil absolu -20%). Grille CAP ET grille de fenêtre de vol
(PAS un retuning du seuil -20% ni de la fenêtre 252j, paramètres de
...
from nonml_deep_drawdown_breadth_vol_targeting_overlay_backtest import (  # noqa: E402
    compute_deep_drawdown_breadth_series, MEDIAN_WINDOW, COST_BPS, TARGET_VOL_ANNUAL, ANNUALIZATION,
)
...
def main():
    ...
    out = ROOT / "results" / "nonml_deep_drawdown_breadth_vol_targeting_overlay_robustness.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
