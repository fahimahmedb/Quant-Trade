"""Robustesse — Rebalancement hebdomadaire du #149 (#154), grille des
fréquences (identique au #131, PAS un retuning).
"""
import sys
...
from nonml_cash_rate_correction_44_weekly_rebalance_backtest import COST_BPS, weekly_hold_position, SOURCES  # noqa: E402
...
def main():
    lines = [
        "# Robustesse — Rebalancement hebdomadaire du #149 (#154), grille des fréquences (PAS un retuning)",
        "",
    ...
    for name, fname in SOURCES.items():
        ...
        lines.append(f"## {name}")
        lines.append("")
        lines.append("| REBAL_FREQ | Sharpe ann. | Rendement total | MDD | PASS |")
        lines.append("|---|---|---|---|---|")
        n_pass = 0
        ...
        lines.append("")
        lines.append(f"Plateau {name} : {n_pass}/{len(FREQ_GRID)} fréquences PASS.")
        lines.append("")

    out = ROOT / "results" / "nonml_cash_rate_correction_44_weekly_rebalance_robustness.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
