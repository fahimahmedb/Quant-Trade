"""Simulation — 300 EUR, Leaders 52-semaines + overlay proximité plus
haut 52-semaines indice, ~3 derniers mois. Spécification pré-enregistrée
...
from nonml_leaders_index52w_high_overlay_backtest import (  # noqa: E402
    load_prices, index_trend_series, LOOKBACK, REBAL_EVERY, TERCILE, COST_BPS, CAP,
)
...
def main():
    ...
    lines = [
        "# Simulation — 300 EUR, Leaders 52-semaines + overlay proximité plus haut 52-semaines indice (~3 derniers mois)",
        "",
        ...
        f"| **Leaders + overlay 52w-high indice x{CAP}** | **{equity_lev[-1]:.2f} EUR** | "
        ...
        "statistique réel reste celui du backtest complet (2022-2026, PASS exceptionnel) et "
        "de la robustesse (plateau parfait sur les deux grilles CAP 1.5x-3.0x et seuil 90-97%)."
    ]

    out = ROOT / "results" / "nonml_leaders_index52w_high_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
