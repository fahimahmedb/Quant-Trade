"""Audit indépendant — Anticipations d'inflation implicites (breakeven 10 ans).
...
from nonml_inflation_breakeven_overlay_backtest import (  # noqa: E402
    COST_BPS, CUT, MARKETS, load_t10yie_lag, expanding_tercile_cut_high,
)


def manual_t10yie_lag(dates: pd.DatetimeIndex) -> np.ndarray:
    raw = pd.read_csv(REPO_ROOT / "data" / "t10yie_daily.csv")
    raw["observation_date"] = pd.to_datetime(raw["observation_date"])
    obs = raw.dropna(subset=["T10YIE"]).sort_values("observation_date")
    obs_dates = obs["observation_date"].values
    vals = obs["T10YIE"].astype(float).values
...
def main():
    lines = ["# Audit — Anticipations d'inflation implicites (breakeven 10 ans)", ""]
    all_ok = True
    ...
    for name, fname in MARKETS.items():
        ...
        t10yie_lag_official = load_t10yie_lag(dates)[1:]
        pos_official = expanding_tercile_cut_high(t10yie_lag_official)

        t10yie_lag_manual = manual_t10yie_lag(dates)[1:]
        diff_align = np.nanmax(np.abs(t10yie_lag_official - t10yie_lag_manual))

        start = int(np.argmax(np.isfinite(t10yie_lag_manual)))
        T = len(t10yie_lag_manual)
        pos_manual = np.full(T, np.nan)
        for t in range(start, T):
            if not np.isfinite(t10yie_lag_manual[t]):
                continue
            hist = t10yie_lag_manual[start:t + 1]
            hist = hist[np.isfinite(hist)]
            thresh = manual_percentile_high_tercile(hist)
            pos_manual[t] = CUT if t10yie_lag_manual[t] >= thresh else 1.0
        ...
        for i in np.where(mask)[0][pos_diff_mask]:
            local_diff = abs(t10yie_lag_official[i] - t10yie_lag_manual[i])
            if local_diff >= 1e-10:
        ...
        if n_pos_diff == 0:
            ...
            lines.append(f"- **{n_pos_diff}/{n_total} désaccords de position** — sensibilité de bord flottante "
                         f"documentée (même pattern que #193/#195/#196/#197/#198/#199) : {'confirmé' if boundary_tie_ok else 'NON confirmé, écart réel'}.")
    ...
    valid_start = int(np.argmax(np.isfinite(t10yie_lag_full)))
    T_CUT = valid_start + 2000
    pos_full = expanding_tercile_cut_high(t10yie_lag_full)
    pos_truncated = expanding_tercile_cut_high(t10yie_lag_full[:T_CUT])
    ...
    out = ROOT / "results" / "nonml_inflation_breakeven_overlay_audit.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
    return all_ok
