"""Backtest — Porte combinée (ET) défaut carte de crédit (#286) +
conditions financières NFCI (#291), overlay défensif (spécification
pré-enregistrée dans PREREG_delinquency_nfci_combined_overlay.md,
committée avant ce script). n_trials=1, aucune dépendance ML. Règle de
succès renforcée sur 5 marchés (≥4/5).

Réutilisation STRICTE des deux définitions déjà validées (Règle 7),
aucune modification : build_delinquency_series/load_delinquency_lag du
#286, build_nfci_series/load_nfci_lag du #291. Même esprit que la porte
combinée kurtosis+ν du #240.
"""
import sys
...
def expanding_tercile_gate_high(level: np.ndarray) -> np.ndarray:
    """Booleen : level(t) est dans son tercile expanding le PLUS HAUT.
    Identique en substance a expanding_tercile_cut_high des #286/#291,
    mais renvoie un booleen (pour combinaison ET) plutot qu'une position."""
    T = len(level)
...
def main():
    ...
    lines = [
        ...
        "",
        f"`position(t) = {CUT}x` si DRCCLACBS_lag(t-1) ET NFCI_lag(t-1) sont TOUS DEUX dans leur "
    ...
    for name, fname in MARKETS.items():
        ...
        if name == "NDX (40 ans)":
            ...
            np.savez(ROOT / "results" / "nonml_delinquency_nfci_combined_overlay_pnl.npz",
    ...
    out = ROOT / "results" / "nonml_delinquency_nfci_combined_overlay_result.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
    return verdict
