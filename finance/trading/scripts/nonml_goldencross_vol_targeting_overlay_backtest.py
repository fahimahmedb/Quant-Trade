"""Backtest — Overlay vol-targeting gaté par le golden cross (SMA50>SMA200,
spécification pré-enregistrée dans PREREG_goldencross_vol_targeting_overlay.md,
committée avant ce script). Combine les cycles #34 (porte) et #46/#47
...
def golden_cross_mask(close: np.ndarray) -> np.ndarray:
    sma_short = pd.Series(close).rolling(SMA_SHORT).mean().values
    sma_long = pd.Series(close).rolling(SMA_LONG).mean().values
...
def combined_position(close: np.ndarray, r: np.ndarray) -> np.ndarray:
    ...
    vol_realisee(t-1), 1.0, CAP) si golden cross actif, 1.0 sinon."""
    gc = golden_cross_mask(close)  # aligne sur close (longueur T)
    # meme convention causale que #47 : gc[i] connu a la cloture du jour
    # i (jour de decision) s'applique a r[i]=log(close[i+1]/close[i])
    # -> gc[:-1], PAS gc[1:] (fuite d'un jour)
    gc_r = gc[:-1]
    ...
    pos = np.where(gc_r, vt_exposure, 1.0)
...
def main():
    lines = [
        ...
        f"si SMA{SMA_SHORT} > SMA{SMA_LONG} (golden cross), sinon 1.0x. Échantillon testable = à "
        f"partir de la {SMA_LONG+1}e séance.",
        "",
    ...
    for name, fname in MARKETS.items():
        ...
        if fname == "nasdaq100_daily.txt":
            np.savez(ROOT / "results" / "nonml_goldencross_vol_targeting_overlay_pnl.npz",
    ...
    out = ROOT / "results" / "nonml_goldencross_vol_targeting_overlay_result.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
