"""Robustesse — Overlay vol-targeting gaté par la pente SMA200. Grille
CAP ET grille de fenêtre de vol (PAS un retuning de la porte SMA200/
SLOPE_LAG), autour des valeurs pré-enregistrées (CAP=2.0x, fenêtre=20j).
"""
import sys
...
from nonml_slope_vol_targeting_overlay_backtest import (  # noqa: E402
    sma_slope_positive_mask, COST_BPS, SMA_WINDOW, SLOPE_LAG, TARGET_VOL_ANNUAL, ANNUALIZATION, MARKETS,
)
...
def position_for(close: np.ndarray, r: np.ndarray, cap: float, vol_window: int) -> np.ndarray:
    slope = sma_slope_positive_mask(close)
    slope_r = slope[:-1]
    ...
    return np.where(slope_r, vt, 1.0)


def run_one(close: np.ndarray, r: np.ndarray, cap: float, vol_window: int) -> bool:
...
def main():
    ...
    lines = [
        "# Robustesse — Overlay vol-targeting gaté par pente SMA200 (grilles CAP et fenêtre, PAS un retuning de la porte)",
        "",
    ...
    for cap in CAP_GRID:
        n_pass = sum(run_one(close, r, cap, 20) for close, r in data.values())
    ...
    for window in WINDOW_GRID:
        n_pass = sum(run_one(close, r, 2.0, window) for close, r in data.values())
        marker = " ← fenêtre pré-enregistrée" if window == 20 else ""
        lines.append(f"| {window}j | {n_pass}/{len(data)}{marker} |")

    out = ROOT / "results" / "nonml_slope_vol_targeting_overlay_robustness.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
