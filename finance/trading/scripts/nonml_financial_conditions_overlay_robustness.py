"""Robustesse — cycle #289 (conditions financières NFCI), perturbation
...
from nonml_financial_conditions_overlay_backtest import (  # noqa: E402
    COST_BPS, CUT, MARKETS, load_nfci_lag, build_nfci_series, TERCILE_PCT,
)
...
def main():
    ...
    lines = [
        "# Robustesse — cycle #289, perturbation ±20% du plancher CUT",
        "",
    ...
    for name, fname in MARKETS.items():
        ...
        for cut in CUT_GRID:
            pos_full = expanding_tercile_cut_high(nfci_lag_full, cut)
    ...
    out = ROOT / "results" / "nonml_financial_conditions_overlay_robustness.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
