"""Backtest — Overlay vol-targeting gaté par la breadth SMA200 NDX-100
(spécification pré-enregistrée dans
PREREG_sma200_breadth_vol_targeting_overlay.md, committée avant ce
script). Breadth = fraction des titres NDX-100 au-dessus de leur propre
SMA200. n_trials=1, aucune dépendance ML. Règle de succès renforcée.
"""
import json
import sys
...
def compute_sma200_breadth_series() -> pd.Series:
    """Renvoie Breadth(t) = fraction des titres cotes (avec SMA200
    calculable) dont le prix est AU-DESSUS de leur propre SMA200,
    ...
    n_calc = calculable.sum(axis=1)
    n_above = (above & calculable).sum(axis=1)
    with np.errstate(divide="ignore", invalid="ignore"):
        breadth = np.where(n_calc > 0, n_above / n_calc, np.nan)
...
def combined_position(close: np.ndarray, r: np.ndarray, gate_aligned: np.ndarray) -> np.ndarray:
    ...
    breadth SMA200 alignee sur close (longueur T, booleenne). Renvoie
    la position pour chaque rendement de r."""
    gate_r = gate_aligned[:-1]  # meme convention causale que #47/#57/#77/#89/#90/#94
...
def main():
    ...
    breadth = compute_sma200_breadth_series()
    ...
    lines = [
        ...
        f"si Breadth SMA200(t) ≥{100*BREADTH_THRESHOLD:.0f}% (fraction des titres NDX-100 au-dessus de "
        f"leur propre SMA{SMA_WINDOW}), sinon 1.0x. {len(bh_t)} séances testables.",
        "",
        f"%j porte breadth SMA200 active : {100*gate_active.mean():.1f}%",
        f"Position moyenne : {pos.mean():.2f}x",
        f"Breadth SMA200 moyenne (toute la période) : {100*np.nanmean(breadth.values):.1f}%",
        "",
        "| | Sharpe ann. | Rendement total net | MDD |",
        "|---|---|---|---|",
        f"| Buy&Hold (NDX) | {me_bh['sharpe_ann']:+.2f} | {100*ret_bh:+.1f}% | {me_bh['max_drawdown_pct']:.1f}% |",
        f"| **Overlay vol-targeting gaté breadth SMA200** | **{me_ov['sharpe_ann']:+.2f}** | "
    ...
    np.savez(ROOT / "results" / "nonml_sma200_breadth_vol_targeting_overlay_pnl.npz",
             pos=pos, r_asset=bh_t, dates=dates_idx.values[1:][start:], cost_bps=COST_BPS)

    out = ROOT / "results" / "nonml_sma200_breadth_vol_targeting_overlay_result.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
