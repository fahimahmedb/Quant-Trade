"""Robustesse — Correction taux réaliste sur le #44 (#149), grille de la
maturité de référence (identique au #134, PAS un retuning de la
position équity ni de TARGET_VOL_ANNUAL=15%).
"""
import sys
...
from nonml_cash_rate_correction_defensive_vol_targeting_44_backtest import COST_BPS, load_dgs10, bond_return_proxy  # noqa: E402
...
def main():
    ...
    lines = [
        "# Robustesse — Correction taux réaliste sur le #44 (#149), grille de maturité (PAS un retuning)",
        "",
        "Maturité pré-enregistrée = 10 ans. Grille 5-20 ans (position équity #44 strictement inchangée).",
        "",
        "| Maturité | Sharpe ann. | Rendement total | MDD | PASS standard |",
        "|---|---|---|---|---|",
    ]
    n_pass = 0
    for maturity in MATURITY_GRID:
        ...
        lines.append(
            ...
            f"{me_ov['max_drawdown_pct']:.1f}% | {'OUI' if ok_std else 'non'} |"
        )
    lines.append("")
    lines.append(f"Plateau : {n_pass}/{len(MATURITY_GRID)} maturités PASS standard.")

    out = ROOT / "results" / "nonml_cash_rate_correction_defensive_vol_targeting_44_robustness.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
