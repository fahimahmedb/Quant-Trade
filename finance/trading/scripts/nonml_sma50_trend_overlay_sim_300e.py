"""Simulation — 300 EUR dans l'overlay filtre de tendance SMA50 (NDX),
...
from nonml_sma50_trend_overlay_backtest import above_sma_mask, COST_BPS, CAP, SMA_WINDOW  # noqa: E402
...
def main():
    ...
    pos_full_all = np.where(above_full[:-1], CAP, 1.0)
    ...
    lines = [
        ...
        "du backtest complet (PASS 4/5 marchés) et de la robustesse (4-5/5 sur la grille CAP "
        "1.5x-3.0x)."
    ]

    out = ROOT / "results" / "nonml_sma50_trend_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
