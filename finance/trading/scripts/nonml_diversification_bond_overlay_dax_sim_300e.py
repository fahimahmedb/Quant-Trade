"""Simulation — 300 EUR dans la diversification obligataire sur DAX
(#140), ~3 derniers mois. Aucun paramètre retouché après les résultats
précédents.
"""
import sys
...
def main():
    d = np.load(ROOT / "results" / "nonml_diversification_bond_overlay_dax_pnl.npz", allow_pickle=True)
    pos_eq_full, r_dax_full, r_bund_full = d["pos"], d["r_asset"], d["r_alt"]
    dates_full = pd.to_datetime(d["dates"])

    pos_eq = pos_eq_full[-WINDOW_DAYS:]
    r_dax = r_dax_full[-WINDOW_DAYS:]
    r_bund = r_bund_full[-WINDOW_DAYS:]
    ...
    lines = [
        ...
        "",
        f"Période : {dates[0].date()} → {dates[-1].date()} ({len(r_dax)} séances). MATURITY_YEARS=10, "
        "aucun paramètre retouché. Limite données (taux allemand mensuel) rappelée.",
        "",
        "| | Capital final | Rendement période | MDD | Sharpe ann. | Calmar |",
        "|---|---|---|---|---|---|",
        f"| BuyHold (DAX 100%) | {equity_bh[-1]:.2f} EUR | {ret_bh_pct:+.1f}% | {mdd(equity_bh):.1f}% | "
        ...
        f"**{calmar_ov:.3f}** |",
        "",
        "**Lecture honnête** : fenêtre courte (~3 mois) illustrative uniquement, ET limite déjà "
        "reconnue (taux allemand mensuel forward-fillé, moins probant que DGS10 quotidien) — le "
        "verdict statistique réel reste celui du backtest complet (PASS niveau 1 standard ET Calmar, "
        "plateau de robustesse parfait 3/3). Doit encore passer la batterie Règle 9 "
        "(`nonml_pass_validation_battery.py diversification_bond_overlay_dax`).",
    ]

    out = ROOT / "results" / "nonml_diversification_bond_overlay_dax_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
