"""Robustesse — Overlay union ToM+Halloween. Grille CAP (PAS un
...
from nonml_tom_halloween_union_overlay_backtest import tom_mask, halloween_mask, COST_BPS, MARKETS  # noqa: E402
...
def main():
    lines = [
        "# Robustesse — Overlay union ToM+Halloween (grille CAP, PAS un retuning)",
        "",
    ...
    out = ROOT / "results" / "nonml_tom_halloween_union_overlay_robustness.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
