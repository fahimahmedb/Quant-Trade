"""Simulation — 300 EUR, porte combinée ET défaut carte de crédit +
NFCI (cycle #296) sur les ~3 derniers mois disponibles (NDX). Aucun
paramètre retouché. Gabarit repris tel quel du #289/#286 (Règle 7).
"""
import sys
...
def main():
    ...
    assert np.isfinite(delinq_lag_full[-WINDOW_DAYS:]).all() and np.isfinite(nfci_lag_full[-WINDOW_DAYS:]).all(), \
    ...
    lines = [
        f"# Simulation — 300 EUR, porte combinée ET défaut carte + NFCI (cycle #296), {MARKET_NAME} (~3 derniers mois)",
        "",
        f"Période : {dates_w[0].date()} → {dates_w[-1].date()} ({len(r)} séances). "
        f"{100*frac_cut:.0f}% de la fenêtre est en régime coupé (défaut de paiement ET conditions financières "
        f"tous deux dans leur tercile expanding le plus haut).",
        "",
        "| | Capital final | Rendement période | MDD | Sharpe ann. |",
        "|---|---|---|---|---|",
        f"| Buy & Hold | {eq_bh[-1]:.2f} EUR | {100 * (eq_bh[-1] / CAPITAL0 - 1):+.1f}% | "
        f"{mdd_pct(eq_bh):.1f}% | {me_bh['sharpe_ann']:+.2f} |",
        f"| **Overlay porte combinée ET** | **{eq_ov[-1]:.2f} EUR** | "
        ...
        "et de la grille de robustesse (15/15, plateau cohérent).",
    ]

    out = ROOT / "results" / "nonml_delinquency_nfci_combined_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
