...
position moyenne dans le range annuel NDX-100, ~3 derniers mois.
...
from nonml_range_position_vol_targeting_overlay_backtest import (  # noqa: E402
    compute_range_position_series, combined_position, MEDIAN_WINDOW, CAP, COST_BPS,
)
...
def main():
    ...
    lines = [
        "# Simulation — 300 EUR, overlay vol-targeting gaté par la position moyenne dans le range annuel (NDX, ~3 derniers mois)",
        "",
        ...
        f"| **Overlay position range** | **{equity_ov[-1]:.2f} EUR** | "
    ...
    out = ROOT / "results" / "nonml_range_position_vol_targeting_overlay_sim_300e.md"
    out.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
    print(f"\nÉcrit dans {out}")
