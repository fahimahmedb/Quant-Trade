# ML-3 — Architecture unique bien régularisée : gradient boosting à early stopping purgé (nasdaq100_daily)
...
- OOS = 9522 séances (19/09/1988 → 10/07/2026), fenêtre strictement identique à l'Étape B officielle, à ML-1 et à ML-2.
...
| HistGB-ES | +0.43 | +0.56 | +0.06 | +11.8 % | -83.2 % | 1.08 | 0.081 | 1.00 |
...
| LogitL2 | 54.20 % | +18.42 |
| HistGB | 53.44 % | +17.56 |
| HistGB-ES | 55.49 % | +59.64 |

## 4. Capacité sélectionnée par l'early stopping (diagnostic, PREREG §5)

- Ré-estimations du candidat : **454** (dont 0 en mode de repli `max_iter=150`).
- `k*` retenu : min **10**, médiane **11**, moyenne **22**, max **231**.
- Blocs atteignant le plafond 400 : **0** (0.0 %) — le plafond n'est donc pas la contrainte active dans la grande majorité des cas.
- Part des blocs où `k*` < 150 (capacité **inférieure** à celle de l'Étape B) : **99.1 %**.

## 5. Deflated Sharpe Ratio

σ²(SR quotidiens des 5 signaux) = 4.2770e-04. Deux lectures :

| Signal | Sharpe quot. | DSR (n_trials=407, campagne ML entière) | DSR (n_trials=4, échelle Étape B) |
|---|---|---|---|
| BuyHold | +0.0328 | **0.002** | 0.859 |
| Momentum | -0.0178 | **0.000** | 0.000 |
| LogitL2 | +0.0219 | **0.000** | 0.507 |
| HistGB | +0.0288 | **0.001** | 0.755 |
| HistGB-ES | +0.0270 | **0.000** | 0.694 |
...
- **(A)** Sharpe HistGB-ES (+0.43) > Sharpe BuyHold (+0.52) **ET** rendement HistGB-ES (+11.8 %) > rendement BuyHold (+14.5 %) → **NON satisfait**.
- **(B)** Calmar HistGB-ES (+0.06) > Calmar BuyHold (+0.08) → **NON satisfait**.

### FAIL

Effet de la régularisation de capacité (early stopping purgé) sur la MÊME architecture : Sharpe +0.46 → +0.43 (-0.03), accuracy 53.44 % → 55.49 % (+2.05 pt), rendement annualisé +12.6 % → +11.8 %, MDD -66.9 % → -83.2 %, turnover 0.376 → 0.081/j, break-even +17.56 → +59.64 bps/trade.
...
- **Baseline recalculée, pas recopiée.** Le `HistGB` mesuré ici (+0.46 de Sharpe) peut différer du chiffre publié dans `results/etape_B_ndx100.md` (+0,23) : `triple_barrier_labels` a été modifié depuis (σ locale = écart-type glissant strict sur [t−20, t)). Les 5 lignes du §2 proviennent **du même run, du même code et des mêmes labels** — la comparaison avec/sans early stopping est donc interne et cohérente, et c'est d'elle que dépend le verdict.
...
- Fichier de positions sauvegardé pour audit : `results/ml_regularized_architecture_pnl.npz` (positions OOS, positions de la baseline HistGB, rendements, dates, coût, σ², n_trials, série des `k*`).
- **Erratum de rédaction (sans effet sur le calcul, ajouté À LA MAIN après génération de ce rapport — aucun chiffre modifié)** : le PREREG §4 écrit « les 21 colonnes endogènes » ; `build_features(df)` en produit en réalité **20** (valeur imprimée dynamiquement au §1 ci-dessus par le script lui-même). Le chiffre 21 était repris tel quel du PREREG ML-2, qui contenait déjà cette coquille. Aucune conséquence : le script consomme `build_features(df)` **en bloc**, sans sélection ni comptage de colonnes — l'univers de features est bien celui de l'Étape B officielle, inchangé. Signalé ici plutôt que corrigé dans le PREREG, qui ne doit pas être modifié après calcul (Règle 1). La même note est désormais générée automatiquement par le script (lecture Composite ci-après).
